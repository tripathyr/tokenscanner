...
def create_database_connection(type, parameters=None):
    ...
    echo_setting = False if type in ['token', 'smart_contract'] else False
    ...
    while True:
        try:
            ...
            session.add(RejectedTransactionHistory(
                ...
                blockchainReference=blockchainReference,
                jsonData=filter_json_data(json.dumps(transaction_data)),
...
def store_rejected_transaction(transaction_data, parsed_data, blockinfo):
    ...
    try:
        ...
        block_number = blockinfo.get("height", 0)
        tx_time = transaction_data.get("time", 0)
        ...
        if tx_type.startswith("smartContract") or tx_type.startswith("contract"):
            rejected_tx = RejectedContractTransactionHistory(
                ...
                sourceFloAddress = parsed_data.get("sourceFloAddress", ""),
                destFloAddress = parsed_data.get("destFloAddress", ""),
...
def process_continuous_event_transfer(
    ...
    # --- Ensure input/output amounts are Decimal 🛠️ FIX
    if len(inputlist) > 1:
    ...
    if contract_subtype == 'tokenswap':
        ...
        if contractStructure['pricetype'] in ['predetermined', 'determined']:
            swapPrice = Decimal(contractStructure['price'])
            ...
            logger.error(f"Unknown pricetype {contractStructure['pricetype']}")
            return 0
        ...
        active_contract_deposits = contract_session.query(
            ...
            ContractDeposits.status != 'deposit-return',
            ContractDeposits.status != 'consumed',
        ...
        # Transfer accepting_token (USD#) from participant to contract
        returnval = transferToken(
        ...
        for a_deposit in active_contract_deposits:
            ...
            # 🛠️ FIX: convert depositBalance to Decimal
            deposit_balance_decimal = Decimal(str(a_deposit.depositBalance))
            ...
            # Transfer accepting_token to depositor
            returnval = transferToken(
            ...
            # 🛠️ FIX: subtract Decimals safely
        ...
        firstInteractionCheck = systemdb_connection.execute(
            f"SELECT * FROM tokenAddressMapping WHERE tokenAddress='{inputlist[0]}' "
            f"AND token='{contractStructure['selling_token']}'"
        ).fetchall()
        if len(firstInteractionCheck) == 0:
            systemdb_connection.execute(
                f"INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) "
                f"VALUES ('{inputlist[0]}', '{contractStructure['selling_token']}', "
                f"'{transaction_data['txid']}', '{transaction_data['blockheight']}', '{transaction_data['blockhash']}')"
            )
...
def process_smart_contract_pays(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    # Load contract structure
    contractStructure = extract_contractStructure(parsed_data['contractName'], outputlist[0])

    # Check if output address matches incorporation address
    ...
    # Check for internal trigger contract
    ...
    if 'minimumsubscriptionamount' in contractStructure:
        ...
        if amountDeposited < minimumsubscriptionamount:
            ...
            for participant in contractParticipants:
                ...
                connection.execute(
                    f"UPDATE contractparticipants SET winningAmount={tokenAmount} WHERE participantAddress='{participantAddress}' AND transactionHash='{txHash}'"
                )
    ...
    if tokenSum > 0:
        contractWinners = connection.execute(
            f'SELECT * FROM contractparticipants WHERE userChoice="{parsed_data["triggerCondition"]}"'
        ).fetchall()

        winnerRows = connection.execute(
            f'SELECT tokenAmount FROM contractparticipants WHERE userChoice="{parsed_data["triggerCondition"]}"'
        ).fetchall()
        ...
        for winner in contractWinners:
            ...
            connection.execute(
                f"""INSERT INTO contractwinners
                (participantAddress, winningAmount, userChoice, transactionHash, blockNumber, blockHash, referenceTxHash)
                VALUES('{winner[1]}', {winnerAmount}, '{parsed_data['triggerCondition']}', '{transaction_data['txid']}',
                '{blockinfo['height']}', '{blockinfo['hash']}', '{winner[4]}');"""
            )
...
def process_smart_contract_deposit(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    if check_database_existence('smart_contract', {'contract_name': f"{parsed_data['contractName']}", 'contract_address': f"{outputlist[0]}"}):
        ...
        if 'contractAddress' in parsed_data:
            if parsed_data['contractAddress'] != outputlist[0]:
                rejectComment = f"Contract deposit at transaction {transaction_data['txid']} rejected as contractAddress specified in flodata, {parsed_data['contractAddress']}, does not match transaction output address {outputlist[0]}"
                logger.info(rejectComment)
                rejected_contract_transaction_history(transaction_data, parsed_data, 'deposit', outputlist[0], inputadd, outputlist[0], rejectComment)
                pushData_SSEapi(f"Error| Mismatch in contract address specified in flodata and transaction output address in {transaction_data['txid']}")
                return 0
        ...
        if returnval == 0:
            logger.info("Something went wrong in the token transfer method")
            pushData_SSEapi(f"Error | Something went wrong while doing internal db transactions for {transaction_data['txid']}")
            return 0

        # Save deposit in contract DB
        session = create_database_session_orm(
            'smart_contract',
            {'contract_name': f"{parsed_data['contractName']}", 'contract_address': f"{outputlist[0]}"},
            ContractBase
        )
        blockchainReference = neturl + 'tx/' + transaction_data['txid']
        ...
        pushData_SSEapi(
            f"Deposit Smart Contract Transaction {transaction_data['txid']} for smart contract {parsed_data['contractName']} at address {outputlist[0]}"
        )

        # Add token mapping if new
        systemdb_connection = create_database_connection('system_dbs', {'db_name': 'system'})
        firstInteractionCheck = systemdb_connection.execute(
            f"SELECT * FROM tokenAddressMapping WHERE tokenAddress='{outputlist[0]}' AND token='{parsed_data['tokenIdentification']}'"
        ).fetchall()
        if len(firstInteractionCheck) == 0:
            systemdb_connection.execute(
                f"INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) "
                f"VALUES ('{outputlist[0]}', '{parsed_data['tokenIdentification']}', '{transaction_data['txid']}', "
                f"'{transaction_data['blockheight']}', '{transaction_data['blockhash']}')"
            )
        systemdb_connection.close()
        ...
        rejectComment = f"Deposit Transaction {transaction_data['txid']} rejected because Smart Contract {parsed_data['contractName']} at address {outputlist[0]} does not exist"
        logger.info(rejectComment)
        rejected_contract_transaction_history(
            transaction_data,
            parsed_data,
...
def scanBlockchain():
    ...
    while True:
        try:
            ...
            logger.info(
                f"Unable to connect to 'system' database... retrying in {DB_RETRY_TIMEOUT} seconds"
            )
            time.sleep(DB_RETRY_TIMEOUT)
    ...
    # Guard against scanning when already up-to-date
