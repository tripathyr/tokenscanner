...
async def clean_idle_connections_with_async_pool(idle_time_threshold=3):
    ...
    try:
        ...
        async with async_connection_pool.acquire() as conn:
            async with conn.cursor() as cursor:
                # Fetch idle connections (limit to 10 for batch processing)
                query = f"""
                SELECT id, time 
                ...
                # Terminate connections concurrently
                ...
                kill_end = time.time()
                print(f"KILL commands executed in {kill_end - kill_start:.2f} seconds.")
