    ...
    while True:
        try:
            ...
            try:
                ...
                if existing_block:
                    logger.info(f"Block {block_number} already exists. No action taken."); conn.execute("COMMIT;"); return True;
...
def detect_reorg():
    ...
    try:
        ...
        MAX_BACKTRACK = 5  # only check this many blocks back
        while block_number > 0 and latest_block - block_number < MAX_BACKTRACK:
            conn = create_database_connection('latest_cache', {'db_name': 'latestCache'})
            ...
            try:
                ...
                response = requests.get(api_url, verify=API_VERIFY, timeout=2); response.raise_for_status()
                ...
                if local_hash != remote_hash:
                    ...
                    logger.error(f"Reorg check API error at block {block_number}: {e}")
                    return None
            except Exception:
                return None
            block_number -= 1
...
def processBlock(blockindex=None, blockhash=None, blockinfo=None, keywords=None, force_store=False):
    ...
    while True:
        ...
        try:
            ...
            block_already_exists = None

        if block_already_exists:
            logger.info(f"Block {blockindex} already exists but will continue processing its transactions.")

        # Detect reorg every 49 blocks
        if not args.rebuild and blockindex is not None and blockindex % 49 == 0:
            fork_point = detect_reorg()
            if fork_point is not None and fork_point < blockindex:
...
def trigger_internal_contract_onvalue(tokenAmount_sum, contractStructure, transaction_data, blockinfo, parsed_data, connection, contract_name, contract_address, transaction_subType):
    ...
    parsed_data['onChain'] = False  # ✅ Explicitly mark trigger as off-chain
    ...
    try:
        ...
        if tokenSum > 0:
            ...
            for winner in contractWinners:
                ...
                # Mutate parsed_data before storing
                parsed_data_copy = dict(parsed_data)
                parsed_data_copy["onChain"] = False
                parsed_data_copy["transactionTrigger"] = transaction_data["txid"]
                connection.execute("""
                    INSERT INTO contractTransactionHistory
                    (transactionType, transactionSubType, sourceFloAddress, destFloAddress, transferAmount, blockNumber, blockHash, time, transactionHash, blockchainReference, jsonData, parsedFloData)
