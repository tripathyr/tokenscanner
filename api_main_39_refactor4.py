...
async def unified_token_list():
    try:
        version = "v1" if "/v1.0/" in request.path else "v2"
...
async def unified_token_info(token=None):
    try:
        ...
        resp = {"result": "error", "description": INTERNAL_ERROR} if version == "v1" else {"description": INTERNAL_ERROR}
        return jsonify(resp), 500
...
async def unified_token_transactions(token=None):
    try:
        ...
        if version == "v2":
            ...
            if limit and not check_integer(limit):
        ...
        resp = {"result": "error", "description": "Database error occurred"} if version == "v1" else {"description": "Database error occurred"}
        return jsonify(resp), 500
    except Exception as e:
...
async def unified_smart_contract_list():
    try:
        ...
        resp = {"result": "error", "description": INTERNAL_ERROR} if version == "v1" else {"description": "Unexpected error occurred"}
        return jsonify(resp), 500
...
async def unified_get_contract_participants():
    try:
                ...
                if 'exitconditions' in contractStructure:
                    ...
                    for row in rows:
                        ...
                        if version == "v2" and contractStatus == 'closed':
                            ...
                            data['winningAmount'] = win['winningAmount'] if win else 0
                        if version == "v2":
        ...
        resp = {"result": "error", "description": "Internal Server Error"} if version == "v1" else {"description": "Unexpected error occurred"}
        return jsonify(resp), 500
...
async def unified_get_participant_details():
    try:
        ...
        for contract in participant_contracts:
            ...
            details = {'contractName': contract_name, 'contractAddress': contract_address}

            contractStructure = await fetchContractStructure(contract_name, contract_address)

            async with get_mysql_conn_ctx(contract_db) as conn:
                async with conn.cursor(DictCursor) as cursor:
                    if contractStructure.get('contractType') == 'tokenswap':
                        await cursor.execute("""
                            SELECT participantAddress, participationAmount, receivedAmount,
                                   transactionHash, blockNumber, blockHash 
                            FROM contractparticipants 
                        ...
                        elif 'exitconditions' in contractStructure:
                            await cursor.execute("""
        ...
        resp = {"result": "error", "description": "Unexpected error occurred"} if version == "v1" else {"description": "Unexpected error occurred"}
        return jsonify(resp), 500
    ...
    try:
        ...
        async with get_mysql_conn_ctx(db_name) as conn:
            async with conn.cursor(DictCursor) as cursor:
                ...
                # Fetch current total deposit balance
                await cursor.execute("""
        ...
        response = {
            'currentDepositBalance': total_balance['totalDepositBalance'] if total_balance else 0,
...
async def unified_block_details(blockdetail):
    try:
        ...
        if blockJson:
            ...
            response = {"result": "ok", "blockDetails": blockDetails} if version == "v1" else {"blockDetails": blockDetails}
            return jsonify(response), 200
...
async def unified_block_transactions(block_keyword):
    try:
        ...
        response = (
            {"result": "ok", "transactions": txs_dict, "blockKeyword": block_keyword}
            if version == "v1"
            else {"transactions": txs_data, "blockKeyword": block_keyword}
        )
...
async def unified_categorise_string(urlstring):
    try:
        ...
        resp = dict(result="error", description=INTERNAL_ERROR) if version == "v1" else dict(description="Internal Error")
        return jsonify(resp), 500
...
async def unified_token_smart_contract_list():
    try:
        ...
        async with get_mysql_conn_ctx("information_schema", no_standardize=True) as conn_info:
            async with conn_info.cursor(DictCursor if version == "v1" else None) as cursor:
                await cursor.execute(f"""
                    SELECT SCHEMA_NAME FROM SCHEMATA
                    WHERE SCHEMA_NAME LIKE '{db_prefix}%' AND SCHEMA_NAME LIKE '%{db_suffix}'
                """)
                all_databases = await cursor.fetchall()
                all_dbs = [row["SCHEMA_NAME"] if version == "v1" else row[0] for row in all_databases]
        ...
        async with get_mysql_conn_ctx("system") as conn_system:
            async with conn_system.cursor(DictCursor if version == "v1" else None) as cursor:
                if version == "v2":
                    contractName = request.args.get("contractName")
                    contractAddress = request.args.get("contractAddress")
