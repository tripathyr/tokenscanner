...
def initialize_neturl(blockbook_neturl_list):
    """
    Initialize a working Blockbook URL, detect its type, and return metadata.

    Returns:
        tuple: (neturl, blockbook_type, initial_blockbook_height, initial_blockbook_time)
    """
    tried_servers = []
    ...
    while len(tried_servers) < len(blockbook_neturl_list):
        ...
        try:
            ...
            logger.info(f"Testing BLOCKBOOK_NETURL: {test_url}")
            response = requests.get(test_url, verify=True, timeout=100)
            response.raise_for_status()
            response_data = response.json()

            if "blockbook" in response_data and "backend" in response_data:
                blockbook_type = "blockbook_legacy"
                initial_blockbook_height = int(response_data["blockbook"]["bestHeight"])
                initial_blockbook_time = response_data["blockbook"].get("lastBlockTime")

            elif "blockheight" in response_data:
                blockbook_type = "address_indexer"
                initial_blockbook_height = int(response_data["blockheight"])
                initial_blockbook_time = response_data.get("latest_time")

            else:
                raise Exception(f"Invalid /latest-block response: {response_data}")

            logger.info(f"Selected '{blockbook_type}' server: {neturl}")
            return neturl, blockbook_type, initial_blockbook_height, initial_blockbook_time

        except Exception as e:
            logger.warning(f"Failed to validate {neturl}: {e}")
...
try:
    neturl, blockbook_type, initial_blockbook_height, initial_blockbook_time = initialize_neturl(serverlist)
    logger.info(f"Initialized Blockbook server: {neturl}")
    logger.info(f"Blockbook type: {blockbook_type}")
    logger.info(f"Initial block height: {initial_blockbook_height}")
    logger.info(f"Initial block time: {initial_blockbook_time}")
except Exception as e:
    logger.error(f"Failed to initialize Blockbook server: {e}")
    sys.exit(1)
...
def sanitize_iso_timestamp(ts):
    """
    Trims fractional seconds in an ISO timestamp to max 6 digits for compatibility.
    """
    if ts.endswith("Z"):
        ts = ts.replace("Z", "+00:00")

    # Match pattern with fractional seconds
    match = re.match(r"^(.*T\d{2}:\d{2}:\d{2})(\.\d+)?(\+.*)?$", ts)
    if not match:
    ...
    if fractional:
        fractional = fractional[:7]  # dot + 6 digits
    return f"{base}{fractional}{tz}"


def fetch_current_block_height():
    if initial_blockbook_time:
        try:
            clean_time_str = sanitize_iso_timestamp(initial_blockbook_time)
            block_time = datetime.fromisoformat(clean_time_str)
            
            # Compare using timezone-aware datetime
            if datetime.now(timezone.utc) < block_time + timedelta(minutes=2):
                logger.info("Using cached block height from recent /latest-block response.")
                return initial_blockbook_height
        except Exception as e:
            logger.warning(f"Failed to parse initial_blockbook_time: {e}")



    # Otherwise, fetch fresh height
    current_index = -1
    while current_index == -1:
        try:
            ...
            if current_index == -1:
                logger.warning("Block height not found in response. Retrying...")
