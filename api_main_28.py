...
async def clean_idle_connections_with_async_pool(idle_time_threshold=3):
    ...
    Args:
        idle_time_threshold (int): Time in seconds beyond which idle connections will be terminated.
    ...
    try:
        ...
        async with async_connection_pool.acquire() as conn:
            async with conn.cursor() as cursor:
                # Fetch all 'Sleep' connections exceeding the idle time threshold
                query = f"""
                SELECT id, time 
                ...
                if not idle_connections:
                    print("No idle connections found exceeding the threshold.")
                    return {"status": "success", "terminated_connections": 0}

                # Terminate each idle connection
                for connection_id, time in idle_connections:
                    print(f"Terminating connection ID {connection_id} (idle for {time} seconds)")
                    await cursor.execute(f"KILL {connection_id};")

                #print(f"Terminated {len(idle_connections)} idle connections.")
                return {"status": "success", "terminated_connections": len(idle_connections)}

    except Exception as e:
        print(f"Error during async idle connection cleanup: {e}")
