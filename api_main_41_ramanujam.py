...
fallback_path = '/home/production/deployed/newscanner/config.ini'  # Fallback directory
...
internalTransactionTypes = [ 'tokenswapDepositSettlement', 'tokenswapParticipationSettlement', 'smartContractDepositReturn','tokenswapRefund','smartContractPays']
...
async def fetch_dynamic_swap_price(contractStructure, blockinfo):
    ...
    try:
        ...
        url = f'{apiUrl}/api/address/{oracle_address}'
...
async def transaction_post_processing(transactionJsonData, contractStructure=None):
    rowarray_list = []
    ...
    for i, row in enumerate(transactionJsonData):
        try:
            ...
            token_id = (
                row.get('token') or
                parsedFloData.get('tokenIdentification') or
                parsedFloData.get('token') or
                (contractStructure.get('tokenIdentification') if contractStructure else None) or
                'unknown'
            )

            contract_name = (
                parsedFloData.get('contractName') or
                (contractStructure.get('contractName') if contractStructure else None)
            )
            contract_address = (
                parsedFloData.get('contractAddress') or
                (contractStructure.get('contractAddress') if contractStructure else None) or
                row.get('destFloAddress')
            )
            ...
            if is_internal:
                transactions_object = {
                    ...
                    'tokenIdentification': token_id,
                    'contractName': contract_name,
                    'contractAddress': contract_address,
                    'transactionTrigger': parsedFloData.get('transactionTrigger') or transactionDetails.get('txid'),
                    'time': transactionDetails.get('time'),
                    'type': row['transactionType'],
                    'onChain': parsedFloData.get('onChain', False)
                }

                if row['transactionType'] == 'smartContractIncorporation':
                    transactions_object.update({
                        'contractConditions': parsedFloData.get('contractConditions'),
                        'floData': parsedFloData.get('floData'),
                        'blockhash': transactionDetails.get('blockhash'),
                        'blockheight': transactionDetails.get('blockheight'),
                        'txid': transactionDetails.get('txid'),
                        'confirmations': latest_blockheight - transactionDetails.get('blockheight', latest_blockheight),
                        'tokenAmount': 0.0,
                        'tokenIdentification': token_id
                    })
                ...
                transactions_object.update({
                    ...
                    'contractAddress': contract_address,
                    'type': row['transactionType'],
                    'onChain': parsedFloData.get('onChain', True),
                    'transactionTrigger': parsedFloData.get('transactionTrigger') or transactionDetails.get('txid'),
                    'time': transactionDetails.get('time'),
                })
            ...
            # 🧠 Inject additional metadata from contractStructure
            if contractStructure:
                # Add continuous-event specific fields
                if contractStructure.get('contractType') == 'continuous-event':
                    for field in ['contractType', 'contractSubtype', 'price', 'priceType',
                                  'accepting_token', 'selling_token', 'oracle_address']:
                        if field in contractStructure:
                            transactions_object[field] = contractStructure[field]

                # Add availableChoices if subtype is external-trigger
                if contractStructure.get('subtype') == 'external-trigger':
                    exit_conditions = contractStructure.get('exitconditions') or []
                    transactions_object['availableChoices'] = exit_conditions
...
async def fetch_contract_transactions(contractName, contractAddress, _from=0, to=100, USE_ASYNC=True):
    """
    Fetches transactions related to a smart contract directly from contractTransactionHistory table,
    including creation, participation, deposit, settlement, refund, etc.
    """
    try:
        sc_db_name = standardize_db_name(f"{contractName}_{contractAddress}")

        # Fetch contract structure for any contextual info (e.g. contractType, tokens)
        ...
        # Query all transaction rows related to the contract
        tx_query = """
            SELECT jsonData, parsedFloData, time, transactionType, sourceFloAddress, destFloAddress,
                   transferAmount, transferToken AS token, transactionSubType
            FROM contractTransactionHistory
            ORDER BY id
            LIMIT %s OFFSET %s;
        """
        ...
        async with get_mysql_conn_ctx(sc_db_name) as conn_sc:
            async with conn_sc.cursor(DictCursor) as cursor_sc:
                await cursor_sc.execute(tx_query, (to - _from, _from))
                transactions = await cursor_sc.fetchall()

        transactionJsonData.extend(transactions)

        return await transaction_post_processing(transactionJsonData,contractStructure)
...
async def unified_smart_contract_transactions():
    ...
    try:
        ...
        if version == "v2":
            _from = int(request.args.get('_from', 0))
...
async def unified_transaction_details(transactionHash):
    ...
    try:
        ...
        on_chain = (
            parseResult.get("onChain")
            if "onChain" in parseResult
            else transactionJson.get("onChain", True)
        )
...
async def unified_categorise_string(urlstring):
    ...
    try:
        ...
        # ✅ Check if it's a known transaction hash in latestCache
        try:
            async with get_mysql_conn_ctx(standardize_db_name("latestCache")) as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
                        SELECT 1 FROM latestTransactions WHERE transactionHash = %s LIMIT 1
                    """, (urlstring,))
                    if await cursor.fetchone():
                        return jsonify(type="transaction"), 200
        except Exception as db_err:
            print("⚠️ DB lookup failed in latestTransactions:", db_err)

        # ✅ Check if it's a known block hash in latestCache
        try:
            async with get_mysql_conn_ctx(standardize_db_name("latestCache")) as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
                        SELECT 1 FROM latestBlocks WHERE blockHash = %s LIMIT 1
                    """, (urlstring,))
                    if await cursor.fetchone():
                        return jsonify(type="block"), 200
        except Exception as db_err:
            print("⚠️ DB lookup failed in latestBlocks:", db_err)

        # ✅ Fallback to external indexer endpoints
        async with aiohttp.ClientSession() as session:
            ...
            # ✅ Try self-call to own /api/v2/transactionDetails
            from quart import url_for
            self_url = url_for("transaction_details", txid=urlstring, _external=True)
            async with session.get(self_url) as resp:
                if resp.status == 200:
                    try:
                        data = await resp.json()
                        if data.get("txid") == urlstring:
                            return jsonify(type="transaction"), 200
                    except Exception:
                        pass  # ignore parsing failure

        # ✅ v2 logic: check databaseTypeMapping
        if version == "v2":
            async with get_mysql_conn_ctx(standardize_db_name("system")) as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
        ...
        # ✅ v1 logic fallback: look at schema names
        prefix = f"{mysql_config.database_prefix}_"
        suffix = "_db"
