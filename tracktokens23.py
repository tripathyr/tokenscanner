...
def initialize_neturl(blockbook_neturl_list):
    """
    Initialize `neturl` by selecting a random server and testing its availability.
    Perform failover if the selected server fails.
    ...
    while len(tried_servers) < len(blockbook_neturl_list):
        ...
        try:
            # Perform a simple test, e.g., a ping or a basic API call
            print(f"Testing BLOCKBOOK_NETURL: {neturl}")
            response = requests.get(f"{neturl}api", verify=True, timeout=5)
            if response.status_code == 200:
                print(f"Selected BLOCKBOOK_NETURL: {neturl}")
                return neturl
            else:
                raise Exception(f"Server {neturl} returned status code {response.status_code}")
        except Exception as e:
