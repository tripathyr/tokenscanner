...
def process_continuous_event_transfer(
    ...
    if contract_subtype == 'tokenswap':
        ...
        if available_deposit_sum >= swapAmount:
            ...
            # Your original logic for performing token transfers follows here unchanged...
            # For brevity, I left out the large token settlement logic in this snippet.
            # Keep your original logic as-is after this block.
            return 1
        else:
            rejectComment = (
...
def process_smart_contract_deposit(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    if check_database_existence(
        ...
        logger.info(f"type(expiry_time_obj.timestamp): {type(getattr(expiry_time_obj, 'timestamp'))}")
        expiry_time_ts = getattr(expiry_time_obj, "timestamp")()
