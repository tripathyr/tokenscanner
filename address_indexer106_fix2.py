...
async def initialize_database():
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            await cursor.execute(create_global_settings_table_query)
            ...
            if index_exists[0] == 0:
                await cursor.execute("CREATE INDEX idx_txid ON address_to_tx (txid);")
                    ...
                    # Create index on utxos.block_height if missing
            await cursor.execute("""
                SELECT COUNT(1)
                FROM information_schema.STATISTICS
            ...
            if index_exists[0] == 0:
                ...
                logger.info("Index idx_utxos_blockheight created.")
...
async def cleanup_utxos_via_rpc(batch_size=1):
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            # --- Find last cleaned block from global settings ---
            last_cleaned_block_str = await get_global_setting("utxo_cleanup_last_block", default="-1")
            last_cleaned_block = int(last_cleaned_block_str)
            logger.info(f"Last cleaned block height: {last_cleaned_block}")

            # Fetch latest block height from the chain
            chain_info = await rpc_request("getblockchaininfo")
            if "result" not in chain_info:
                raise Exception(f"RPC error: {chain_info.get('error')}")
            latest_block_height = chain_info["result"]["blocks"]

            if latest_block_height <= last_cleaned_block:
                logger.info(f"No new blocks to clean. Blockchain height is {latest_block_height}.")
                return

            # Only select UTXOs from blocks above last cleaned
            logger.info(f"Fetching UTXOs from blocks > {last_cleaned_block} ...")
            await cursor.execute("""
            ...
            current_cleaned_block = last_cleaned_block
            for i, (txid, vout, block_height) in enumerate(utxos):
                try:
                ...
                current_cleaned_block = max(current_cleaned_block, block_height)
            ...
            # Save progress back to global_settings
            if current_cleaned_block > last_cleaned_block:
                await set_global_setting("utxo_cleanup_last_block", str(current_cleaned_block))
                logger.info(f"Updated cleanup progress to block {current_cleaned_block}.")
