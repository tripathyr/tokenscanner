...
def extract_contract_conditions(text, contract_type, marker=None, blocktime=None):
    ...
    # ✅ PATCH: ensure rulestext is a string
    if not isinstance(rulestext, str):
        logger.info(f"extract_contract_conditions → Non-string rulestext: {rulestext}")
        return False
    ...
    for i in range(len(numberList)):
        ...
        if i+1 < len(numberList):
            end_marker = f"({i+2})"
            rule = rulestext.split(start_marker, 1)[1].split(end_marker, 1)[0]
        else:
            rule = rulestext.split(start_marker, 1)[1]
    ...
    if contract_type == 'one-time-event':
        ...
        for rule in rulelist:
            if rule == '':
                continue
            elif rule[:14].lower() == 'contractamount':
            ...
            elif rule[:11].lower() == 'userchoices':
            ...
            elif rule[:25].lower() == 'minimumsubscriptionamount':
            ...
            elif rule[:25].lower() == 'maximumsubscriptionamount':
            ...
            elif rule[:12].lower() == 'payeeaddress':
        ...
        for rule in rulelist:
            if rule == '':
                ...
                if price and (price[0] == "'" or price[0] == '"'):
                    price = price[1:]
                if price and (price[-1] == "'" or price[-1] == '"'):
                    price = price[:-1]
...
def extract_tokenswap_contract_conditions(processed_text, contract_type, contract_token):
    rulestext = re.split('contract-conditions:\s*', processed_text, flags=re.IGNORECASE)[-1]

    # ✅ PATCH: Ensure rulestext is a non-empty string
    ...
    # ✅ PATCH: Optionally remove leading colon if present
    ...
    for i in range(len(numberList)):
        try:
            if i + 1 < len(numberList):
                rule = rulestext.split(f"({i+1})", 1)[1].split(f"({i+2})", 1)[0]
            else:
                rule = rulestext.split(f"({i+1})", 1)[1]
        except IndexError:
    ...
    if contract_type == 'continuous-event':
        ...
        for rule in rulelist:
            if rule == '':
                ...
                pattern = re.compile(r'(^subtype\s*=\s*)(.*)')
                ...
                match = pattern.search(rule)
                if match:
                    accepting_token = match.group(0).strip()
                    extractedRules['accepting_token'] = accepting_token
                else:
                ...
                match = pattern.search(rule)
                if match:
                    selling_token = match.group(0).strip()
                    extractedRules['selling_token'] = selling_token
                else:
...
def extract_deposit_conditions(text, blocktime=None):
    ...
    # ✅ PATCH: Ensure rulestext is valid and not empty
    if not isinstance(rulestext, str) or rulestext.strip() == "":
    ...
    # ✅ Optional PATCH: Remove leading colon if present
    if rulestext.startswith(':'):
        rulestext = rulestext[1:].strip()
    ...
    # Check condition numbering is sequential
    for idx in range(len(numberList) - 1):
        if numberList[idx] + 1 != numberList[idx + 1]:
    ...
    for rule in rulelist:
        ...
        if rule.lower().startswith('expirytime'):
            try:
                ...
                # Remove anything in parentheses at the end (e.g. (GMT+0530))
                expirytime_clean = expirytime.split(' (')[0].strip()
...
def extract_trigger_condition(text):
    # Non-greedy match for double or single quotes
