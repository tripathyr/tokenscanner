...
def extract_contract_conditions(text, contract_type, marker=None, blocktime=None):
    ...
    if contract_type:
        ...
        logger.info(f"Sanitized contract_type: {contract_type}")

    try:
        rulestext = extract_substring_between(text, 'contract-conditions', 'end-contract-conditions')
    except Exception as e:
        logger.warning(f"Could not extract contract-conditions block: {e}")
        return False

    if not rulestext or not isinstance(rulestext, str):
        logger.warning("Empty or invalid rulestext.")
        return False

    rulestext = rulestext.strip()
    if rulestext == "":
        logger.warning("rulestext is blank after trimming.")
        return False

    if rulestext.startswith(":"):
        rulestext = rulestext[1:].strip()

    # Split into numbered rules
    numberList = re.findall(r'\(\d+\)', rulestext)
    logger.info(f"Found numbered conditions: {numberList}")

    try:
        numberList = [int(item[1:-1]) for item in numberList]
    except:
        logger.warning("Error converting condition numbers to integers.")
        return False

    if len(numberList) >= 2:
        for idx in range(len(numberList) - 1):
            if numberList[idx] + 1 != numberList[idx + 1]:
                logger.warning("Contract condition numbers are not sequential.")
                return False

    # Extract each rule text
    rulelist = []
    for i in range(len(numberList)):
        ...
        if i+1 < len(numberList):
            ...
            if len(parts) < 2:
                logger.warning(f"Could not extract after marker {start_marker}")
                return False
            after_start = parts[1]
    ...
    if not rulelist:
        logger.warning("No rules found after splitting.")
        return False

    # Helper to extract values
    def extract_value(pattern, rule):
    ...
    if contract_type == 'one-time-event':
        ...
        for rule in rulelist:
            ...
            if lower_rule.startswith('expirytime'):
                try:
                    ...
                    if blocktime is None:
                        logger.warning("blocktime is None for expirytime comparison.")
                        return False
                    blocktime_object = arrow.get(blocktime).to(dt_tz.UTC)
                    if expirytime_object < blocktime_object:
                        logger.warning("expiryTime is earlier than blocktime.")
                        return False
                    extractedRules['expiryTime'] = expirytime
                    extractedRules['unix_expiryTime'] = int(expirytime_object.timestamp)
                    logger.info(f"Parsed expiryTime: {expirytime}")
                except Exception as e:
                    logger.warning(f"Error parsing expiry time: {e}")
                    return False

        # Additional one-time-event attributes
        for rule in rulelist:
            if rule == '':
                continue
            lower_rule = rule.lower()

            if lower_rule.startswith('contractamount'):
                pattern = re.compile(r'(^contractamount\s*=\s*)(.*)', re.IGNORECASE)
                val = extract_value(pattern, rule)
                if val:
                    if marker:
                        val = val.split(marker)[0]
                    try:
                        extractedRules['contractAmount'] = float(val)
                        logger.info(f"Parsed contractAmount: {val}")
                    except:
                        logger.warning(f"Non-numeric contractAmount ignored: {val}")

            elif lower_rule.startswith('userchoices'):
                pattern = re.compile(r'(^userchoices\s*=\s*)(.*)', re.IGNORECASE)
                val = extract_value(pattern, rule)
                if val:
                    choices = [c.strip() for c in val.split('|')]
                    extractedRules['userchoices'] = {i: choice for i, choice in enumerate(choices)}
                    logger.info(f"Parsed userchoices: {extractedRules['userchoices']}")

            elif lower_rule.startswith('minimumsubscriptionamount'):
                ...
                if val:
                    if marker:
                        val = val.split(marker)[0]
                    try:
                        extractedRules['minimumsubscriptionamount'] = float(val)
                        logger.info(f"Parsed minimumsubscriptionamount: {val}")
                    except:
                        logger.warning(f"Non-numeric minimumsubscriptionamount ignored: {val}")

            elif lower_rule.startswith('maximumsubscriptionamount'):
                ...
                if val:
                    if marker:
                        val = val.split(marker)[0]
                    try:
                        extractedRules['maximumsubscriptionamount'] = float(val)
                        logger.info(f"Parsed maximumsubscriptionamount: {val}")
                    except:
                        logger.warning(f"Non-numeric maximumsubscriptionamount ignored: {val}")

            elif lower_rule.startswith('payeeaddress'):
                pattern = re.compile(r'(^payeeaddress\s*=\s*)(.*)', re.IGNORECASE)
                val = extract_value(pattern, rule)
                if val:
                    if marker:
                        val = val.split(marker)[0]
                    payeeAddress = val.strip('"').strip("'")
                    extractedRules['payeeAddress'] = payeeAddress
                    logger.info(f"Parsed payeeAddress: {payeeAddress}")

        if 'expiryTime' in extractedRules:
            return extractedRules
        else:
            logger.warning("No expiryTime found for one-time-event.")
            return False
    ...
    # --- Continuous-Event ---
        ...
        for rule in rulelist:
            ...
            if lower_rule.startswith('subtype'):
                pattern = re.compile(r'(^subtype\s*=\s*)(.*)', re.IGNORECASE)
                match = pattern.search(rule)
                if match:
                    subtype = match.group(2).strip().rstrip("*")
                    extractedRules['subtype'] = subtype
                    logger.info(f"Parsed subtype: {subtype}")

            elif re.search(r'accepting[_Tt]oken\s*=', rule, re.IGNORECASE):
                pattern = re.compile(r'accepting[_Tt]oken\s*=\s*(.*)', re.IGNORECASE)
                match = pattern.search(rule)
                if match:
                    token = match.group(1).strip().rstrip("#")
                    if token:
                        extractedRules['accepting_token'] = token
                        logger.info(f"Parsed accepting_token: {token}")

            elif re.search(r'selling[_Tt]oken\s*=', rule, re.IGNORECASE):
                pattern = re.compile(r'selling[_Tt]oken\s*=\s*(.*)', re.IGNORECASE)
                match = pattern.search(rule)
                if match:
                    token = match.group(1).strip().rstrip("#")
                    if token:
                        extractedRules['selling_token'] = token
                        logger.info(f"Parsed selling_token: {token}")

            elif lower_rule.startswith('pricetype'):
                pattern = re.compile(r'(^pricetype\s*=\s*)(.*)', re.IGNORECASE)
                match = pattern.search(rule)
                if match:
                    priceType = match.group(2).strip().rstrip("*").replace("'", "").replace('"', '')
                    extractedRules['priceType'] = priceType
                    logger.info(f"Parsed priceType: {priceType}")

            elif lower_rule.startswith('price'):
                ...
                if match:
                    ...
                    try:
                        ...
                        logger.warning(f"Price is not numeric: {price}")
                        return False

            elif lower_rule.startswith('direction'):
            ...
            elif lower_rule.startswith('oracle_address'):
                pattern = re.compile(r'(^oracle_address\s*=\s*)(.*)', re.IGNORECASE)
                match = pattern.search(rule)
                if match:
                    oracle_address = match.group(2).strip().strip('"').strip("'")
                    if oracle_address:
                        extractedRules['oracle_address'] = oracle_address
                        logger.info(f"Parsed oracle_address: {oracle_address}")

        # Check required fields
        required_keys = ['subtype', 'accepting_token', 'selling_token', 'priceType', 'price']
        for k in required_keys:
            if k not in extractedRules:
                logger.warning(f"Missing required field in contract conditions: {k}")
                return False

        return extractedRules if extractedRules else False

    else:
        logger.warning(f"Unknown contract_type: {contract_type}")
        return False
...
def extract_tokenswap_contract_conditions(processed_text, contract_type, contract_token):
    ...
    # Extract contract-conditions block
    rulestext_split = re.split(r'contract-conditions\s*:\s*', processed_text, flags=re.IGNORECASE)
    if len(rulestext_split) < 2:
        logger.info("extract_tokenswap_contract_conditions → no contract-conditions block found.")
        return False
    ...
    try:
        ...
        logger.info("Error parsing condition numbers.")
        return False

    numberList = sorted(numberList)
    for idx in range(len(numberList) - 1):
        if numberList[idx] + 1 != numberList[idx + 1]:
            logger.info('Contract condition numbers are not sequential.')
            return False

    # Split into individual rule texts
    rulelist = []
    for i in range(len(numberList)):
        start_marker = f"({i+1})"
        if i + 1 < len(numberList):
            ...
            parts = rulestext.split(start_marker, 1)
            if len(parts) < 2:
                logger.info(f"Error extracting rule between {start_marker} and {end_marker}")
                return False
            after_start = parts[1]
            parts = after_start.split(end_marker, 1)
            rule = parts[0] if parts else ""
            ...
            parts = rulestext.split(start_marker, 1)
            if len(parts) < 2:
                logger.info(f"Error extracting rule after {start_marker}")
                return False
            rule = parts[1]
        rulelist.append(rule.strip())
    ...
    for rule in rulelist:
        ...
        # Universal pattern: key = value
        m = re.match(r"^\s*(\w[\w_]*)\s*=\s*(.*?)\s*$", rule, re.IGNORECASE)
        if not m:
            logger.info(f"Could not parse rule: {rule}")
            continue

        key = m.group(1).lower().strip()
        val = m.group(2).strip()

        # Remove trailing # or * from values
        val = val.rstrip("#*")
        ...
        if key == "price":
            try:
                extractedRules[key] = float(val)
                logger.info(f"Parsed {key}: {val}")
            except ValueError:
                logger.warning(f"Price value not numeric: {val}")
                return False
        else:
            # PATCH HERE for oracle_address trailing garbage
            if key == "oracle_address":
                if val.lower().endswith("end-contract-conditions"):
                    val = val[: -len("end-contract-conditions")].strip()
            extractedRules[key] = val
            logger.info(f"Parsed {key}: {val}")

    # Check mandatory fields for tokenswap
    if contract_type == "continuous-event":
        mandatory_fields = ["subtype", "accepting_token", "selling_token", "pricetype", "price"]
...
def process_smart_contract_creation_ce_tokenswap(
    ...
    logger.info("=== Entered process_smart_contract_creation_ce_tokenswap ===")
    ...
    logger.info(f"apply_rule1 → operation: {operation}")
    if operation != 'category1':
        logger.warning("→ Operation check failed: not category1")
        return outputreturn('noise')
    ...
    if not contract_type or not check_existence_of_keyword(['continuous-event'], [contract_type]):
        logger.warning(f"→ Contract type mismatch: got {contract_type}")
        return outputreturn('noise')

    contract_name = extract_special_character_word(first_classification['wordlist'], '@')
    logger.info(f"Extracted contract_name (raw): {contract_name}")
    if contract_name:
        contract_name = contract_name.rstrip("@").strip()
        logger.info(f"Sanitized contract_name: {contract_name}")

    if not contract_name or not check_regex("^[A-Za-z][A-Za-z0-9_-]*[A-Za-z0-9]$", contract_name):
        logger.warning(f"→ Contract name regex check failed: {contract_name}")
        return outputreturn('noise')

    contract_token = extract_special_character_word(first_classification['wordlist'], '#')
    logger.info(f"Extracted contract_token (raw): {contract_token}")
    if contract_token:
        contract_token = contract_token.rstrip("#").strip()
        logger.info(f"Sanitized contract_token: {contract_token}")

    if not contract_token or not check_regex("^[A-Za-z][A-Za-z0-9_-]*[A-Za-z0-9]$", contract_token):
        logger.warning(f"→ Contract token regex check failed: {contract_token}")
        return outputreturn('noise')

    contract_address = extract_special_character_word(first_classification['wordlist'], '$')
    logger.info(f"Extracted contract_address (raw): {contract_address}")
    if contract_address is False:
        logger.warning("→ No contract address found in classification.")
        return outputreturn('noise')

    contract_address = contract_address.rstrip("$").strip()
    contract_address = find_original_case(contract_address, clean_text)
    logger.info(f"Final contract_address: {contract_address}")

    if not check_flo_address(contract_address, is_testnet):
        logger.warning(f"→ FLO address check failed: {contract_address}")
        return outputreturn('noise')

    logger.info("Calling extract_tokenswap_contract_conditions()...")
    contract_conditions = extract_tokenswap_contract_conditions(
        processed_text,
        contract_type,
        contract_token,
    )
    logger.info(f"Contract conditions extraction result: {contract_conditions}")
    if contract_conditions is False:
        logger.warning("→ Contract conditions extraction returned False.")
        return outputreturn('noise')

    try:
        logger.info(f"Checking contract_conditions subtype: {contract_conditions.get('subtype')}")
        if contract_conditions['subtype'] != 'tokenswap':
            logger.warning(f"→ Subtype mismatch: {contract_conditions['subtype']}")
            return outputreturn('noise')

        logger.info("Validating accepting_token...")
        if not check_regex("^[A-Za-z][A-Za-z0-9_-]*[A-Za-z0-9]$", contract_conditions['accepting_token']):
            logger.warning(f"→ Invalid accepting_token: {contract_conditions['accepting_token']}")
            return outputreturn('noise')

        logger.info("Validating selling_token...")
        if not check_regex("^[A-Za-z][A-Za-z0-9_-]*[A-Za-z0-9]$", contract_conditions['selling_token']):
            logger.warning(f"→ Invalid selling_token: {contract_conditions['selling_token']}")
            return outputreturn('noise')

        price_type = contract_conditions['pricetype'].strip("'\"").lower()
        logger.info(f"Detected price_type: {price_type}")

        if price_type in ['determined', 'predetermined']:
            logger.info("Checking price for determined/predetermined type...")
            try:
                price_value = float(contract_conditions['price'])
                logger.info(f"Price value: {price_value}")
                if price_value <= 0:
                    logger.warning("→ Price must be positive.")
                    return outputreturn('noise')
            ...
            logger.info("Checking price for dynamic price type...")
            try:
                price_value = float(contract_conditions['price'])
                logger.info(f"Price value: {price_value}")
                if price_value <= 0:
            ...
            logger.info(f"Extracted oracle_address: {oracle_addr}")
            if not oracle_addr:
                logger.warning("→ Oracle address is missing.")
                return outputreturn('noise')

            # ✅ Patch here to get original-case value
            oracle_addr_original = find_original_case_regex(oracle_addr, clean_text)
            logger.info(f"Original-case oracle address: {oracle_addr_original}")

            if not check_flo_address(oracle_addr_original, is_testnet):
                logger.warning(f"→ Oracle address is not valid FLO address: {oracle_addr_original}")
                return outputreturn('noise')

            if oracle_addr_original == contract_address:
                logger.warning("→ Oracle address is same as contract address.")
                return outputreturn('noise')

            # ✅ Save the original-case oracle address
            contract_conditions['oracle_address'] = oracle_addr_original

        elif price_type == 'statef':
            logger.info("Price type is statef. No oracle needed.")
            contract_conditions['oracle_address'] = False

        else:
            logger.warning(f"→ Unknown price_type: {price_type}")
            return outputreturn('noise')
    ...
    logger.info(f"Final parsed result:\n"
                f"  contract_token: {contract_token}\n"
                f"  contract_name: {contract_name}\n"
                f"  contract_address: {contract_address}\n"
                f"  subtype: {contract_conditions['subtype']}\n"
                f"  accepting_token: {contract_conditions['accepting_token']}\n"
                f"  selling_token: {contract_conditions['selling_token']}\n"
                f"  priceType: {contract_conditions['pricetype']}\n"
                f"  price: {contract_conditions['price']}\n"
                f"  oracle_address: {contract_conditions['oracle_address']}\n"
                f"  stateF_mapping: {stateF_mapping}"
    )
...
def parse_flodata(text, blockinfo, net):
    logger.info("=== parse_flodata START ===")
    logger.info(f"Original FLO data text:\n{text}\n")

    is_testnet = net == 'testnet'

    if text == '':
        logger.info("Input text is empty. Returning noise.")
        return outputreturn('noise')

    clean_text, processed_text = text_preprocessing(text)
    logger.info(f"Clean Text:\n{clean_text}")
    logger.info(f"Processed Text:\n{processed_text}")

    first_classification = firstclassification_rawstring(processed_text)
    logger.info(f"First Classification Result:\n{first_classification}")

    if not first_classification or 'categorization' not in first_classification:
        logger.info("first_classification is None or missing 'categorization'. Returning noise.")
        return outputreturn('noise')

    categorization = first_classification['categorization']
    logger.info(f"Detected categorization: {categorization}")

    if categorization != 'noise':
        stateF_mapping = isStateF(processed_text)
        logger.info(f"StateF mapping result: {stateF_mapping}")
        if stateF_mapping is not False and not isinstance(stateF_mapping, dict):
            logger.info("Invalid StateF mapping. Returning noise.")
            return outputreturn('noise')
    else:
        stateF_mapping = False
        logger.info("Categorization was noise, setting stateF_mapping = False.")

    # Decision routing
    if categorization == 'tokensystem-C':
        logger.info("→ Routing to process_tokensystem_C")
        ...
        logger.info("→ Routing to process_smart_contract_creation_C")
        ...
        logger.info("→ Routing to process_smart_contract_participation_deposit_C")
        ...
        logger.info("→ Routing to process_smart_contract_participation_ote_ce_C")
        ...
        logger.info("→ Routing to process_userchoice_trigger")
        ...
        logger.info("→ Routing to process_smart_contract_creation_ce_tokenswap")
    ...
    logger.info("Categorization did not match any known type. Returning noise.")
    return outputreturn('noise')
