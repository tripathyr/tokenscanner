...
async def before_serving():
    """Initialize connection pools before serving requests."""
    await initialize_connection_pools()
    print("Connection pools initialized before serving requests.")

@app.after_serving
async def after_serving():
    """Clean up connection pools after serving requests."""
    try:
...
async def after_serving():
    """
    Triggered after the Quart app stops serving requests.
...
async def shutdown_resources():
    """
    Cleans up resources like connection pools and scheduler.
    """
...
async def shutdown(loop, signal=None):
    ...
    for task in tasks:
        ...
        try:
            ...
            print(f"Task {task.get_name()} took too long to cancel.")
...
async def clean_idle_connections_with_async_pool(idle_time_threshold=3):
    ...
    """
    try:
        logger.info("Starting idle connection cleanup process.")

        if not async_connection_pool:
            raise RuntimeError("Async connection pool is not initialized.")
        logger.info("Async connection pool is initialized and ready.")
        ...
        async with async_connection_pool.acquire() as conn:
            ...
            async with conn.cursor() as cursor:
                logger.info("Cursor acquired, preparing to execute query.")

                # Fetch idle connections
                query = f"""
                SELECT id, time 
                FROM performance_schema.processlist 
                WHERE command = 'Sleep' AND time > {idle_time_threshold};
                """
                logger.info(f"Executing query to fetch idle connections:\n{query}")
                query_start = time.time()
                await cursor.execute(query)
                idle_connections = await cursor.fetchall()
                query_end = time.time()
                logger.info(f"Query executed in {query_end - query_start:.2f} seconds.")
                logger.info(f"Idle connections fetched: {idle_connections}")

                # If no idle connections are found
                ...
                logger.info(f"Attempting to terminate {len(idle_connections)} idle connections.")
                kill_start = time.time()
                tasks = [
                    cursor.execute(f"KILL {connection_id};")
                    for connection_id, _ in idle_connections
                ]
                logger.info(f"Generated {len(tasks)} KILL tasks, executing them concurrently.")
                await asyncio.gather(*tasks)
                kill_end = time.time()
                logger.info(f"KILL commands executed in {kill_end - kill_start:.2f} seconds.")
...
async def init_process():
    ...
    async_connection_pool = await initialize_db()
    scheduler.add_job(run_async_update_prices, trigger="interval", seconds=600)
    scheduler.add_job(clean_idle_connections_with_async_pool, trigger="interval", seconds=30)  # Run every 5 seconds
    scheduler.start()
...
async def start_api_server(config):
    ...
    hypercorn_config.shutdown_timeout = 60
