...
def extract_contract_conditions(text, contract_type, marker=None, blocktime=None):
    ...
    # ✅ Patch for safely splitting rules
    ...
    if contract_type == 'one-time-event':
        ...
        for rule in rulelist:
            if rule == '':
                ...
                expirytime = re.split(r'expirytime[\s]*=[\s]*', rule)[1].strip()
                try:
                    expirytime_split = expirytime.split(' ')
                    parse_string = '{}/{}/{} {}'.format(
                        expirytime_split[3],
                        months[expirytime_split[1]],
                        expirytime_split[2],
                        expirytime_split[4]
                    )
        ...
        for rule in rulelist:
            if rule == '':
                ...
                pattern = re.compile(r'(^contractamount\s*=\s*)(.*)')
                ...
                pattern = re.compile(r'(^userchoices\s*=\s*)(.*)')
                ...
                pattern = re.compile(r'(^minimumsubscriptionamount\s*=\s*)(.*)')
                searchResult = pattern.search(rule).group(2)
                if marker:
                ...
                pattern = re.compile(r'(^maximumsubscriptionamount\s*=\s*)(.*)')
                searchResult = pattern.search(rule).group(2)
                if marker:
                ...
                pattern = re.compile(r'(^payeeaddress\s*=\s*)(.*)')
                searchResult = pattern.search(rule).group(2)
                if marker:
