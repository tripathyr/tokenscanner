...
async def shutdown_resources():
    ...
    try:
        ...
        if background_async_pool:
            ...
            await background_async_pool.wait_closed()

        # Clean up the thread we started for background connection cleanup
        global background_cleanup_thread
        if background_cleanup_thread and background_cleanup_thread.is_alive():
            logger.info("Waiting briefly for background thread to exit...")
            # Just wait a moment and move on — don't block the event loop
