...
async def rpc_request(method, params=None):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(RPC_ADDRESS, json=payload, auth=auth, timeout=300) as response:
...
async def b_compute_vin_values(vin):
    SAFE_LIMIT = 1  # Only allow parallel processing if vin count is 20 or less

    async def fetch_and_process(vin_entry):
        if "txid" not in vin_entry:
            return vin_entry

        txid = vin_entry["txid"]
        vout_index = vin_entry.get("vout", -1)

        try:
            tx_data = await b_fetch_transaction(txid)
            vout_list = tx_data.get("result", {}).get("vout", [])

            if vout_index < len(vout_list):
                vout = vout_list[vout_index]
                vin_entry["value"] = vout.get("value", 0)
                vin_entry["valueSat"] = math.floor(vout.get("value", 0) * 1e8)
                spk = vout.get("scriptPubKey", {})
                vin_entry["addresses"] = spk.get("addresses", [])
                vin_entry["isAddress"] = bool(spk.get("addresses"))
            else:
                vin_entry.update({"value": 0, "valueSat": 0, "addresses": [], "isAddress": False})

        except Exception as e:
            logger.error(f"Error processing VIN {txid}:{vout_index} - {e}")
            vin_entry.update({"value": 0, "valueSat": 0, "addresses": [], "isAddress": False})

        return vin_entry

    if len(vin) > SAFE_LIMIT:
        logger.warning(f"VIN count = {len(vin)} exceeds safe limit. Using serial mode.")
        for i in range(len(vin)):
            vin[i] = await fetch_and_process(vin[i])
    else:
        vin = await asyncio.gather(*[fetch_and_process(entry) for entry in vin])
...
async def get_block_by_hash(blockhash):
    ...
    logger.info(f"📥 Received request for block {blockhash}")

    if not is_valid_hash(blockhash):
        return Response(json.dumps({"error": "Invalid block hash"}), status=400, mimetype="application/json")
    ...
    try:
        ...
        if isinstance(block_output, str):
            ...
            return Response(json.dumps({"error": block_output}), status=500, mimetype="application/json")
        ...
        if isinstance(block_output, dict) and "error" in block_output:
            ...
            return Response(json.dumps(block_output), status=500, mimetype="application/json")
        ...
        return Response(json.dumps({"error": f"Internal error: {str(e)}"}), status=500, mimetype="application/json")
...
# Debug tool to analyze a block's transactions and scripts
async def analyze_block_structure_by_number(block_height):
    """Debug tool: Analyze structure of a block (by height) and log vin/vout sizes."""
    logger.info(f"🔍 Analyzing block at height {block_height}")

    # Step 1: Convert block height to block hash
    blockhash_response = await rpc_request("getblockhash", [block_height])
    blockhash = blockhash_response.get("result")

    if not blockhash:
        logger.error(f"❌ Block height {block_height} could not be resolved to a hash")
        return

    logger.info(f"🔗 Resolved block height {block_height} to hash {blockhash}")

    # Step 2: Fetch full block from flod
    block_data = await b_fetch_block_data_flod(blockhash)
    if "result" not in block_data:
        logger.error(f"❌ Block {blockhash} not found or malformed")
        return

    txs = block_data["result"].get("tx", [])
    logger.info(f"📦 Block {blockhash} has {len(txs)} transactions")

    for index, tx in enumerate(txs):
        txid = tx if isinstance(tx, str) else tx.get("txid", f"<unknown-{index}>")
        logger.info(f"⛓ TX #{index + 1} - {txid}")
        ...
        try:
            ...
            logger.info(f"  🔸 VIN count: {len(vin)}, VOUT count: {len(vout)}")

            # Check for large scripts
            for vin_idx, vin_entry in enumerate(vin):
                asm = vin_entry.get("scriptSig", {}).get("asm", "")
                if len(asm) > 1000:
                    logger.warning(f"    ⚠️ VIN[{vin_idx}] scriptSig.asm length: {len(asm)}")

            for vout_idx, vout_entry in enumerate(vout):
                asm = vout_entry.get("scriptPubKey", {}).get("asm", "")
                if len(asm) > 1000:
                    logger.warning(f"    ⚠️ VOUT[{vout_idx}] scriptPubKey.asm length: {len(asm)}")

        except Exception as e:
            logger.error(f"❌ Failed to fetch or analyze TX {txid}: {e}", exc_info=True)
...
async def clean_idle_connections_with_async_pool(pool, idle_time_threshold=2):
        ...
        async def main():
            ...
            try:
                ...
                while not stop_event.is_set():
                    try:
                        await clean_idle_connections_with_async_pool(cleaner_connection_pool, idle_time_threshold=5)
...
def stop_cleaner_thread():
    ...
    if cleaner_thread.is_alive():
        cleaner_thread.join(timeout=5)  # Wait up to 5 seconds for the thread to exit
...
async def main():
    try:
        ...
        await analyze_block_structure_by_number(2113179)
...
async def shutdown(loop):
    ...
    global async_connection_pool, cleaner_connection_pool, scheduler  # Ensure scheduler is accessible
