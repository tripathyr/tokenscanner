...
if __name__ == "__main__":
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            logger.warning("Event loop already running. Creating task for main() instead of running it directly.")
            loop.create_task(main())  # Run main() as a background task if the loop is already running
        else:
            asyncio.run(main())  # Safe to use asyncio.run() if no loop is running
    except KeyboardInterrupt:
        print("Received Ctrl+C, shutting down.")
    except RuntimeError as e:
        logger.error(f"Unexpected RuntimeError in main execution: {e}")
