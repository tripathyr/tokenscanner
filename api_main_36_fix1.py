...
async def transaction_post_processing(transactionJsonData):
    ...
    for idx, row in enumerate(transactionJsonData):
        transactions_object = {}
        try:
            print(f"[DEBUG] Processing row {idx}: {row}")

            if len(row) < 2:
                logger.warning(f"Row {idx} has insufficient columns: {row}")
                continue
            ...
            if len(row) >= 9 and (
                row[3] in internalTransactionTypes or (row[3] == 'trigger' and row[8] != 'committee')
            ):
                internal_info = {
                    'senderAddress': row[4],
