...
async def get_dynamic_pool_size():
    ...
    MIN_POOL_SIZE = 5    # Start conservatively with 5 connections
    MAX_POOL_SIZE = 30  # Maximum allowed pool size
...
scheduler.add_job(adjust_pool_size, "interval", seconds=10)
scheduler.start()    
...
async def clean_idle_connections_with_async_pool(idle_time_threshold=2):
    """Cleans idle MySQL connections using the async connection pool."""
    global async_connection_pool

    if async_connection_pool is None:
        logger.error("Async connection pool is not initialized.")
    ...
    try:
        ...
        # Run terminations concurrently but avoid connection starvation
        ...
        tasks = [terminate_connection(conn_id, time) for conn_id, time in idle_connections]
...
def set_thread_high_priority():
    """Sets the cleanup thread to the highest possible priority on Linux."""
    try:
        pid = os.getpid()
        tid = threading.get_native_id()  # Get thread ID

        # ✅ Set the scheduler to real-time FIFO scheduling
        param = os.sched_param(99)  # 99 = highest real-time priority
        os.sched_setscheduler(0, os.SCHED_FIFO, param)

        # ✅ Set the lowest nice value (-20) for highest priority
        p = psutil.Process(pid)
        p.nice(-20)

        logger.info(f"Cleanup thread priority set to highest (SCHED_FIFO, nice=-20, thread={tid}).")
    except Exception as e:
        logger.error(f"Failed to set cleanup thread priority: {e}")


def run_cleaner_in_thread():
    """Runs the async cleanup function in a dedicated high-priority thread with a configurable interval."""
    global cleaner_thread
    
    def thread_target():
        """The function that runs inside the cleaner thread."""
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        
        async def main():
            try:
                set_thread_high_priority()  # Set highest priority for this thread
                while not stop_event.is_set():  # Keep running until stop signal is received
                    await clean_idle_connections_with_async_pool(idle_time_threshold=5)
                    await asyncio.sleep(CLEANUP_INTERVAL_SECONDS)  # Dynamically configurable interval
            except asyncio.CancelledError:
                logger.info("Cleanup thread received exit signal.")
            finally:
                await shutdown_pool()
...
def stop_cleaner_thread():
    """Gracefully stops the cleanup thread and closes resources."""
    global cleaner_thread
    ...
    if cleaner_thread.is_alive():
        ...
        if cleaner_thread.is_alive():
            logger.error("Cleanup thread did not exit within timeout.")
...
def signal_handler(sig, frame):
    ...
    stop_cleaner_thread()  # Stop the cleaner thread

    loop = asyncio.get_running_loop()
    if loop.is_running():
        loop.create_task(shutdown_pool())  # Use create_task() instead of asyncio.run()
    else:
...
async def main():
    try:
        ...
        # ✅ Step 5: Start Background ZMQ Subscriber Task
        zmq_task = asyncio.create_task(zmq_block_subscriber())

        # ✅ Step 6: Configure Hypercorn
        hypercorn_config = HypercornConfig()
        hypercorn_config.shutdown_timeout = 0  # 10 sec graceful shutdown
        hypercorn_config.bind = [f"{ADDRESS_INDEXER_HOST}:{ADDRESS_INDEXER_PORT}"]

        # ✅ Step 7: Setup Shutdown Signal Handling Properly
        loop = asyncio.get_running_loop()

        stop_event = asyncio.Event()  # Use an event to signal shutdown

        async def handle_shutdown(signal):
            print(f"Received shutdown signal: {signal}. Cleaning up...")
            await shutdown(loop)
            stop_event.set()  # Signal the event loop to exit

        for sig in (signal.SIGINT, signal.SIGTERM):
            loop.add_signal_handler(sig, lambda: asyncio.create_task(handle_shutdown(sig)))

        # ✅ Step 8: Run Hypercorn Server & Wait for Shutdown Event
        server_task = asyncio.create_task(serve(address_indexer_app, hypercorn_config))
        await stop_event.wait()  # Wait until the event is set (signal received)

        # ✅ Step 9: Cleanup on Shutdown
