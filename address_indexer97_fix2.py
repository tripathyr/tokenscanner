...
def set_thread_high_priority():
    ...
    try:
        ...
        # Also lower the 'nice' value for highest CPU priority
        p = psutil.Process(pid)
        p.nice(-20)  # -20 is the highest priority in Linux
...
def run_cleaner_in_thread(loop):
    """Runs the async cleanup function in a dedicated high-priority thread."""
    ...
    def thread_target():
        ...
        while not stop_event.is_set():
            try:
                # Schedule the async cleanup in the main loop
                future = asyncio.run_coroutine_threadsafe(
                    clean_idle_connections_with_async_pool(idle_time_threshold=5), loop
                )
                future.result(timeout=10)  # Ensure execution and catch errors
            except asyncio.TimeoutError:
                logger.warning("Cleaner thread timeout occurred.")
            except Exception as e:
                logger.error(f"Error running async cleanup: {e}")

            time.sleep(CLEANUP_INTERVAL_SECONDS)  # Configurable interval
    ...
    try:
        ...
        logger.error(f"Failed to start cleaner thread: {e}")


def stop_cleaner_thread():
    """Gracefully stops the cleanup thread."""
    global cleaner_thread
    ...
    cleaner_thread.join(timeout=5)  # Wait for the thread to terminate
...
if __name__ == "__main__":
    ...
    cleaner_thread = run_cleaner_in_thread(loop)  
