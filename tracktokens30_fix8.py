...
def fetchDynamicSwapPrice(contractStructure, blockinfo):
    ...
    # Build relative path without leading slash
    ...
    if contract_subtype == 'tokenswap':
        ...
        if contractStructure['pricetype'] in ['predetermined', 'determined']:
            ...
            if transaction_data['senderAddress'] == contractStructure['oracle_address']:
                logger.warning(
                    f"Transaction {transaction_data['txid']} rejected as the oracle address {contractStructure['oracle_address']} is attempting to participate. Please report this to the contract owner"
                )
                pushData_SSEapi(
                    f"Transaction {transaction_data['txid']} rejected as the oracle address {contractStructure['oracle_address']} is attempting to participate. Please report this to the contract owner"
                )
            ...
            try:
                ...
                logger.error(f"Error fetching dynamic swap price for contract {parsed_data['contractName']}: {e}")
                return 0
        ...
        if available_deposit_sum >= swapAmount:
            logger.info(
                f"Accepting 'tokenswapParticipation' transaction ID {transaction_data['txid']} from participant {inputlist[0]} "
                f"for {parsed_data['tokenAmount']} {parsed_data['tokenIdentification']}# to smart contract address "
                f"for swap amount {swapAmount} and swap-price {swapPrice}"
            )
            ...
            # Add token mapping if first interaction
            ...
            for a_deposit in available_deposits:
                if a_deposit.depositBalance > remaining_amount:
                    returnval = transferToken(
                        ...
                        perform_decimal_operation('multiply', remaining_amount, swapPrice),
                    ...
                    if returnval == 0:
                        logger.info("CRITICAL ERROR | Something went wrong in token transfer during local Smart Contract Participation deposit swap operation")
                        return 0

                    systemdb_connection = create_database_connection('system_dbs', {'db_name': 'system'})
                    firstInteractionCheck = systemdb_connection.execute(
                        f"SELECT * FROM tokenAddressMapping WHERE tokenAddress='{a_deposit.depositorAddress}' "
                        f"AND token='{contractStructure['accepting_token']}'"
                    ).fetchall()
                    if not firstInteractionCheck:
                        systemdb_connection.execute(
                            f"INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) "
                            f"VALUES ('{a_deposit.depositorAddress}', '{contractStructure['accepting_token']}', "
                    ...
                    logger.info(
                        f"Performing 'tokenswapSettlement' transaction ID {transaction_data['txid']} from participant {a_deposit.depositorAddress} "
                        f"for {perform_decimal_operation('multiply', a_deposit.depositBalance, swapPrice)} {contractStructure['accepting_token']}#"
                    )
                    returnval = transferToken(
                        contractStructure['accepting_token'],
                        perform_decimal_operation('multiply', a_deposit.depositBalance, swapPrice),
                        contractStructure['contractAddress'],
                        a_deposit.depositorAddress,
                        transaction_data=transaction_data,
                        parsed_data=parsed_data,
                    ...
                    systemdb_entry = systemdb_session.query(
            ...
            logger.info(
                f"Performing 'tokenswapParticipationSettlement' transaction ID {transaction_data['txid']} "
                f"from participant {outputlist[0]} for {swapAmount} {contractStructure['selling_token']}#"
            )
            returnval = transferToken(
            ...
            if returnval == 0:
                logger.info("CRITICAL ERROR | Something went wrong in token transfer during local Smart Contract Participation")
                return 0
            ...
            if not firstInteractionCheck:
                systemdb_connection.execute(
                    f"INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) "
                    f"VALUES ('{inputlist[0]}', '{contractStructure['selling_token']}', '{transaction_data['txid']}', "
                    f"'{transaction_data['blockheight']}', '{transaction_data['blockhash']}')"
                )
            systemdb_connection.close()
            ...
            pushData_SSEapi(
                ...
                f"with the transaction {transaction_data['txid']}"
            )

        else:
            rejectComment = (
                f"Swap participation at transaction {transaction_data['txid']} rejected as "
                f"requested swap amount is {swapAmount} but {available_deposit_sum} is available"
            )
            logger.info(rejectComment)
    ...
    if check_database_existence(
        ...
        if 'contractAddress' in parsed_data:
            if parsed_data['contractAddress'] != outputlist[0]:
                rejectComment = (
                    ...
                    f"doesnt not match with transaction's output address {outputlist[0]}"
                )
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data,
        ...
        session.add(
            ContractDeposits(
                ...
                unix_expiryTime=convert_datetime_to_arrowobject(
                    parsed_data['depositConditions']['expiryTime']
                ).timestamp(),   # ← FIXED: no trailing comma
                status='active',
                transactionHash=transaction_data['txid'],
                blockNumber=transaction_data['blockheight'],
                blockHash=transaction_data['blockhash']
        ...
        rejectComment = (
            ...
            f"the name {parsed_data['contractName']} at address {outputlist[0]} doesnt exist"
        )
        logger.info(rejectComment)
        rejected_contract_transaction_history(
            transaction_data,
