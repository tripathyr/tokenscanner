...
async def systemData():
    try:
        ...
        # Combine all 4 queries in system DB
        ...
        # Combine both queries in latestCache DB
...
async def getTokenList():
    ...
    try:
        ...
        # Fetch database names quickly and close the connection immediately
        ...
        # Process results outside the DB context
        token_list = []
...
async def getTokenInfo():
    try:
        ...
        # Fetch all DB data as quickly as possible
        async with get_mysql_conn_ctx(db_name) as conn:
            async with conn.cursor(DictCursor) as cursor:
                ...
                # Fetch number of distinct addresses
        ...
        # Transform associated contracts outside DB connection
...
async def getTokenTransactions():
    try:
        ...
        # --- Fetch DB rows as fast as possible ---
        async with get_mysql_conn_ctx(db_name) as conn:
            async with conn.cursor(DictCursor) as cursor:
        ...
        # Transform results outside DB context
        rowarray_list = {}
...
async def getTokenBalances():
    try:
        ...
        # --- Fetch from DB as quickly as possible ---
        async with get_mysql_conn_ctx(db_name) as conn:
            async with conn.cursor() as cursor:
        ...
        # Post-process the result outside DB context
        returnList = {
...
async def getFloAddressInfo():
    try:
        ...
        # --- Fetch data from DB as quickly as possible ---
        async with get_mysql_conn_ctx(db_name) as conn:
            async with conn.cursor() as cursor:
                # Fetch associated tokens
                query_tokens = "SELECT token FROM tokenAddressMapping WHERE tokenAddress = %s"
                await cursor.execute(query_tokens, (floAddress,))
                tokenNames = await cursor.fetchall()

                # Fetch incorporated contracts
                query_contracts = """
                ...
                """
                await cursor.execute(query_contracts, (floAddress,))
                incorporatedContracts = await cursor.fetchall()

        # --- DB connection released here ---

        # Prepare token details asynchronously
        detailList = {}
        if tokenNames:
            tasks = []
            ...
            # Run all token balance fetches concurrently
        ...
        # Prepare contract details outside DB connection
...
async def getContractInfo():
    ...
    try:
        ...
        # Fetch contract structure from the DB
        try:
        ...
        # Process contract structure data (outside DB connection)
        contractStructure = {}
        conditionDict = {}
        for item in result:
...
async def getcontractparticipants():
    try:
        ...
        # Fetch rows from DB but do no processing inside DB context
        triggers = []
        token_row = None
        participants_result = []
            ...
            async with conn.cursor(DictCursor) as cursor:
                ...
                if 'exitconditions' in contractStructure:
                    ...
                    if len(triggers) == 1:
                        ...
                        await cursor.execute(
                            'SELECT id, participantAddress, tokenAmount, userChoice, transactionHash, winningAmount '
                            'FROM contractparticipants'
                        )
                        participants_result = await cursor.fetchall()
        ...
        # Build returnval outside the connection context
        returnval = {}
...
async def getParticipantDetails():
    try:
        ...
        # Pre-fetch contract details from system DB for all contracts in one go
        contract_system_data = {}
        contracts_to_fetch = [(contract[3], contract[4]) for contract in participant_address_contracts]

        async with get_mysql_conn_ctx(system_db_name) as conn_system:
            async with conn_system.cursor() as cursor_system:
                for cname, caddr in contracts_to_fetch:
                    await cursor_system.execute('''
                    ...
                    contract_system_data[(cname, caddr)] = temp
            ...
            async with get_mysql_conn_ctx(db_name) as conn_contract:
                async with conn_contract.cursor() as cursor_contract:
                    ...
                    if contractStructure['contractType'] == 'continuos-event' and contractStructure['subtype'] == 'tokenswap':
                        ...
                        token_amount = result[0] if result else 0

                        temp = contract_system_data.get((contract_name, contract_address))
                        detailsDict.update({
                        ...
                        """, (floAddress,))
                        result = await cursor_contract.fetchone()
                        userChoice = result[0] if result else None
                        winningAmount = result[1] if result else None
...
async def getsmartcontracttransactions():
    try:
        ...
        # Fetch contract transaction data as quickly as possible
        async with get_mysql_conn_ctx(contract_db_name) as conn:
            async with conn.cursor() as cursor:
                query = '''
                    SELECT jsonData, parsedFloData 
        ...
        # Process rows outside the connection context
        returnval = {}
...
async def getLatestBlockDetails():
    try:
        ...
        # Fetch raw rows as quickly as possible
        async with get_mysql_conn_ctx(db_name) as conn:
            async with conn.cursor() as cursor:
                query = """
                    SELECT lb.* 
        ...
        # Process block details outside DB connection
        block_details = {}
        for item in latestBlocks:
            try:
                block_json = json.loads(item[3])
                block_hash = block_json.get('hash')
                if block_hash:
                    block_details[block_hash] = block_json
            except json.JSONDecodeError as json_error:
                print(f"JSON decode error for block {item[0]}: {json_error}")
            except Exception as e:
                print(f"Error processing block {item[0]}: {e}")
...
async def getblocktransactions(blockdetail):
    try:
        ...
        blockJsonRows = await blockdetailhelper(blockdetail)

        if len(blockJsonRows) != 0:
            block_json_data = json.loads(blockJsonRows[0][0])
            blocktxlist = block_json_data['txs']
    ...
    try:
        ...
        async with get_mysql_conn_ctx(system_db_name) as conn_system:
            async with conn_system.cursor() as cursor:
                await cursor.execute("SELECT COUNT(DISTINCT tokenAddress) FROM tokenAddressMapping")
                tokenAddressCount = (await cursor.fetchone())[0]

                await cursor.execute("SELECT COUNT(DISTINCT token) FROM tokenAddressMapping")
                tokenCount = (await cursor.fetchone())[0]

                await cursor.execute("SELECT COUNT(DISTINCT contractName) FROM contractAddressMapping")
                contractCount = (await cursor.fetchone())[0]

                await cursor.execute("SELECT value FROM systemData WHERE attribute='lastblockscanned'")
                lastscannedblock = int((await cursor.fetchone())[0])

        # --- LATEST CACHE DB QUERIES ---
        async with get_mysql_conn_ctx(latest_cache_db_name) as conn_cache:
            async with conn_cache.cursor() as cursor:
                await cursor.execute("SELECT COUNT(DISTINCT blockNumber) FROM latestBlocks")
                validatedBlockCount = (await cursor.fetchone())[0]

                await cursor.execute("SELECT COUNT(DISTINCT transactionHash) FROM latestTransactions")
                validatedTransactionCount = (await cursor.fetchone())[0]
...
async def tokenList():
    try:
        ...
        # --- PROCESS DATABASES OUTSIDE CONNECTION ---
...
async def tokenInfo(token):
    try:
        ...
        try:
            ...
            # --- Process fetched data outside DB connection ---
            associatedContractList = [
                {
                    'contractName': item['contractName'],
                    'contractAddress': item['contractAddress'],
...
async def tokenTransactions(token):
    try:
        ...
        # --- Connection block for DB operations ---
        async with get_mysql_conn_ctx(token_db_name) as conn:
            async with conn.cursor(DictCursor) as cursor:
        ...
        # --- JSON parsing and formatting outside DB connection ---
...
async def tokenBalances(token):
    try:
        ...
        try:
            # Fetch address balances from DB
            async with get_mysql_conn_ctx(token_db_name) as conn:
                async with conn.cursor() as cursor:
                    query = """
            ...
            # Transform data outside DB connection
            returnList = {
                address: balance
                for address, balance in addressBalances
            }
...
async def floAddressInfo(floAddress):
    try:
        ...
        # Fetch associated tokens and contracts in a single connection
        async with get_mysql_conn_ctx(standardize_db_name("system")) as conn:
            async with conn.cursor(DictCursor) as cursor:
                
                # Fetch tokens linked to the address
                await cursor.execute("""
        ...
        # Fetch token balances concurrently after closing DB connection
        if token_names:
        ...
        # Format smart contracts for response
...
async def floAddressBalance(floAddress):
    try:
        ...
        # Case 1: No token specified → fetch balances for all tokens
        if not token:
            ...
            # Fetch balances in parallel after DB connection is released
        ...
        # Case 2: Specific token → fetch only that token's balance
        else:
...
async def floAddressTransactions(floAddress):
    try:
        ...
        # Case 1: No token specified → fetch transactions for all tokens
        if token is None:
            async with get_mysql_conn_ctx(standardize_db_name("system")) as system_conn:
                async with system_conn.cursor(DictCursor) as cursor:
                    await cursor.execute("""
            ...
            # Fetch transactions in parallel after connection is released
            tasks = [
        ...
        # Case 2: Specific token provided
        else:
        ...
        print(f"Database error in floAddressTransactions: {e}")
        return jsonify(description="Database error occurred"), 500

    except Exception as e:
        print(f"floAddressTransactions: {e}")
        return jsonify(description="Unexpected error occurred"), 500
...
async def getContractList_v2():
    try:
        ...
        # Morph smart contract data outside DB context
        smart_contracts_morphed = await smartcontract_morph_helper(smart_contracts)

        # Fetch committee list outside DB context
...
async def getContractInfo():
    try:
        ...
        # --- Fetch contract structure & participant data ---
