...
def process_committee_flodata(flodata):
    ...
    a list of committee addresses after applying adds/removes.
    """
    flo_address_list = []
    try:
        ...
        # Process removals second
        for action in contract_committee_actions:
            if action == 'remove':
                for floid in contract_committee_actions[action]:
                    if floid in flo_address_list:
                        flo_address_list.remove(floid)
                    else:
                        logger.warning(f"Tried to remove non-existing committee address: {floid}")
...
def refresh_committee_list(admin_flo_id, api_url, blocktime):
    ...
    for txid in transactions:
        try:
            url_tx = urljoin(api_url, f'api/tx/{txid}')
            logger.info(f"Fetching committee transaction: {url_tx}")
            ...
            vin_addresses = tx_info.get('vin', [{}])[0].get('addresses', [])
            blocktime_value = tx_info.get('blocktime', float('inf'))

            if (
                vin_addresses and
                vin_addresses[0] == admin_flo_id and
                blocktime_value <= blocktime
            ):
                try:
                    tx_flodata = json.loads(tx_info.get('floData', '{}'))
                    addresses = process_committee_flodata(tx_flodata)
                    # Add unique addresses only
                    for addr in addresses:
                        if addr not in committee_list:
                            committee_list.append(addr)
                except Exception as e:
...
def processBlock(blockindex=None, blockhash=None, blockinfo=None, keywords=None):
    ...
    while True:
        ...
        # Always save block if any parsing happened
        if any_parsed:
            # Attach parsed results
    ...
    try:
        connection.execute(
            ...
            (
                ...
                contractStatus,
                contractStructure['tokenIdentification'],
                contractStructure['contractType'],
                transactionHash,
                blockNumber,
                blockHash,
                incorporationDate,
    ...
    for query in active_deposits:
        ...
        if query.activity == 'contract-deposit':
            if blocktime > query_time:
                ...
                deposit_query = contract_db.query(ContractDeposits).filter(
                ...
                sellingToken = contract_db.query(
                ...
                if float(returnAmount) <= 0:
                    ...
                    # Record the return as "deposit-return" in the DB
                    ...
                    # 🔶 CHANGED: UPDATE TimeActions instead of adding new record
                ...
                # Otherwise, proceed with the token transfer
...
def process_continuous_event_transfer(
    ...
    # --- Fetch contract subtype ---
    try:
        logger.info("Fetching contract subtype...")
        contract_subtype_row = contract_session.query(ContractStructure.value).filter(
            ContractStructure.attribute == 'subtype'
        ).first()
        logger.info(f"contract_subtype_row = {contract_subtype_row}")
    except Exception as e:
        logger.error(f"Failed to query contract subtype: {e}")
        logger.error(traceback.format_exc())
        return 0

    if not contract_subtype_row:
        logger.error(f"Unable to find subtype for contract {parsed_data['contractName']}")
        return 0

    contract_subtype = contract_subtype_row[0]
    logger.info(f"Contract subtype = {contract_subtype}")

    if contract_subtype == 'tokenswap':

        # --- Check transaction hash exists ---
        try:
            logger.info("Checking if transaction already exists in contractparticipants table...")
            ...
            logger.info(f"participantAdd_txhash_T = {participantAdd_txhash_T}")
        except Exception as e:
            logger.error(f"Failed checking contractparticipants: {e}")
            logger.error(traceback.format_exc())
            return 0
        ...
        if 'contractAddress' in parsed_data:
            logger.info("Checking if contractAddress matches outputlist...")
            if parsed_data['contractAddress'] != outputlist[0]:
                rejectComment = f"Contract participation at transaction {transaction_data['txid']} rejected as contractAddress specified in flodata, {parsed_data['contractAddress']}, doesn't match with transaction's output address {outputlist[0]}"
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data, parsed_data,
                    'participation', outputlist[0],
                    inputadd, outputlist[0], rejectComment
                )
                pushData_SSEapi(f"Error| Mismatch in contract address specified in flodata and the output address of the transaction {transaction_data['txid']}")
                return 0
            logger.info("Contract address matches outputlist.")

        # --- Determine swap price ---
        logger.info("Determining swap price...")
        try:
            if contractStructure['pricetype'] in ['predetermined', 'determined']:
                swapPrice = float(contractStructure['price'])
                logger.info(f"Predetermined swapPrice = {swapPrice}")
            elif contractStructure['pricetype'] == 'dynamic':
                logger.info("Contract uses dynamic pricing...")
                if transaction_data['senderAddress'] == contractStructure['oracle_address']:
                    logger.warning(
                        f"Transaction {transaction_data['txid']} rejected: oracle address {contractStructure['oracle_address']} trying to participate."
                    )
                    pushData_SSEapi(
                ...
                logger.info(f"Fetched dynamic swapPrice = {swapPrice}")

                priceTimestamp = blockinfo['time']
                priceBlockheight = blockinfo['height']
                logger.info(f"Updating dynamic price at blockheight {priceBlockheight}...")
                ...
                logger.info("Dynamic swap price updated.")
            ...
            logger.error(f"Error determining swap price: {e}")
            logger.error(traceback.format_exc())
            return 0
        ...
        try:
            ...
            logger.error(f"Failed calculating swap amount: {e}")
            logger.error(traceback.format_exc())
            return 0

        # --- Query active deposits ---
        try:
            logger.info("Querying active contract deposits...")
            ...
            logger.error(f"Error querying contract deposits: {e}")
            logger.error(traceback.format_exc())
            return 0
        ...
        if available_deposit_sum < swapAmount:
            rejectComment = (
                f"Swap participation at transaction {transaction_data['txid']} rejected as "
                f"requested swap amount is {swapAmount} but only {available_deposit_sum} is available"
            )
            logger.info(rejectComment)
        ...
        logger.info(f"Proceeding with tokenswapParticipation for transaction {transaction_data['txid']}")

        # ------------------------------------------------------------
        # 🔥 Execute the actual token swap
        # ------------------------------------------------------------

        # 1. Transfer participant's tokens (e.g. USD#) → contract address
        returnval = transferToken(
        ...
        # 2. Transfer selling token (e.g. tokenroom#) → participant
        remaining_amount = Decimal(str(swapAmount))
        for a_deposit in available_deposits:
            if remaining_amount <= Decimal("0"):
                break

            deposit_balance = Decimal(str(a_deposit.depositBalance))

            if deposit_balance > remaining_amount:
                settlement_amount = perform_decimal_operation('multiplication', remaining_amount, swapPrice)
                ...
                if returnval == 0:
                    logger.info("CRITICAL ERROR | Error during swap deposit settlement")
                    return 0
                ...
                new_balance = perform_decimal_operation(
                    'subtraction',
                    deposit_balance,
                    remaining_amount
                )
                ...
                settlement_amount = perform_decimal_operation('multiplication', deposit_balance, swapPrice)
                ...
                remaining_amount = perform_decimal_operation(
                    'subtraction',
                    remaining_amount,
                    deposit_balance
                )

        # 3. Transfer selling token to participant
        returnval = transferToken(
        ...
        if returnval == 0:
            logger.info("CRITICAL ERROR | Something went wrong in the token transfer method while doing local Smart Contract Participation")
            return 0

        # 4. Record contract participant
        contract_session.add(ContractParticipants(
            ...
            blockNumber=blockinfo['height'],
            blockHash=blockinfo['hash'],
            winningAmount=float(swapAmount)
        ))
        ...
        add_contract_transaction_history(
            ...
            transferAmount=float(swapAmount),
            blockNumber=blockinfo['height'],
            blockHash=blockinfo['hash'],
            blocktime=blockinfo['time'],
            transactionHash=transaction_data['txid'],
        ...
        # 5. Update token mapping for participant
        ...
        firstInteractionCheck = systemdb_connection.execute(
            f"SELECT * FROM tokenAddressMapping WHERE tokenAddress='{inputlist[0]}' AND token='{contractStructure['selling_token']}'"
        ).fetchall()
        if len(firstInteractionCheck) == 0:
            systemdb_connection.execute(
                f"INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) "
                f"VALUES ('{inputlist[0]}', '{contractStructure['selling_token']}', '{transaction_data['txid']}', '{transaction_data['blockheight']}', '{transaction_data['blockhash']}')"
            )
        ...
        pushData_SSEapi(
            f"Token swap successfully performed at contract {parsed_data['contractName']}_{outputlist[0]} with the transaction {transaction_data['txid']}"
        )
...
def process_smart_contract_deposit(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    if check_database_existence(
        ...
        expiry_time_obj = convert_datetime_to_arrowobject(parsed_data['depositConditions']['expiryTime'])
        logger.info(f"Parsed expiry_time_obj: {expiry_time_obj}")
        logger.info(f"type(expiry_time_obj.timestamp): {type(expiry_time_obj.timestamp)}")

        # Correctly treat timestamp as property, not method
        expiry_time_ts = expiry_time_obj.timestamp
        logger.info(f"expiry_time_ts (unix): {expiry_time_ts}")

        if blockinfo['time'] > expiry_time_ts:
            rejectComment = (
        ...
        if (
            participantAdd_txhash_T
            and transaction_data['txid'] in list(participantAdd_txhash_T[1])
        ):
            rejectComment = (
                f"Contract deposit at transaction {transaction_data['txid']} "
                f"rejected as it already exists in the Smart Contract db. "
                f"This is unusual, please check your code"
            )
            logger.warning(rejectComment)
            rejected_contract_transaction_history(
                transaction_data,
        ...
        if 'contractAddress' in parsed_data:
            if parsed_data['contractAddress'] != outputlist[0]:
                rejectComment = (
                    f"Contract deposit at transaction {transaction_data['txid']} rejected as "
                    f"contractAddress specified in flodata, {parsed_data['contractAddress']}, "
                    f"doesn't match with transaction's output address {outputlist[0]}"
                )
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data,
                ...
                pushData_SSEapi(
                    f"Error| Mismatch in contract address specified in flodata and the output "
                    f"address of the transaction {transaction_data['txid']}"
                )
                return 0
        ...
        session.add(
            ContractDeposits(
                ...
                unix_expiryTime=expiry_time_ts,
                status='active',
                transactionHash=transaction_data['txid'],
                blockNumber=transaction_data['blockheight'],
                blockHash=transaction_data['blockhash']
        ...
        session.commit()
        pushData_SSEapi(
            f"Deposit Smart Contract Transaction {transaction_data['txid']} "
            f"for the Smart contract named {parsed_data['contractName']} "
            f"at the address {outputlist[0]}"
        )

        # If this is the first interaction of the outputlist's address with the given token name, add it to token mapping
        systemdb_connection = create_database_connection(
            'system_dbs',
            {'db_name': 'system'}
        )
        firstInteractionCheck = systemdb_connection.execute(
        ...
        if len(firstInteractionCheck) == 0:
            systemdb_connection.execute(
                f"INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) "
                f"VALUES ('{outputlist[0]}', '{parsed_data['tokenIdentification']}', "
        ...
        rejectComment = (
            f"Deposit Transaction {transaction_data['txid']} rejected as a Smart Contract with "
            f"the name {parsed_data['contractName']} at address {outputlist[0]} doesn't exist"
        )
        logger.info(rejectComment)
        rejected_contract_transaction_history(
            transaction_data,
