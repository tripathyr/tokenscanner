        ...
        if amountDeposited < minimumsubscriptionamount:
            ...
            try:
                ...
                for participant in contractParticipants:
                    ...
                    connection.execute(
                        "UPDATE contractparticipants SET winningAmount=%s WHERE participantAddress=%s AND transactionHash=%s",
                        (tokenAmount, participantAddress, txHash)
                    )
    ...
    try:
        ...
        if tokenSum > 0:
            contractWinners = connection.execute(
                "SELECT * FROM contractparticipants WHERE userChoice = %s",
                (parsed_data["triggerCondition"],)
            ).fetchall()

            winnerRows = connection.execute(
                "SELECT tokenAmount FROM contractparticipants WHERE userChoice = %s",
                (parsed_data["triggerCondition"],)
            ).fetchall()
