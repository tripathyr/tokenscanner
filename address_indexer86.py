        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Error releasing MySQL connection: {release_error}")
            ...
            async with aiohttp.ClientSession() as session:
                while True:
                    ...
                    try:
                        ...
                        async with session.get(paginated_url, timeout=API_TIMEOUT) as response:
                            if response.status != 200:
                                logger.error(f"Failed to fetch paginated data for {flo_address} (Status: {response.status}). Skipping to next batch.")
                            ...
                            if len(transactions) < batch_size:
                                logger.info(f"Fetched all transactions for address: {flo_address}. Total: {total_transactions_fetched}")
                                break
                    except Exception as e:
                        logger.error(f"Error during paginated fetch for {flo_address} at {paginated_url}: {e}", exc_info=True)
                        from_index += batch_size  # Skip to the next batch
            ...
            for txid in all_new_txids:
                try:
                    ...
                    logger.error(f"Error processing transaction {txid}: {e}", exc_info=True)
            ...
            logger.error(f"Error fetching address data for {flo_address} using {get_indexer_url(current_index)}: {e}", exc_info=True)
            current_index += 1
