...
# Dynamically assign ZMQ_BLOCK_HASH_ADDRESS from config.ini
try:
...
# Constants
API_TIMEOUT = int(config['API'].get('API_TIMEOUT', 10))
...
# Address Indexer Configurations
...
ADDRESS_INDEXER_URLS = [url.strip() for url in config['ADDRESS_INDEXER'].get('URL', '').split(',')]
...
async def initialize_connection_pool():
    ...
    maxsize = 45  # Default maxsize for the pool
...
async def b_process_block(blockhash):
    ...
    if block_data.get("error") or not block_data.get("result"):
        return f"Error fetching block data: {block_data.get('error')}"
    ...
    items_on_page = 1000  # Define the max number of items per page
    total_pages = math.ceil(tx_count / items_on_page)
    current_page = 1  # Assuming we are always displaying the first page in this example
...
async def process_address_externalsource(flo_address):
    """
    Process and store transactions for a specific FLO address using external sources.
    """
    current_index = 0
    ...
    while current_index < len(ADDRESS_INDEXER_URLS):
        ...
        try:
            # Fetch stored transaction IDs for this FLO address
            conn = await get_mysql_connection()
            try:
                async with conn.cursor() as cursor:
                    await cursor.execute(f"USE `{ADDRESS_INDEXER_DB_NAME}`")
            ...
            # Fetch address data
            async with aiohttp.ClientSession() as session:
                logger.info(f"Fetching address data from {address_api_url}")
                async with session.get(address_api_url, timeout=API_TIMEOUT) as response:
                    if response.status != 200:
                        logger.error(f"Failed to fetch address data for {flo_address} (Status: {response.status}). Moving to next URL.")
                        current_index += 1
                        continue
                    address_data = await response.json()
                    logger.info(f"Fetched address data successfully for {flo_address}. Transactions: {len(address_data.get('transactions', []))}")

            # Filter new transactions
            new_txids = [txid for txid in address_data.get("transactions", []) if txid not in stored_txids]
            logger.info(f"{len(new_txids)} new transactions to process for address: {flo_address}")

            # Process new transactions
            for txid in new_txids:
                try:
                    # Construct the transaction data URL
                    transaction_data_url = f"{SELF_URL}/api/tx/{txid}"
                    async with aiohttp.ClientSession() as session:
                        async with session.get(transaction_data_url, timeout=API_TIMEOUT) as response:
                            if response.status != 200:
                    ...
                    logger.info(f"Storing transaction data for {txid}")
...
async def store_transaction_data(transaction_data):
    """
    Store transaction data in the database and update the address-to-transaction mapping.
    Handles multiple addresses in vin and vout properly.
    """
    conn = await get_mysql_connection()
    transaction_query = f"""
    ...
    failed_mappings = []  # List to track failed mappings
        ...
        async with conn.cursor() as cursor:
            ...
            for vout in transaction_data.get("vout", []):
                if "scriptPubKey" in vout and "addresses" in vout["scriptPubKey"]:
                    vout_addresses.extend(vout["scriptPubKey"]["addresses"])  # Collect all addresses from vout

            raw_transaction_json = json.dumps(transaction_data)
            flo_data = transaction_data.get("floData", None)
            blockheight = transaction_data.get("blockheight")

            # Fetch block number if not already available
            if blockheight is None and "blockhash" in transaction_data:
                try:
                    ...
                    logger.error(f"Error fetching block height for blockhash {transaction_data['blockhash']}: {e}")
            ...
            for address, txid in mappings:
                try:
                    ...
                    if "Deadlock" in str(e):
                        logger.warning(f"Deadlock detected for {address} and {txid}. Adding to retry queue.")
                        failed_mappings.append((address, txid))  # Add to failed mappings
                    else:
                        logger.error(f"Error adding mapping for {address} and {txid}: {e}")
                        raise  # Re-raise non-deadlock exceptions
            ...
            if failed_mappings:
                logger.info(f"Retrying {len(failed_mappings)} failed mappings.")
                for address, txid in failed_mappings:
                    try:
                        await cursor.execute(mapping_query, (address, txid))
                        logger.info(f"Successfully retried mapping for address {address} and transaction {txid}.")
                    except Exception as e:
                        logger.error(f"Retry failed for {address} and {txid}: {e}")
...
async def process_blocks(start_block, end_block, batch_size=2000, max_concurrent_batches=10, retries=5):
    ...
    async def process_transaction(cursor, tx, blockhash, block_height, block_time):
        """Process a single transaction, adding it to the database and mappings."""
        txid = tx["txid"]
        vin_addresses = []
        ...
        for vin in tx.get("vin", []):
            try:
                ...
                logger.error(f"Error fetching input address for vin: {vin}, error: {e}")
        ...
        # Use block_time as transaction_time
        transaction_time = block_time
        ...
        raw_transaction_json = json.dumps(tx)
        ...
        try:
            await cursor.execute(
                ...
                (
                    ...
                    blockhash,
                    block_height,
                    transaction_time,  # Always use block_time
        ...
        async with semaphore:
            ...
            try:
                async with conn.cursor() as cursor:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(block_data_url, timeout=API_TIMEOUT) as response:
                            if response.status != 200:
...
async def process_all_transactions_for_coinbase(conn, flo_address, results, page=1, page_size=1000):
    """
    Process all transactions for the given FLO address when it appears in a coinbase transaction.
    Fetch full block information for accurate block-level calculations using SELF_URL.
    Store balance data and return paginated transactions.
    """
        ...
        try:
            ...
            if not raw_transaction.get("vin"):
                ...
                if blockhash not in processed_blocks:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(f"{block_api_url}/{blockhash}") as response:
                            if response.status != 200:
                    ...
                    # Step 3: Total block reward
                    block_reward = coinbase_reward + total_transaction_fees
    ...
    # Log the first transaction for minimal logging
    if paginated_transactions:
        logger.info(f"First Transaction: {json.dumps(paginated_transactions[0], indent=4)}")
    else:
        logger.info("No transactions to log.")
    ...
    response_data = OrderedDict([
        ...
        ("addrStr", flo_address),
        ("balance", float(truncate_satoshi_to_btc(balance))),  # Convert and truncate
        ("balanceSat", int(balance * Decimal(1e8))),
        ("totalReceived", float(truncate_satoshi_to_btc(total_received))),  # Convert and truncate
        ("totalReceivedSat", int(total_received * Decimal(1e8))),
        ("totalSent", float(truncate_satoshi_to_btc(total_sent))),  # Convert and truncate
...
async def calculate_balance_and_update(conn, flo_address):
    """
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            balance_sat = Decimal(stored_data['balance_sat']) if stored_data else Decimal(0)
            total_received_sat = Decimal(stored_data['total_received_sat']) if stored_data else Decimal(0)
            total_sent_sat = Decimal(stored_data['total_sent_sat']) if stored_data else Decimal(0)
            last_computed_blockheight = stored_data['last_computed_blockheight'] if stored_data else 0

            # Fetch new transactions
            await cursor.execute(new_transactions_query, (flo_address, last_computed_blockheight))
            new_transactions = await cursor.fetchall()
            ...
            for transaction in new_transactions:
                try:
                    ...
                    logger.error(f"Error processing new transaction {transaction.get('txid')}: {e}")
                    continue

            # Update stored balances with the latest block height
            if new_transactions:
                latest_block = max(tx["blockheight"] for tx in new_transactions)
                await cursor.execute(store_balance_query, (
                    flo_address,
                    int(balance_sat),
...
async def get_block_by_hash(blockhash):
    ...
    try:
        # Use the b_run_block_process function to process the block
        block_output = await b_process_block(blockhash)

        # Serialize the output using json.dumps to preserve order
        return Response(
            json.dumps(block_output),  # Ensure block_output is already ordered
            status=200,
...
async def get_tx_by_hash(txhash):
    ...
    await b_compute_transaction_values(tx_data, blockhash, None)
...
async def fetch_priority_transactions(flo_address):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(address_api_url, params=query_params) as response:
                if response.status != 200:
...
async def get_block_or_tx(hash_value):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            block_task = session.get(block_url, timeout=API_TIMEOUT)
            tx_task = session.get(transaction_url, timeout=API_TIMEOUT)
            ...
            if block_data and tx_data:
                # Return the response with the longer content
                ...
                # Neither response contained valid data
                return Response(
...
async def get_block_by_number(block_height):
    ...
    try:
        ...
        block_data_url = f"{SELF_URL}/api/block/{blockhash}"
        async with aiohttp.ClientSession() as session:
            async with session.get(block_data_url, timeout=API_TIMEOUT) as block_data_response:
                ...
                block_data = await block_data_response.json()
...
async def rollback_all_blocks(conn, start_block, end_block):
    ...
    and adjust the balances of associated indexed addresses.
    ...
    """
    # Queries for rollback operations
    fetch_block_transactions_query = """
    SELECT txid, raw_transaction_json, blockheight, blockhash
    FROM `transactions`
    ...
    delete_processed_blocks_query = """
    DELETE FROM `processed_blocks`
    WHERE block_height BETWEEN %s AND %s;
    """

    delete_address_to_tx_query = """
    DELETE FROM `address_to_tx`
    WHERE txid IN (
        SELECT txid
        FROM `transactions`
        WHERE blockheight BETWEEN %s AND %s
    ...
    fetch_indexed_addresses_query = """
    SELECT DISTINCT flo_address
    FROM balance_data;
    """
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            # Process each transaction to rollback balance effects
            for transaction in transactions_to_rollback:
                try:
                    ...
                    if blockhash not in processed_blocks:
                        ...
                        # Process block-level transaction fees
                        total_transaction_fees = Decimal(0)
                        for tx in block_data['txs']:
                            if 'vin' in tx and not tx['vin'][0].get("coinbase"):
                                tx_value_in = sum(Decimal(vin.get("value", 0)) for vin in tx.get("vin", []))
                        ...
                        # Divide fees proportionally among miner addresses (coinbase vouts)
                        if total_transaction_fees > 0:
                            for vout in coinbase_tx.get("vout", []):
                                addresses = vout.get("scriptPubKey", {}).get("addresses", [])
                                value_sat = Decimal(vout.get("value", 0)) * Decimal(1e8)
                                if value_sat > 0:
                                    fee_share = total_transaction_fees * (value_sat / sum(
                                        Decimal(v.get("value", 0)) * Decimal(1e8)
                                        for v in coinbase_tx.get("vout", [])
                                    ))
                                    for address in addresses:
                                        if address in indexed_addresses:
                                            address_balance_changes[address]["received"] -= fee_share
            ...
            await cursor.execute(delete_transactions_query, (start_block, end_block))
            await cursor.execute(delete_processed_blocks_query, (start_block, end_block))
            await cursor.execute(delete_address_to_tx_query, (start_block, end_block))
            ...
            # Commit changes
            await conn.commit()
            logger.info(f"Rollback and balance adjustment completed for blocks {start_block} to {end_block}.")
...
async def websocket_endpoint():
    ...
    """
    # Log that the handshake was initiated
    ...
    logger.info(f"New WebSocket client added: {client}")
    ...
    try:
        while True:
            ...
            if subscription_request.get('method') == 'subscribeNewBlock':
                logger.info(f"Client subscribed to new block notifications: {client}")
...
async def clean_idle_connections_with_async_pool(idle_time_threshold=2):
    if not async_connection_pool:
        raise RuntimeError("Async connection pool is not initialized.")

    semaphore = asyncio.Semaphore(max_concurrent_tasks)

    async def terminate_connection(connection_id, idle_time):
        async with semaphore:
    ...
    async with async_connection_pool.acquire() as conn:
        async with conn.cursor() as cursor:
            ...
            tasks = [
                terminate_connection(connection_id, time)
                for connection_id, time in idle_connections
            ]
            await asyncio.gather(*tasks)
            logger.info(f"Terminated {len(idle_connections)} idle connections.")
...
async def scheduled_cleanup():
    try:
        result = await clean_idle_connections_with_async_pool(idle_time_threshold=5)
...
async def main():
    try:
        ...
        # Add ZMQ subscriber as an asyncio task
