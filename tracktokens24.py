            ...
            if blockindex is not None and blockhash is None:
                ...
                while blockhash is None or blockhash == '':
                    ...
                    try:
                        ...
                        logger.info(f"API call hash/{blockindex} failed to give proper response. Retrying.")
...
def initialize_neturl(blockbook_neturl_list):
    ...
    while len(tried_servers) < len(blockbook_neturl_list):
        ...
        try:
            ...
            print(f"Testing BLOCKBOOK_NETURL: {neturl.rstrip('/')}/api/latest-block")
            response = requests.get(f"{neturl.rstrip('/')}/api/latest-block", verify=True, timeout=10)
            ...
            response_data = response.json()
            if "blockheight" in response_data and "blockhash" in response_data and "latest_time" in response_data:
...
def fetch_current_block_height():
    ...
    while current_index == -1:  # Keep trying until a valid block height is retrieved
        try:
            ...
            current_index = response['blockheight']  # Extract block height from response
