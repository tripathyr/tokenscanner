...
def run_cleaner_in_thread():
    ...
    def thread_target():
        ...
        async def main():
            ...
            cleaner_connection_pool = await create_cleaner_connection_pool(loop)  # Create cleaner pool with size 2
            ...
            try:
                ...
                await shutdown_pool(cleaner_connection_pool)
...
def stop_cleaner_thread():
    ...
    if cleaner_connection_pool:
        loop = asyncio.new_event_loop()
        loop.run_until_complete(shutdown_pool(cleaner_connection_pool))
...
def signal_handler(sig, frame):
    ...
    if loop.is_running():
        ...
        asyncio.run(shutdown(loop))  # Ensure loop is passed to shutdown()
...
async def shutdown(loop):
    ...
    global async_connection_pool, cleaner_connection_pool
