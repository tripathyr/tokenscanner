...
async def initialize_connection_pool():
    ...
    # First, ensure the database exists before initializing the connection pool
    for port in ports_to_try:
        try:
            ...
            temp_pool = await aiomysql.create_pool(
                ...
                db=None,  # Connect without selecting a database
                charset="utf8mb4",
                maxsize=1,  # Temporary pool size
                minsize=1,
                loop=asyncio.get_event_loop(),
            )
            async with temp_pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    logger.info(f"Checking if database `{ADDRESS_INDEXER_DB_NAME}` exists on port {port}.")
                    await cursor.execute(f"CREATE DATABASE IF NOT EXISTS `{ADDRESS_INDEXER_DB_NAME}`;")
                    logger.info(f"Database `{ADDRESS_INDEXER_DB_NAME}` is ready.")
            temp_pool.close()
            await temp_pool.wait_closed()

            # Initialize the actual connection pool for the database
            async_connection_pool = await aiomysql.create_pool(
                host=MYSQL_HOST,
                user=MYSQL_USER,
                port=port,
                password=MYSQL_PASSWORD,
                db=ADDRESS_INDEXER_DB_NAME,  # Bind the connection pool to the specific database
                charset="utf8mb4",
                maxsize=maxsize,
                minsize=1,
                loop=asyncio.get_event_loop(),
            ...
            logger.info(f"Connection pool initialized on port {port} with maxsize={maxsize} for database `{ADDRESS_INDEXER_DB_NAME}`.")
...
async def initialize_database():
    """Create and initialize the necessary tables in the database."""
    ...
    create_balance_data_table_query = """
        CREATE TABLE IF NOT EXISTS balance_data (
            ...
            appeared_in_coinbase BOOLEAN DEFAULT FALSE,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        ...
        while current_index < len(ADDRESS_INDEXER_URLS):
            ...
            try:
                ...
                try:
                    ...
                    conn.release()  # Properly release the connection
...
async def store_transaction_data(transaction_data):
    ...
    """
    mapping_query = """
    INSERT IGNORE INTO address_to_tx (address, txid)
    VALUES (%s, %s)
    """
    utxo_insert_query = """
    INSERT INTO utxos (
    ...
    try:
        ...
        try:
            conn.release()  # Release connection using updated method
        except Exception as release_error:
...
async def get_last_processed_block():
    """
    Fetch the highest block height from the processed_blocks table.
    Returns 0 if no blocks are processed or in case of an error.
    """
    try:
...
async def process_blocks(start_block, end_block, batch_size=2000, max_concurrent_batches=5, retries=5):
    ...
    async def process_transaction(cursor, tx, blockhash, block_height, block_time):
        ...
        # Process `vin` to fetch input addresses and DELETE UTXOs
        ...
        for vout_index, vout in enumerate(tx.get("vout", [])):
            try:
                if "scriptPubKey" in vout and "addresses" in vout["scriptPubKey"]:
                    ...
                    await cursor.execute(
                        """
                        INSERT INTO utxos (txid, vout, address, amount, satoshis, block_height, spent)
...
async def address_fetch(flo_address):
    ...
    try:
        ...
        page = int(query_params.get("page", 1))  # Default to 1 if page is absent
        pagesize = int(query_params.get("pagesize", 1000))  # Default to 1000 if pagesize is absent
        details = query_params.get("details", "txs")  # Default to "txs"
...
async def get_latest_block():
    ...
    """
    try:
        logger.info("Attempting to fetch the latest block...")
        async with (await get_mysql_connection()) as conn:
            async with conn.cursor() as cursor:
                logger.info("Executing query on processed_blocks table...")
                query = """
                ...
                if not result:
                    logger.warning("No blocks found in the processed_blocks table.")
                    return jsonify({"error": "No blocks found in the processed_blocks table"}), 400

                block_height, block_hash, block_time = result
                logger.info(f"Fetched block_height: {block_height}, block_hash: {block_hash}, block_time: {block_time}")
...
async def get_utxos_for_address(address):
    ...
    Query Parameters:
        - confirmed (boolean, optional): If true (default), only return UTXOs with confirmations > 0.
    """
    try:
        # Validate the address
        if not is_valid_flo_address(address):
            return jsonify({"error": "Invalid FLO address"}), 400

        # Extract and handle the 'confirmed' query parameter, defaulting to true
        confirmed_only = request.args.get('confirmed', 'true').lower() in ['true', '1', 'yes']
        ...
        async with (await get_mysql_connection()) as conn:
            async with conn.cursor() as cursor:
                ...
                if not utxos:
                    return jsonify({"utxos": [], "message": "No UTXOs found for the given address"}), 200
                ...
                for utxo in utxos:
                    ...
                    if block_height is None:
                        logger.warning(f"Invalid block_height for UTXO: {utxo}")
                        continue  # Skip UTXOs with invalid block_height
