...
def process_smart_contract_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if parsed_data['contractAddress'] != inputadd:
        ...
        logger.info(rejectComment); rejected_contract_transaction_history(transaction_data, parsed_data, 'incorporation', inputadd, inputadd, outputlist[0] if outputlist else None, rejectComment)
...
def create_one_time_event_contract(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if parsed_data.get('contractAddress') == inputadd:
        ...
        session.commit()
        session.add(ContractAddressMapping(address=inputadd, addressType='incorporation', tokenAmount=None, contractName=parsed_data.get('contractName',''), contractAddress=inputadd, transactionHash=transaction_data.get('txid',''), blockNumber=transaction_data.get('blockheight'), blockHash=transaction_data.get('blockhash')))
        ...
        session.commit(); session.close()
        updateLatestTransaction(transaction_data, parsed_data, f"{parsed_data.get('contractName','')}-{parsed_data.get('contractAddress','')}")
    ...
    if parsed_data.get('contractAddress') == inputadd:
        ...
        if 'subtype' in contractConditions:
            ...
            if (contractConditions.get('subtype') == 'tokenswap' and isinstance(selling_token,str) and isinstance(accepting_token,str) and check_database_existence('token', {'token_name': selling_token.split('#')[0]}) and check_database_existence('token', {'token_name': accepting_token.split('#')[0]})):
                ...
                system_session.commit()
                system_session.add(ContractAddressMapping(address=inputadd, addressType='incorporation', tokenAmount=None, contractName=parsed_data.get('contractName',''), contractAddress=inputadd, transactionHash=transaction_data.get('txid',''), blockNumber=transaction_data.get('blockheight'), blockHash=transaction_data.get('blockhash')))
...
def process_smart_contract_deposit(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    if check_database_existence('smart_contract', {'contract_name': f"{parsed_data['contractName']}", 'contract_address': f"{outputlist[0]}"}):
        ...
        session.add(TimeActions(time=parsed_data['depositConditions']['expiryTime'], activity='contract-deposit', status='active', contractName=parsed_data['contractName'], contractAddress=outputlist[0], contractType='continuous-event-swap', tokens_db=f"{parsed_data['tokenIdentification']}", parsed_data=json.dumps(parsed_data), transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight']))
        session.commit()
