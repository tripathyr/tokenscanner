...
async def process_address_externalsource(flo_address):
    """
    Process and store transactions for a specific FLO address using external sources with support for paginated requests.
    """
    current_index = 0
    logger.info(f"Starting transaction processing for FLO address: {flo_address}")

    async with aiohttp.ClientSession() as session:  # Single session for the entire function
        while current_index < len(ADDRESS_INDEXER_URLS):
            ...
            try:
                ...
                # Pagination setup
                from_index = 0
                batch_size = 100  # Number of transactions to fetch in each request
                ...
                while True:
                    ...
                    try:
                        ...
                        # If less than the batch size is returned, assume all transactions are fetched
