...
def refresh_committee_list(admin_flo_id, api_url, blocktime):
    ...
    def process_transaction(transaction_info):
        ...
        try:
            ...
            logger.warning(f"Failed to parse floData: {e}")
            pass
...
def process_smart_contract_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    # PATCH: Correct typo if present
    if parsed_data.get('contractType', '').lower() == 'continuos-event':
        logger.info("Correcting typo in contractType from 'continuos-event' to 'continuous-event'")
    ...
    if not check_database_existence('smart_contract', {'contract_name': f"{parsed_data['contractName']}", 'contract_address': f"{parsed_data['contractAddress']}"}):
        ...
        if len(tokenAddressMapping_of_contractAddress) == 0:
            ...
            if parsed_data['contractType'] == 'one-time-event':
                ...
                if parsed_data['contractAddress'] == inputadd:
                    ...
                    session.add(TimeActions(
                        time=parsed_data['contractConditions']['expiryTime'],
                    ...
                    pushData_SSEapi(f'Contract | Contract incorporated at transaction {transaction_data["txid"]} with name {parsed_data["contractName"]}-{parsed_data["contractAddress"]}')
                    return 1
                else:
            ...
            if parsed_data['contractType'] == 'continuous-event':
                ...
                # (rest of your continuous-event block remains unchanged)
                # We just removed the OR check for the typo and rely on the correction above
