        ...
        try:
            ...
            try:
                ...
                existing_block = conn.execute(
                    'SELECT * FROM RecentBlocks WHERE blockNumber = %s',
                    (block_number,)
                ).fetchone()
                if existing_block:
                ...
                block_count = conn.execute(
                    'SELECT COUNT(*) FROM RecentBlocks'
                ).fetchone()[0]
                ...
                if block_count > 1000:
                    ...
                    for block in oldest_blocks:
                        conn.execute(
                            'DELETE FROM RecentBlocks WHERE id = %s',
                            (block[0],)
                        )
                    #logger.info(f"Deleted {excess_count} oldest block(s) to maintain the limit of 1000 blocks.")
...
def detect_reorg():
    ...
    try:
        ...
        result = conn.execute(
            "SELECT value FROM systemData WHERE attribute = %s", ('lastblockscanned',)
        )
        row = result.fetchone()
        conn.close()
        ...
        while block_number > 0:
            ...
            conn = create_database_connection('latest_cache', {'db_name': 'latestCache'})
            result = conn.execute(
                "SELECT blockHash FROM RecentBlocks WHERE blockNumber = %s",
                (block_number,)
            )
...
def add_transaction_history(token_name, sourceFloAddress, destFloAddress, transferAmount, blockNumber, blockHash, blocktime, transactionHash, jsonData, transactionType, parsedFloData):
    if not token_name:
        logger.error("Missing token_name for add_transaction_history. Aborting.")
        return

    transferAmount = float(transferAmount) if transferAmount is not None else 0
    blockNumber = int(blockNumber) if blockNumber is not None else 0
    blocktime = int(blocktime) if blocktime is not None else 0
    ...
    while True:
        try:
            ...
            try:
                json_payload = filter_json_data(jsonData)
            except Exception as e:
                logger.warning(f"Failed to filter/serialize jsonData: {e}")
                json_payload = "{}"
...
def add_contract_transaction_history(contract_name, contract_address, transactionType, transactionSubType, sourceFloAddress, destFloAddress, transferAmount, blockNumber, blockHash, blocktime, transactionHash, jsonData, parsedFloData):
    if not contract_name or not contract_address:
        logger.error("Missing contract_name or contract_address. Aborting add_contract_transaction_history.")
        return
    ...
    while True:
        try:
            ...
            session.add(ContractTransactionHistory(
                transactionType=transactionType,
...
def rejected_transaction_history(transaction_data, parsed_data, sourceFloAddress, destFloAddress, rejectComment):
    ...
    tokenAmount = parsed_data.get('tokenAmount')
    transferAmount = float(tokenAmount) if tokenAmount is not None else 0

    blockNumber = int(transaction_data.get('blockheight', 0))
    blocktime = int(transaction_data.get('time', 0))
    transactionHash = transaction_data.get('txid')
    blockHash = transaction_data.get('blockhash')

    try:
        transaction_json = json.dumps(transaction_data, default=str)
    except Exception as e:
        logger.warning(f"Failed to serialize transaction_data: {e}")
        transaction_json = "{}"

    try:
        parsedFloData_json = json.dumps(parsed_data, default=str)
    except Exception as e:
        logger.warning(f"Failed to serialize parsed_data: {e}")
        parsedFloData_json = "{}"
    ...
    while True:
        try:
            ...
            session.add(RejectedTransactionHistory(
                ...
                transactionHash=transactionHash,
                blockchainReference=blockchainReference,
                jsonData=filter_json_data(transaction_json),
                rejectComment=rejectComment,
                transactionType=parsed_data.get('type'),
                parsedFloData=parsedFloData_json
            ))
...
def rejected_contract_transaction_history(transaction_data, parsed_data, transactionType, contractAddress, sourceFloAddress, destFloAddress, rejectComment):
    contractName = parsed_data.get('contractName', None)
    blockNumber = transaction_data.get('blockheight', 0)
    blockHash = transaction_data.get('blockhash', "")
    blocktime = transaction_data.get('time', 0)
    transactionHash = transaction_data.get('txid', "")

    try:
        jsonData = json.dumps(transaction_data, default=str)
    except Exception as e:
        logger.warning(f"Failed to serialize transaction_data: {e}")
        jsonData = "{}"
    ...
    while True:
        try:
            ...
            session.add(RejectedContractTransactionHistory(
                ...
                transactionHash=transactionHash,
                blockchainReference=blockchainReference,
                jsonData=filter_json_data(jsonData),
                rejectComment=rejectComment,
                parsedFloData=parsedFloData_json
            ))
...
def is_a_contract_address(floAddress):
    ...
    while True:
        try:
            ...
            # Check if any contract exists for this address
            record = session.query(
                ContractAddressMapping.contractAddress
            ).filter(
                ContractAddressMapping.contractAddress == floAddress
            ).first()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for transaction in txs:
                    ...
                    try:
                        tx_time = int(transaction.get('time', 0))
                        logger.debug(f"Transaction time: {tx_time}")
                    except (ValueError, TypeError):
                        logger.warning("Invalid time in transaction, skipping.")
                        continue

                    if tx_time >= int(blockinfo['time']):
                        logger.debug(f"Transaction time {tx_time} newer than block time {blockinfo['time']}. Skipping.")
                        continue
                    ...
                    try:
                        ...
                        if sender_address != oracle_address:
                            logger.debug("Sender mismatch. Skipping transaction.")
                            continue

                        if receiver_address != contractStructure['contractAddress']:
                            logger.debug("Receiver mismatch. Skipping transaction.")
                            continue

                        try:
                            floData_json = json.loads(floData)
                        except json.JSONDecodeError as e:
                            logger.warning(f"JSON decode error in floData: {e}. Skipping transaction.")
                            continue
...
def store_rejected_transaction(transaction_data, parsed_data, blockinfo):
    """
    Stores a transaction into the appropriate rejected history table
    if it was parsed but rejected by processTransaction.
    """
    session = None
    try:
        ...
        reject_comment = "Rejected in processTransaction"

        blockchain_reference = f"{config['DEFAULT']['NET']}/tx/{transaction_data['txid']}"
        block_hash = blockinfo.get("hash", "")
        block_number = int(blockinfo.get("height") or 0)
        ...
        # Check if it's a smart contract or token transfer
        if tx_type.startswith("smartContract") or tx_type.startswith("contract"):
            rejected_tx = RejectedContractTransactionHistory(
                transactionType = tx_type,
                transactionSubType = parsed_data.get("subtype", ""),
                contractName = parsed_data.get("contractName", ""),
                contractAddress = parsed_data.get("contractAddress", ""),
                sourceFloAddress = sourceFloAddress,
                destFloAddress = destFloAddress,
                transferAmount = transferAmount,
...
def transferToken(tokenIdentification, tokenAmount, inputAddress, outputAddress,
                  transaction_data=None, parsed_data=None,
                  isInfiniteToken=None, blockinfo=None, transactionType=None):
    ...
    # ✅ SAFETY PATCH: SKIP ZERO TRANSFERS
    if tokenAmount <= 0:
        logger.info(
            f"Skipping token transfer of zero amount from {inputAddress} to {outputAddress}."
        )

        # Optionally still record the transaction history for audit
        if transaction_data and parsed_data:
            add_transaction_history(
                token_name=tokenIdentification,
                sourceFloAddress=inputAddress,
                destFloAddress=outputAddress,
                transferAmount=tokenAmount,
                blockNumber=blockinfo['height'] if blockinfo else None,
                blockHash=blockinfo['hash'] if blockinfo else None,
                blocktime=blockinfo['time'] if blockinfo else None,
                transactionHash=transaction_data['txid'],
    ...
    # Continue normal transfer logic...
    while True:
        try:
            session = create_database_session_orm(
                'token',
                {'token_name': f"{tokenIdentification}"},
                TokenBase
            )
            break
        except:
    ...
    if isInfiniteToken == True:
        # Make new entry
        receiverAddress_details = session.query(ActiveTable).filter(
            ActiveTable.address == outputAddress,
            ActiveTable.addressBalance != None
        ).first()

        if receiverAddress_details is None:
            addressBalance = tokenAmount
        else:
            addressBalance = perform_decimal_operation(
                'addition',
                receiverAddress_details.addressBalance,
                tokenAmount
            )
            receiverAddress_details.addressBalance = None

        session.add(ActiveTable(
            address=outputAddress,
            consumedpid='1',
            transferBalance=tokenAmount,
        ...
        query_data = session.query(ActiveTable.transferBalance).filter_by(
            address=inputAddress
        ).all()

        availableTokens = float(sum(
            Decimal(f"{amount[0]}") if amount[0] is not None else Decimal(0)
            for amount in query_data
        ))

        logger.info(
            f"The sender address {inputAddress} owns {availableTokens} {tokenIdentification.upper()} tokens"
        )
        ...
        if availableTokens is None:
            logger.info(
                f"The sender address {inputAddress} doesn't own any {tokenIdentification.upper()} tokens"
            )
            session.close()
            return 0
            ...
            logger.info(
                "The transfer amount is more than the user balance. "
            ...
            logger.info(
                f"System has accepted transfer of {commentTransferAmount} "
                f"{tokenIdentification.upper()}# from {inputAddress} to {outputAddress}"
            )

            table = session.query(ActiveTable).filter(
                ActiveTable.address == inputAddress
            ).all()
            ...
            for row in table:
                ...
                checksum = perform_decimal_operation(
                    'addition',
                    checksum,
                    row.transferBalance
                )
            ...
            if checksum == commentTransferAmount:
                ...
                for piditem in pidlst:
                    entry = session.query(ActiveTable).filter(
                        ActiveTable.id == piditem[0]
                    ).all()
                    consumedpid_string += f"{piditem[0]},"
                    piddict[piditem[0]] = piditem[1]

                    session.add(TransferLogs(
                        sourceFloAddress=inputAddress,
                        destFloAddress=outputAddress,
                        transferAmount=entry[0].transferBalance,
                        sourceId=piditem[0],
                ...
                if receiverAddress_details is None:
                    ...
                    addressBalance = perform_decimal_operation(
                        'addition',
                        receiverAddress_details.addressBalance,
                        commentTransferAmount
                    )
                    receiverAddress_details.addressBalance = None

                session.add(ActiveTable(
                    address=outputAddress,
                    consumedpid=str(piddict),
                    transferBalance=commentTransferAmount,
                    addressBalance=addressBalance,
                    blockNumber=blockinfo['height']
                ))

                senderAddress_details = session.query(ActiveTable).filter_by(
                    address=inputAddress
                ).order_by(ActiveTable.id.desc()).first()

                if senderAddress_details:
                    senderAddress_details.addressBalance = perform_decimal_operation(
                        'subtraction',
                        senderAddress_details.addressBalance,
                        commentTransferAmount
                    )

                # Migration
                for piditem in pidlst:
                    entries = session.query(ActiveTable).filter(
                        ActiveTable.parentid == piditem[0]
                    ).all()
                    process_pids(entries, session, piditem)

                    entries = session.query(ConsumedTable).filter(
                        ConsumedTable.parentid == piditem[0]
                    ).all()
                    process_pids(entries, session, piditem)

                    session.execute(
                        'INSERT INTO consumedTable (id, address, parentid, consumedpid, '
                        'transferBalance, addressBalance, orphaned_parentid, blockNumber) '
                        'SELECT id, address, parentid, consumedpid, transferBalance, '
                        'addressBalance, orphaned_parentid, blockNumber '
                        'FROM activeTable WHERE id={}'.format(piditem[0])
                    )
                    session.execute(
                        'DELETE FROM activeTable WHERE id={}'.format(piditem[0])
                    )
                    session.commit()
                ...
                for idx, piditem in enumerate(pidlst):
                    ...
                    if idx != len(pidlst) - 1:
                        ...
                        remaining_transfer = perform_decimal_operation(
                            'subtraction',
                            piditem[1],
                            perform_decimal_operation(
                                'subtraction',
                                checksum,
                                commentTransferAmount
                            )
                        ...
                        session.add(TransferLogs(
                            sourceFloAddress=inputAddress,
                            destFloAddress=outputAddress,
                            transferAmount=remaining_transfer,
                            sourceId=piditem[0],
                            destinationId=lastid + 1,
                            blockNumber=blockinfo['height'],
                            time=blockinfo['time'],
                        ...
                        entry[0].transferBalance = perform_decimal_operation(
                            'subtraction',
                            checksum,
                            commentTransferAmount
                        )
...
def close_expire_contract(
    ...
    try:
        session.execute(
            text("""
                INSERT INTO activecontracts
                (id, contractName, contractAddress, status, tokenIdentification, contractType,
                 transactionHash, blockNumber, blockHash, incorporationDate, expiryDate, closeDate)
                VALUES (:id, :contractName, :contractAddress, :status, :tokenIdentification, :contractType,
                        :transactionHash, :blockNumber, :blockHash, :incorporationDate, :expiryDate, :closeDate)
            """),
            {
                'id': None,
                'contractName': contractStructure['contractName'],
        ...
        session.execute(
            text("""
                INSERT INTO time_actions
                (id, time, activity, status, contractName, contractAddress, contractType,
                 tokens_db, parsed_data, transactionHash, blockNumber)
                VALUES (:id, :time, :activity, :status, :contractName, :contractAddress,
                        :contractType, :tokens_db, :parsed_data, :transactionHash, :blockNumber)
            """),
            {
                'id': None,
                'time': trigger_time,
...
def return_time_active_deposits(session):
    """
    Finds all active contract-deposit entries where the transaction appears only once.
    """
    try:
        subquery_filter = (
            session.query(TimeActions.id)
            .group_by(TimeActions.transactionHash, TimeActions.id)
            .having(func.count(TimeActions.transactionHash) == 1)
            .subquery()
        )

        active_deposits = session.query(TimeActions).filter(
            TimeActions.id.in_(subquery_filter.select()),
            ...
            if query.activity == 'contract-time-trigger':
                ...
                try:
                    connection = create_database_connection(
                        'smart_contract',
                        {
                            'contract_name': query.contractName,
                ...
                if contract_type == 'one-time-event':
                    ...
                    try:
                        activecontracts_table_info = systemdb_session.query(
                            ActiveContracts.blockHash,
                            ActiveContracts.incorporationDate
                        ).filter(
                            ActiveContracts.contractName == query.contractName,
                            ActiveContracts.contractAddress == query.contractAddress,
                            ActiveContracts.status == 'active'
                        ).first()
                    except SQLAlchemyError as e:
                    ...
                    if activecontracts_table_info is None:
                        logger.error(
                            f"No active contract found for {query.contractName} at {query.contractAddress}"
                        )
                        continue
                    ...
                    if 'exitconditions' in contractStructure:
                        # Committee trigger contract type

                        # maximumsubscription check
                        if 'maximumsubscriptionamount' in contractStructure:
                            try:
                                rows = connection.execute(
                                    'SELECT tokenAmount FROM contractparticipants'
                                ).fetchall()
                            except Exception as e:
                                logger.error(f"Error reading contractparticipants for {query.contractName}: {e}")
                                continue

                            tokenAmount_sum = float(sum(
                                Decimal(f"{row[0]}") for row in rows if row[0] is not None
                            ))
                            ...
                            if tokenAmount_sum >= maximumsubscriptionamount:
                                logger.info(
                                    f"Maximum Subscription amount {maximumsubscriptionamount} reached for "
                                    f"{query.contractName}_{query.contractAddress}. Expiring the contract"
                                )
                                close_expire_contract(
                                    contractStructure, 'expired',
                                    transaction_data['txid'],
                                    blockinfo['height'], blockinfo['hash'],
                                    incorporationDate,
                                    blockinfo['time'],
                                    None,
                                    query.time, query.activity,
                        ...
                        if blocktime > query_time:
                            if 'minimumsubscriptionamount' in contractStructure:
                                result = process_minimum_subscriptionamount(
                                    contractStructure, connection,
                                    blockinfo, transaction_data, parsed_data
                                )
                                if result:
                                    logger.info(
                                        f"Contract trigger time {query_time} achieved and "
                                        f"Minimum subscription amount reached for "
                                        f"{query.contractName}_{query.contractAddress}. Closing the contract."
                                    )
                                    close_expire_contract(
                                        contractStructure, 'closed',
                                        transaction_data['txid'],
                                        blockinfo['height'], blockinfo['hash'],
                                        incorporationDate,
                                        blockinfo['time'],
                            ...
                            # Expire the contract
                            logger.info(
                                f"Contract trigger time {query_time} achieved for "
                                f"{query.contractName}_{query.contractAddress}. Expiring the contract"
                            )
                            close_expire_contract(
                                contractStructure, 'expired',
                        ...
                        # Internal trigger contract type
                        ...
                        if 'maximumsubscriptionamount' in contractStructure:
                            ...
                            if tokenAmount_sum >= maximumsubscriptionamount:
                                logger.info(
                                    f"Triggering {query.contractName}_{query.contractAddress} "
                                    f"as maximum subscription amount {maximumsubscriptionamount} has been reached"
                                )
                                success = trigger_internal_contract_onvalue(
                                    tokenAmount_sum, contractStructure,
                                    transaction_data, blockinfo, parsed_data,
                                    connection,
                                    contract_name=query.contractName,
                                    contract_address=query.contractAddress,
                                    transaction_subType='maximumsubscriptionamount'
                                )
                                if not success:
                                    continue

                                logger.info(
                                    f"Closing {query.contractName}_{query.contractAddress} "
                                    f"as maximum subscription amount {maximumsubscriptionamount} has been reached"
                                )
                                close_expire_contract(
                                    contractStructure, 'closed',
                        ...
                        if blocktime > query_time:
                            ...
                            logger.info(
                                f"Triggering the contract {query.contractName}_{query.contractAddress}"
                            )
                            success = trigger_internal_contract_onvalue(
                                tokenAmount_sum, contractStructure,
                                transaction_data, blockinfo, parsed_data,
                                ...
                                contract_name=query.contractName,
                                contract_address=query.contractAddress,
                                transaction_subType='expiryTime'
                            )
                            if not success:
                                continue

                            logger.info(
                                f"Closing the contract {query.contractName}_{query.contractAddress}"
                            )
                            close_expire_contract(
                                contractStructure, 'closed',
                                transaction_data['txid'],
    ...
    for query in active_deposits:
        try:
            ...
            if query.activity == 'contract-deposit' and blocktime > query_time:
                try:
                    contract_db = create_database_session_orm(
                        'smart_contract',
                        {
                            'contract_name': query.contractName,
                            'contract_address': query.contractAddress
                ...
                try:
                    deposit_last_latest_entry = contract_db.query(ContractDeposits).filter(
                        ContractDeposits.transactionHash == query.transactionHash
                    ).order_by(ContractDeposits.id.desc()).first()
                except SQLAlchemyError as e:
                ...
                if deposit_last_latest_entry is None:
                    logger.error(
                        f"No deposit entry found for transaction {query.transactionHash} in contract {query.contractName}_{query.contractAddress}"
                    )
                    continue
                ...
                try:
                    sellingToken_row = contract_db.query(
                        ContractStructure.value
                    ).filter(
                        ContractStructure.attribute == 'selling_token'
                    ).first()
                except SQLAlchemyError as e:
                ...
                if sellingToken_row is None:
                    logger.error(
                        f"Missing selling_token attribute in contract {query.contractName}_{query.contractAddress}"
                    )
                    continue
                ...
                parsed_data = {
                    'type': 'smartContractDepositReturn',
                    'contractName': query.contractName,
                    'contractAddress': query.contractAddress
                }
                ...
                logger.info(
                    f"Initiating smartContractDepositReturn after time expiry {query_time} "
                    f"for {depositorAddress} with amount {returnAmount} {sellingToken}# "
                    f"from {query.contractName}_{query.contractAddress} contract "
                )

                if float(returnAmount) <= 0:
                    logger.info(
                        f"Skipping smartContractDepositReturn for transaction {query.transactionHash} "
                        f"because returnAmount is zero ({returnAmount})."
                    )
                    try:
                        ...
                        systemdb_session.query(TimeActions).filter(
                            TimeActions.transactionHash == query.transactionHash,
                            TimeActions.contractName == query.contractName,
                            TimeActions.contractAddress == query.contractAddress,
                            TimeActions.activity == query.activity,
                            TimeActions.status == 'active'
                        ).update({
                            TimeActions.status: 'returned',
                ...
                if returnval == 0:
                    logger.critical(
                        f"Something went wrong in the token transfer method while returning contract deposit. "
                        f"THIS IS A CRITICAL ERROR for transaction {query.transactionHash}"
                    )
                    return
                else:
                    try:
                        ...
                        logger.info(
                            f"✅ Successfully processed smartContractDepositReturn transaction ID {query.transactionHash} "
                            f"after time expiry {query_time} for {depositorAddress} with amount {returnAmount} "
                            f"{sellingToken}# from {query.contractName}_{query.contractAddress} contract "
                        )
...
def checkLocal_time_expiry_trigger_deposit(blockinfo):
    """
    Check for contracts and deposits whose time triggers have expired,
    ...
    while True:
        try:
            systemdb_session = create_database_session_orm(
                'system_dbs',
                {'db_name':'system'},
                SystemBase
            )
    ...
    try:
        try:
            attributevaluepair = connection.execute(
                "SELECT attribute, value FROM contractstructure WHERE attribute != 'flodata'"
            ).fetchall()
        except Exception as e:
...
def process_flo_checks(transaction_data):
    ...
    # Check all input addresses are identical
    temp = vinlist[0][0]
...
def process_token_transfer(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    try:
        ...
        if is_a_contract_address(inputlist[0]) or is_a_contract_address(outputlist[0]):
            rejectComment = (
                f"Token transfer at transaction {txid} rejected because either the input or output "
                f"address is a contract address."
            )
        ...
        if not check_database_existence('token', {'token_name': token_name}):
            rejectComment = (
                f"Token transfer at transaction {txid} rejected because token "
                f"{token_name} does not exist."
            )
            logger.info(rejectComment)
            rejected_transaction_history(transaction_data, parsed_data, inputadd, outputlist[0], rejectComment)
            pushData_SSEapi(rejectComment)
        ...
        try:
            ...
            result = connection.execute(
                "SELECT db_name, db_type, keyword, object_format "
                "FROM databaseTypeMapping WHERE db_name = %s",
                (token_name,)
            ).fetchall()
        ...
        # Check if transaction exists in token DB
        connection = None
        try:
            connection = create_database_connection('token', {'token_name': token_name})
            rows = connection.execute(
                "SELECT blockNumber, transactionHash FROM transactionHistory"
            ).fetchall()
        ...
        try:
            returnval = transferToken(
                token_name,
                parsed_data.get('tokenAmount'),
                inputlist[0],
                outputlist[0],
                transaction_data,
                parsed_data,
                isInfiniteToken=isInfiniteToken,
                blockinfo=blockinfo
            )
            if returnval == 0:
        ...
        # Check if output address exists in mapping
        connection = None
        try:
            connection = create_database_connection('system_dbs', {'db_name': 'system'})

            output_address = outputlist[0] if isinstance(outputlist, list) else outputlist
            rows = connection.execute(
                "SELECT * FROM tokenAddressMapping "
                "WHERE tokenAddress = %s AND token = %s",
                (output_address, token_name)
            ).fetchall()

            if not rows:
                connection.execute(
                    "INSERT INTO tokenAddressMapping "
                    "(tokenAddress, token, transactionHash, blockNumber, blockHash) "
                    "VALUES (%s, %s, %s, %s, %s)",
                    (
                        output_address,
                        token_name,
                        txid,
                        transaction_data.get('blockheight'),
                        transaction_data.get('blockhash'),
                    )
                )
        except Exception as e:
            logger.error(f"Error updating tokenAddressMapping for {token_name}: {e}")
            return 0
        finally:
            if connection:
                connection.close()
...
def process_one_time_event_transfer(
    parsed_data, transaction_data, blockinfo,
    inputlist, outputlist, inputadd,
    connection, session, contractStructure
):
    logger.info(f"Processing one-time event transfer for transaction {transaction_data.get('txid')}")

    try:
        # Check if transaction already exists in participants table
        participant_rows = connection.execute(
            text("SELECT participantAddress, transactionHash FROM contractparticipants")
        ).fetchall()
        ...
        # Check if contractAddress matches outputlist
        if "contractAddress" in parsed_data:
            if parsed_data["contractAddress"] != outputlist[0]:
                rejectComment = (
                    f"Contract participation at transaction {transaction_data['txid']} rejected as contractAddress "
                    f"specified in flodata ({parsed_data['contractAddress']}) does not match transaction's output address {outputlist[0]}"
                )
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data, parsed_data,
                    'participation', outputlist[0], inputadd, outputlist[0], rejectComment
                )
                pushData_SSEapi(f"Error | {rejectComment}")
                return 0

        # Check contract status
        contractStatus = check_contract_status(
            parsed_data.get("contractName", ""), outputlist[0]
        )
        if contractStatus == "closed":
            rejectComment = (
                f"Transaction {transaction_data['txid']} rejected as Smart contract "
                f"{parsed_data.get('contractName')} at the {outputlist[0]} is closed."
            )
            logger.info(rejectComment)
            rejected_contract_transaction_history(
                transaction_data, parsed_data,
        ...
        if expirytime_result:
            ...
            parse_string = "{}/{}/{} {}".format(
                expiry_split[3],
                parsing.months[expiry_split[1]],
                expiry_split[2],
                expiry_split[4]
            )
            expiry_obj = parsing.arrow.get(
                parse_string, "YYYY/M/D HH:mm:ss"
            ).replace(tzinfo=expiry_split[5][3:])
            blocktime_obj = parsing.arrow.get(transaction_data["time"]).to("Asia/Kolkata")

            if blocktime_obj > expiry_obj:
                rejectComment = (
                    f"Transaction {transaction_data['txid']} rejected as Smart contract "
                    f"{parsed_data['contractName']}_{outputlist[0]} has expired."
                )
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data, parsed_data,
        ...
        # Check userChoice vs contract type
        if "userChoice" in parsed_data and "exitconditions" not in contractStructure:
            rejectComment = (
                f"Transaction {transaction_data['txid']} rejected as userChoice "
                f"{parsed_data['userChoice']} is not valid for Smart Contract "
                f"{parsed_data['contractName']} at address {outputlist[0]}"
            )
            logger.info(rejectComment)
            rejected_contract_transaction_history(
                transaction_data, parsed_data,
        ...
        # Check correct token is being transferred
        if parsed_data.get("tokenIdentification") != contractStructure.get("tokenIdentification"):
            rejectComment = (
                f"Transaction {transaction_data['txid']} rejected because token "
                f"{parsed_data['tokenIdentification']} does not match Smart Contract "
                f"{parsed_data['contractName']} at {outputlist[0]}"
            )
            logger.info(rejectComment)
            rejected_contract_transaction_history(
                transaction_data, parsed_data,
        ...
        if "contractAmount" in contractStructure:
            try:
                ...
                if contract_amount != token_amount:
                    rejectComment = (
                        f"Transaction {transaction_data['txid']} rejected as transferred amount "
                        f"({token_amount}) does not match contractAmount ({contract_amount})."
                    )
                    logger.info(rejectComment)
                    rejected_contract_transaction_history(
                        transaction_data, parsed_data,
        ...
        # Check maximum subscription amount
        ...
        if "maximumsubscriptionamount" in contractStructure:
            try:
                ...
                amountDeposited = sum(
                    Decimal(str(amount[0])) if amount[0] is not None else Decimal(0)
                    for amount in query_data
                )
                session.close()
            except Exception as e:
            ...
            if amountDeposited >= maximumsubscriptionamount:
                rejectComment = (
                    f"Transaction {transaction_data['txid']} rejected because max subscription "
                    f"amount has been reached for Smart Contract {parsed_data.get('contractName')} "
                    f"at {outputlist[0]}"
                )
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                ...
                pushData_SSEapi(rejectComment)
                return 0
            elif perform_decimal_operation(
                "addition",
                float(amountDeposited),
                float(parsed_data.get("tokenAmount", 0))
            ) > maximumsubscriptionamount:
                if "contractAmount" in contractStructure:
                    rejectComment = (
                        f"Transaction {transaction_data['txid']} rejected as contractAmount surpasses "
                        f"maximum subscription amount {maximumsubscriptionamount}."
                    )
                    logger.info(rejectComment)
                    rejected_contract_transaction_history(
                        transaction_data, parsed_data,
                    ...
                    rejectComment = (
                        f"Transaction {transaction_data['txid']} rejected as partial transfer of token "
                        f"{contractStructure.get('tokenIdentification')} is not allowed for "
                        f"Smart Contract {parsed_data.get('contractName')} at {outputlist[0]}"
                    )
                    logger.info(rejectComment)
                    rejected_contract_transaction_history(
                        transaction_data, parsed_data,
        ...
        if "exitconditions" in contractStructure:
            exitconditionsList = [
                contractStructure["exitconditions"][key]
                for key in contractStructure["exitconditions"]
            ]
            if parsed_data["userChoice"] in exitconditionsList:
                # do token transfer
                transfer_result = transferToken(
                    parsed_data["tokenIdentification"],
                    parsed_data["tokenAmount"],
                    inputlist[0],
                    outputlist[0],
                    transaction_data,
                    parsed_data,
                ...
                if transfer_result != 0:
                    # Record participant
                    session.add(ContractParticipants(
                        participantAddress=inputadd,
                    ...
                    # Participant mapping
                    system_session = create_database_session_orm(
                        "system_dbs", {"db_name": "system"}, SystemBase
                    )
                    system_session.add(ContractAddressMapping(
                        address=inputadd,
                    ...
                    # Check token mapping
                    connection2 = create_database_connection(
                        "system_dbs", {"db_name": "system"}
                    )
                    rows = connection2.execute(
                        text("""
                            SELECT * FROM tokenAddressMapping
                            WHERE tokenAddress=:tokenAddress
                            AND token=:token
                        """),
                        {"tokenAddress": outputlist[0],
                    ...
                    if not rows:
                        connection2.execute(
                            text("""
                                INSERT INTO tokenAddressMapping
                                (tokenAddress, token, transactionHash, blockNumber, blockHash)
                                VALUES (:tokenAddress, :token, :transactionHash, :blockNumber, :blockHash)
                            """),
                            {
                                "tokenAddress": outputlist[0],
                                "token": parsed_data["tokenIdentification"],
                    ...
                    updateLatestTransaction(
                        transaction_data,
                        parsed_data,
                        f"{parsed_data['contractName']}_{outputlist[0]}",
                        transactionType="ote-externaltrigger-participation"
                    )
                    return 1
                else:
                ...
                rejectComment = (
                    f"Transaction {transaction_data['txid']} rejected due to invalid userChoice "
                    f"for Smart Contract {parsed_data['contractName']} at {outputlist[0]}"
                )
                logger.info(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data, parsed_data,
                    'participation', outputlist[0], inputadd, outputlist[0],
                    rejectComment
                )
                pushData_SSEapi(rejectComment)
                return 0
        ...
        # Internal-trigger contracts
            ...
            if partialTransferCounter == 0:
                ...
                transferAmount = perform_decimal_operation(
                    "subtraction",
                    maximumsubscriptionamount,
                    float(amountDeposited)
                )
            ...
            if transfer_result != 0:
                ...
                updateLatestTransaction(
                    ...
                    parsed_data,
                    f"{parsed_data['contractName']}_{outputlist[0]}",
                    transactionType="ote-internaltrigger-participation"
                )
                return 1
            else:
...
def process_continuous_event_transfer(
    parsed_data,
    transaction_data,
    blockinfo,
    inputlist,
    ...
    try:
        ...
        # --- Ensure input/output amounts are Decimal
        if len(inputlist) > 1:
            inputlist_amount = Decimal(str(inputlist[1]))
        else:
            inputlist_amount = Decimal('0')

        if len(outputlist) > 1:
            outputlist_amount = Decimal(str(outputlist[1]))
        else:
            outputlist_amount = Decimal('0')

        # --- Determine contract subtype
        contract_subtype = contract_session.query(
            ContractStructure.value
        ).filter(
            ContractStructure.attribute == 'subtype'
        ).first()
        if contract_subtype:
            contract_subtype = contract_subtype[0]
        else:
        ...
        if contract_subtype == 'tokenswap':
            ...
            if 'contractAddress' in parsed_data:
                if parsed_data['contractAddress'] != outputlist[0]:
                    rejectComment = (
                        f"Contract participation at transaction {transaction_data['txid']} rejected as "
                        f"contractAddress specified in flodata {parsed_data['contractAddress']} "
                        f"doesn't match output address {outputlist[0]}"
                    )
                    logger.info(rejectComment)
                    rejected_contract_transaction_history(
                        transaction_data, parsed_data, 'participation',
                        outputlist[0], inputadd, outputlist[0], rejectComment
                    )
                    pushData_SSEapi(
                        f"Error | Mismatch in contract address for transaction {transaction_data['txid']}"
                    )
                    return 0
            ...
            if pricetype in ['predetermined', 'determined']:
                ...
                if transaction_data['senderAddress'] == contractStructure['oracle_address']:
                    logger.warning(
                        f"Transaction {transaction_data['txid']} rejected: "
                        f"oracle address {contractStructure['oracle_address']} trying to participate."
                    )
                    pushData_SSEapi(
                        f"Transaction {transaction_data['txid']} rejected: oracle address trying to participate."
                    )
                    return 0

                swapPrice = Decimal(fetchDynamicSwapPrice(contractStructure, blockinfo))

                # Save dynamic price snapshot
                update_dynamic_swap_price(
                    session=contract_session,
                    price=swapPrice,
                    price_timestamp=blockinfo['time'],
                    blockheight=blockinfo['height']
                )
            else:
            ...
            # --- Compute swap amount
            swapAmount = perform_decimal_operation(
                'division',
                Decimal(parsed_data['tokenAmount']),
                swapPrice
            )
            logger.info(f"Calculated swapAmount = {swapAmount}")

            # --- Check available deposits
            subquery = contract_session.query(
                func.max(ContractDeposits.id)
            ).group_by(ContractDeposits.transactionHash)

            active_contract_deposits = contract_session.query(
                ContractDeposits
            ).filter(
                ContractDeposits.id.in_(subquery),
                ContractDeposits.status.notin_(['deposit-return', 'consumed']),
                ContractDeposits.status == 'active'
            ).order_by(ContractDeposits.id.asc()).all()

            query_data = contract_session.query(
                ContractDeposits.depositBalance
            ).filter(
                ContractDeposits.id.in_(subquery),
                ContractDeposits.status.notin_(['deposit-return']),
                ContractDeposits.status == 'active'
            ).all()

            available_deposit_sum = sum(
                Decimal(str(amount[0])) if amount[0] is not None else Decimal(0)
                for amount in query_data
            )
            logger.info(f"Available deposit sum = {available_deposit_sum}")

            # --- Check if enough deposit exists
            if available_deposit_sum < Decimal('0.00000001'):
                ...
                rejected_contract_transaction_history(
                    ...
                    parsed_data,
                    parsed_data["type"],
                    contractStructure["contractAddress"],
                    inputlist[0],
                    outputlist[0],
                    rejectComment
                )
            ...
            usd_required = perform_decimal_operation(
                'multiplication',
                fulfilled_swapAmount,
                swapPrice
            )

            refund_usd_amount = perform_decimal_operation(
                'subtraction',
                Decimal(parsed_data['tokenAmount']),
                usd_required
            ) if swapAmount > available_deposit_sum else Decimal(0)

            logger.info(f"Will fulfill {fulfilled_swapAmount} selling tokens for "
                        f"{usd_required} accepting tokens. Refund required: {refund_usd_amount}")

            # Transfer accepting_token from participant to contract
            returnval = transferToken(
                parsed_data['tokenIdentification'],
                Decimal(parsed_data['tokenAmount']),
                inputlist[0],
                ...
                transaction_data=transaction_data,
                parsed_data=parsed_data,
                isInfiniteToken=None,
                blockinfo=blockinfo,
                transactionType='tokenswapParticipation'
            )
            if returnval == 0:
            ...
            if refund_usd_amount > Decimal(0):
                logger.info(
                    f"Refunding {refund_usd_amount} {parsed_data['tokenIdentification']}# "
                    f"to participant {inputlist[0]}"
                )
                returnval = transferToken(
                    parsed_data['tokenIdentification'],
                    refund_usd_amount,
            ...
            # Distribute accepting_token to depositors
            ...
            for a_deposit in active_contract_deposits:
                ...
                usd_for_depositor = perform_decimal_operation(
                    'multiplication',
                    used_amount,
                    swapPrice
                )
                ...
                returnval = transferToken(
                    contractStructure['accepting_token'],
                    usd_for_depositor,
                    contractStructure['contractAddress'],
                    a_deposit.depositorAddress,
                    transaction_data=transaction_data,
                    parsed_data=parsed_data,
                    isInfiniteToken=None,
                    blockinfo=blockinfo,
                ...
                new_deposit_balance = perform_decimal_operation(
                    'subtraction',
                    deposit_balance_decimal,
                    used_amount
                )
                ...
                if new_deposit_balance == Decimal('0'):
                    ...
                    try:
                        systemdb_session.query(TimeActions).filter(
                            TimeActions.transactionHash == a_deposit.transactionHash,
                            TimeActions.contractName == contractStructure['contractName'],
                            TimeActions.contractAddress == contractStructure['contractAddress'],
                            TimeActions.activity == 'contract-deposit',
                            TimeActions.status == 'active'
                        ).update({
                            TimeActions.status: 'consumed',
                            TimeActions.blockNumber: blockinfo['height']
                        })
                        systemdb_session.commit()
                        logger.info(
                            f"Marked TimeAction as consumed for fully consumed deposit {a_deposit.transactionHash}"
                        )
                    finally:
                        systemdb_session.close()

                contract_session.add(ContractDeposits(
                    depositorAddress=a_deposit.depositorAddress,
                    depositAmount=-used_amount,
                    depositBalance=new_deposit_balance,
                    expiryTime=a_deposit.expiryTime,
                    unix_expiryTime=a_deposit.unix_expiryTime,
                    status='consumed' if new_deposit_balance == Decimal('0') else 'active',
                    transactionHash=a_deposit.transactionHash,
                    blockNumber=blockinfo['height'],
                    blockHash=blockinfo['hash']
                ))

                remaining_swapAmount = perform_decimal_operation(
                    'subtraction',
                    remaining_swapAmount,
                    used_amount
                )

            # Send selling_token to participant
            returnval = transferToken(
                contractStructure['selling_token'],
                fulfilled_swapAmount,
                outputlist[0],
                inputlist[0],
                transaction_data=transaction_data,
            ...
            # Record ContractParticipants
            contract_session.add(ContractParticipants(
                participantAddress=transaction_data['senderAddress'],
                tokenAmount=Decimal(parsed_data['tokenAmount']),
                userChoice=swapPrice,
                transactionHash=transaction_data['txid'],
                blockNumber=blockinfo['height'],
                blockHash=blockinfo['hash'],
                winningAmount=fulfilled_swapAmount
            ))
            ...
            # Save tokenAddressMapping for participant
            systemdb_connection = create_database_connection('system_dbs', {'db_name': 'system'})
            try:
                firstInteractionCheck = systemdb_connection.execute(
                    text("""
                        SELECT * FROM tokenAddressMapping
                        WHERE tokenAddress = :tokenAddress AND token = :token
                    """),
                    {
                        "tokenAddress": inputlist[0],
                        "token": contractStructure["selling_token"]
                    }
                ).fetchall()

                if len(firstInteractionCheck) == 0:
                    systemdb_connection.execute(
                        text("""
                            INSERT INTO tokenAddressMapping
                            (tokenAddress, token, transactionHash, blockNumber, blockHash)
                            VALUES (:tokenAddress, :token, :transactionHash, :blockNumber, :blockHash)
            ...
            pushData_SSEapi(
                f"Token swap successfully performed at contract {parsed_data['contractName']}_{outputlist[0]} "
                f"with transaction {transaction_data['txid']}"
            )
...
def process_smart_contract_transfer(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    try:
        ...
        if not contract_name or not contract_address:
            rejectComment = (
                f"Smart contract transfer at transaction {transaction_data['txid']} rejected "
                f"due to missing contractName or contractAddress"
            )
            logger.info(rejectComment)
            rejected_transaction_history(
                transaction_data, parsed_data, inputlist[0], contract_address or '-', rejectComment
            )
            pushData_SSEapi(rejectComment)
            return 0
        ...
        if check_database_existence(
            ...
            try:
                ...
                contract_session = create_database_session_orm(
                    'smart_contract',
                    {'contract_name': contract_name, 'contract_address': contract_address},
                    ContractBase
                )
                ...
                result = contract_session.query(
                    ContractStructure.value
                ).filter(
                    ContractStructure.attribute == 'contractType'
                ).first()
                ...
                if not contract_type:
                    rejectComment = (
                        f"Smart contract transfer at transaction {transaction_data['txid']} rejected "
                        f"due to missing contract type in contract structure"
                    )
                    logger.info(rejectComment)
                    rejected_transaction_history(
                        transaction_data, parsed_data, inputlist[0], contract_address, rejectComment
                    )
                    pushData_SSEapi(rejectComment)
                    return 0
                ...
                if contract_type == 'one-time-event':
                    return process_one_time_event_transfer(
                        parsed_data, transaction_data, blockinfo,
                        inputlist, outputlist, inputadd,
                        connection, contract_session, contractStructure
                    )
                elif contract_type == 'continuous-event':
                    return process_continuous_event_transfer(
                        parsed_data, transaction_data, blockinfo,
                        inputlist, outputlist, inputadd,
                        connection, contract_session, contractStructure
                    )
                    ...
                    rejectComment = (
                        f"Smart contract transfer at transaction {transaction_data['txid']} rejected "
                        f"due to unknown contract type '{contract_type}'"
                    )
                    logger.info(rejectComment)
                    rejected_transaction_history(
                        transaction_data, parsed_data, inputlist[0], contract_address, rejectComment
                ...
                logger.error(traceback.format_exc())
                rejectComment = (
                    f"Smart contract transfer at transaction {transaction_data['txid']} rejected due to DB error"
                )
                rejected_transaction_history(
                    transaction_data, parsed_data, inputlist[0], contract_address, rejectComment
                )
            ...
            rejectComment = (
                f"Smart contract transfer at transaction {transaction_data['txid']} rejected "
                f"as the smart contract does not exist"
            )
            logger.info(rejectComment)
            rejected_transaction_history(
                transaction_data, parsed_data, inputlist[0], contract_address, rejectComment
        ...
        logger.error(traceback.format_exc())
        rejectComment = (
            f"Smart contract transfer at transaction {transaction_data['txid']} failed due to system error"
        )
        rejected_transaction_history(
            transaction_data, parsed_data, inputlist[0],
            outputlist[0] if outputlist else '-', rejectComment
        )
        pushData_SSEapi(rejectComment)
        return 0
...
def process_token_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if is_a_contract_address(inputlist[0]):
        rejectComment = (
            f"Token incorporation at transaction {transaction_data['txid']} "
            f"rejected as either the input address is part of a contract address"
        )
        logger.info(rejectComment)
        rejected_transaction_history(
            transaction_data,
    ...
    # Proceed if token does not already exist
    if check_database_existence(
        'token',
        {'token_name': f"{parsed_data['tokenIdentification']}"}
    ):
        rejectComment = (
            f"Token incorporation rejected at transaction {transaction_data['txid']} "
            f"as token {parsed_data['tokenIdentification']} already exists"
        )
        logger.info(rejectComment)
        rejected_transaction_history(
            transaction_data,
    ...
    try:
        ...
        session.add(
            ActiveTable(
                ...
                addressBalance=parsed_data['tokenAmount'],
                blockNumber=blockinfo['height']
            )
        )
        ...
        add_transaction_history(
            ...
            blockNumber=transaction_data['blockheight'],
            blockHash=transaction_data['blockhash'],
            blocktime=transaction_data['time'],
            transactionHash=transaction_data['txid'],
            jsonData=transaction_data_json,
            transactionType=parsed_data['type'],
            parsedFloData=parsed_data_json
        )
        ...
        logger.error(f"Error during token incorporation DB insert: {e}", exc_info=True)
        return 0
    ...
    try:
        ...
        connection.execute(
            ...
            (
                ...
                parsed_data['tokenIdentification'],
                transaction_data['txid'],
                transaction_data['blockheight'],
                transaction_data['blockhash']
            )
        )

        # Insert databaseTypeMapping
        connection.execute(
            """
            INSERT INTO databaseTypeMapping
                (db_name, db_type, keyword, object_format, blockNumber)
            VALUES
                (%s, %s, %s, %s, %s)
            """,
            (
        ...
        logger.error(f"Error during token incorporation system DB insert: {e}", exc_info=True)
        return 0
    finally:
        connection.close()

    # Update the latest transaction table
    updateLatestTransaction(
        transaction_data,
        parsed_data,
    ...
    logger.info(
        f"Token | Successfully incorporated token {parsed_data['tokenIdentification']} "
        f"at transaction {transaction_data['txid']}"
    )
    pushData_SSEapi(
        f"Token | Successfully incorporated token {parsed_data['tokenIdentification']} "
        f"at transaction {transaction_data['txid']}"
...
def process_smart_contract_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if parsed_data.get("contractType") in ["continuous-event", "continuos-event"]:
        parsed_data["contractType"] = "continuous-event"

    try:
        exists = check_database_existence(
            'smart_contract',
            {'contract_name': f"{parsed_data['contractName']}",
             'contract_address': f"{parsed_data['contractAddress']}"}
        )
        ...
        if not exists:
            ...
            try:
                ...
                tokenAddressMapping_of_contractAddress = systemdb_session.query(
                    TokenAddressMapping
                ).filter(
                    TokenAddressMapping.tokenAddress == parsed_data['contractAddress']
                ).all()

                if len(tokenAddressMapping_of_contractAddress) == 0:
                    if parsed_data.get('contractType') == 'one-time-event':
                        logger.info("Smart contract is of the type one-time-event")

                        contract_conditions = parsed_data.get('contractConditions', {})

                        # Check either userchoices or payeeAddress is present
                        if 'userchoices' not in contract_conditions and 'payeeAddress' not in contract_conditions:
                            ...
                            rejected_contract_transaction_history(
                                transaction_data, parsed_data,
                                'incorporation', inputadd, inputadd, outputlist[0], rejectComment
                            )
                            delete_contract_database({
                                'contract_name': parsed_data['contractName'],
                                'contract_address': parsed_data['contractAddress']
                            })
                            return 0

                        if 'userchoices' in contract_conditions and 'payeeAddress' in contract_conditions:
                        ...
                        if parsed_data['contractAddress'] == inputadd:
                            ...
                            try:
                                ...
                                session.add(ContractStructure(
                                    attribute='contractType', index=0,
                                    value=parsed_data['contractType'], blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='subtype', index=0,
                                    value=parsed_data.get('subtype', ''),
                                    blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='contractName', index=0,
                                    value=parsed_data['contractName'], blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='tokenIdentification', index=0,
                                    value=parsed_data['tokenIdentification'], blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='contractAddress', index=0,
                                    value=parsed_data['contractAddress'], blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='flodata', index=0,
                                    value=parsed_data['flodata'], blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='expiryTime', index=0,
                                    value=contract_conditions.get('expiryTime', ''),
                                    blockNumber=blockinfo['height']
                                ))
                                session.add(ContractStructure(
                                    attribute='unix_expiryTime', index=0,
                                    value=contract_conditions.get('unix_expiryTime', ''),
                                    blockNumber=blockinfo['height']
                                ))

                                if 'contractAmount' in contract_conditions:
                                    session.add(ContractStructure(
                                        attribute='contractAmount', index=0,
                                        value=contract_conditions['contractAmount'],
                                        blockNumber=blockinfo['height']
                                    ))

                                if 'minimumsubscriptionamount' in contract_conditions:
                                    session.add(ContractStructure(
                                        attribute='minimumsubscriptionamount', index=0,
                                        value=contract_conditions['minimumsubscriptionamount'],
                                        blockNumber=blockinfo['height']
                                    ))

                                if 'maximumsubscriptionamount' in contract_conditions:
                                    session.add(ContractStructure(
                                        attribute='maximumsubscriptionamount', index=0,
                                        value=contract_conditions['maximumsubscriptionamount'],
                                        blockNumber=blockinfo['height']
                                    ))

                                if 'userchoices' in contract_conditions:
                                    for key, value in literal_eval(contract_conditions['userchoices']).items():
                                        session.add(ContractStructure(
                                            attribute='exitconditions',
                                            index=key,
                                            value=value,
                                            blockNumber=blockinfo['height']
                                        ))

                                if 'payeeAddress' in contract_conditions:
                                    session.add(ContractStructure(
                                        attribute='payeeAddress', index=0,
                                        value=json.dumps(contract_conditions['payeeAddress']),
                                        blockNumber=blockinfo['height']
                                    ))
                                ...
                                logger.error(f"Error during smart contract incorporation (one-time-event): {e}")
                                if session:
                                    session.rollback()
                                raise
                            finally:
                            ...
                            try:
                                # Add contract to token contract association
                                ...
                                session.add(TokenContractAssociation(
                                ...
                                logger.error(f"Error writing to TokenContractAssociation: {e}")
                                if session:
                                    session.rollback()
                                raise
                            finally:
                            ...
                            try:
                                ...
                                session.add(ContractAddressMapping(
                                    address=inputadd,
                                    addressType='incorporation',
                                    tokenAmount=None,
                                    contractName=parsed_data['contractName'],
                                ...
                                session.add(DatabaseTypeMapping(
                                    db_name=f"{parsed_data['contractName']}_{inputadd}",
                                ...
                                session.add(TimeActions(
                                    time=contract_conditions.get('expiryTime', ''),
                                ...
                                logger.error(f"Error updating system DB tables for contract incorporation: {e}")
                                if session:
                                    session.rollback()
                                raise
                            finally:
                            ...
                            updateLatestTransaction(
                                transaction_data,
                                parsed_data,
                                f"{parsed_data['contractName']}-{parsed_data['contractAddress']}"
                            )

                            pushData_SSEapi(
                                f"Contract | Contract incorporated at transaction {transaction_data['txid']} with name {parsed_data['contractName']}-{parsed_data['contractAddress']}"
                            )
                            return 1
                        else:
                            ...
                            logger.info(rejectComment)
                            rejected_contract_transaction_history(
                                transaction_data, parsed_data, 'incorporation',
                                inputadd, inputadd, outputlist[0], rejectComment
                            )
                            pushData_SSEapi(
                                f"Error | Contract Incorporation rejected as address in Flodata and input address are different at transaction {transaction_data['txid']}"
                            )
                            delete_contract_database({
                                'contract_name': parsed_data['contractName'],
                                'contract_address': parsed_data['contractAddress']
                    ...
                    # Continuous-event processing remains unchanged (patch it the same way if used)
        ...
        logger.critical(f"Unhandled exception in process_smart_contract_incorporation: {e}", exc_info=True)
        delete_contract_database({
            'contract_name': parsed_data['contractName'],
            'contract_address': parsed_data['contractAddress']
        })
...
def process_smart_contract_pays(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if not check_database_existence(
        'smart_contract',
        {'contract_name': parsed_data['contractName'], 'contract_address': outputlist[0]}
    ):
    ...
    try:
        ...
        participantAdd_txhash = connection.execute(
            "SELECT sourceFloAddress, transactionHash FROM contractTransactionHistory WHERE transactionType != 'incorporation'"
        ).fetchall()
        ...
        if participantAdd_txhash_T and transaction_data['txid'] in participantAdd_txhash_T[1]:
            logger.warning(f"Transaction {transaction_data['txid']} rejected as it already exists in the Smart Contract db.")
            pushData_SSEapi(f"Error | Transaction {transaction_data['txid']} rejected as it already exists in the Smart Contract db.")
            return 0
    ...
    try:
        ...
        activecontracts_table_info = systemdb_session.query(
            ActiveContracts.blockHash,
            ActiveContracts.incorporationDate,
            ActiveContracts.expiryDate
        ).filter(
            ActiveContracts.contractName == parsed_data['contractName'],
            ActiveContracts.contractAddress == outputlist[0],
            ActiveContracts.status == 'expired'
        ).first()

        timeactions_table_info = systemdb_session.query(
            TimeActions.time,
            TimeActions.activity,
            TimeActions.contractType,
            TimeActions.tokens_db,
            TimeActions.parsed_data
        ).filter(
            TimeActions.contractName == parsed_data['contractName'],
            TimeActions.contractAddress == outputlist[0],
            TimeActions.status == 'active'
        ).first()
    finally:
        if systemdb_session:
    ...
    if 'minimumsubscriptionamount' in contractStructure:
        ...
        if amountDeposited < minimumsubscriptionamount:
            ...
            try:
                ...
                for participant in contractParticipants:
                    ...
                    row2 = connection.execute(
                        'SELECT value FROM contractstructure WHERE attribute="contractAddress"'
                    ).fetchone()
                    ...
                    if returnval == 0:
                        logger.error("CRITICAL ERROR during refund in smart contract minimum subscription check.")
                        return 0

                    connection.execute(
                        f"UPDATE contractparticipants SET winningAmount={tokenAmount} WHERE participantAddress=:addr AND transactionHash=:txid",
                        {"addr": participantAddress, "txid": txHash}
                    )
            ...
            pushData_SSEapi(
                f"Trigger | Minimum subscription amount not reached at contract {parsed_data['contractName']}_{outputlist[0]} at transaction {transaction_data['txid']}. Tokens will be refunded."
            )
            return 1

    # Otherwise, proceed to trigger the contract
    connection = None
    try:
        connection = create_database_connection(
            'smart_contract',
        ...
        if tokenSum > 0:
            contractWinners = connection.execute(
                "SELECT * FROM contractparticipants WHERE userChoice = :choice",
                {"choice": parsed_data["triggerCondition"]}
            ).fetchall()

            winnerRows = connection.execute(
                "SELECT tokenAmount FROM contractparticipants WHERE userChoice = :choice",
                {"choice": parsed_data["triggerCondition"]}
            ).fetchall()
            ...
            for winner in contractWinners:
                winnerAmount = "%.8f" % perform_decimal_operation(
                    'multiplication',
                    perform_decimal_operation('division', winner[2], winnerSum),
                    tokenSum
                )
                returnval = transferToken(
                    tokenIdentification,
                ...
                if returnval == 0:
                    logger.critical("Critical error during contract trigger token transfer.")
                    return 0

                connection.execute(
                    """
                    INSERT INTO contractwinners
                    (participantAddress, winningAmount, userChoice, transactionHash, blockNumber, blockHash, referenceTxHash)
                    VALUES (:addr, :amount, :choice, :txid, :blocknum, :blockhash, :refTx)
                    """,
                    {
                        "addr": winner[1],
                        "amount": winnerAmount,
    ...
    close_expire_contract(
        ...
        timeActionTime,
        timeActionActivity,
        contractStructure['contractName'],
        contractStructure['contractAddress'],
        contractType,
        tokens_db,
        parsed_data_ta,
    ...
    pushData_SSEapi(
        f"Trigger | Contract triggered of the name {parsed_data['contractName']}_{outputlist[0]} is active currently at transaction {transaction_data['txid']}"
    )
    return 1
...
def process_smart_contract_deposit(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    try:
        if check_database_existence('smart_contract', {
            'contract_name': f"{parsed_data['contractName']}",
            'contract_address': f"{outputlist[0]}"
        }):

            # Reject if the deposit expiry time is before current block time
            expiry_time = convert_datetime_to_arrowobject(parsed_data['depositConditions']['expiryTime'])
            if not hasattr(expiry_time, "timestamp"):
                logger.error(f"Expiry time object has no attribute 'timestamp'. Rejecting transaction {transaction_data['txid']}")
                return 0

            if blockinfo['time'] > expiry_time.timestamp:
                rejectComment = (
                    f"Contract deposit of transaction {transaction_data['txid']} "
                    f"rejected as expiryTime before current block time"
                )
                logger.warning(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data,
            ...
            # Check if transaction already exists in the contract DB
            connection = create_database_connection(
                'smart_contract',
                {'contract_name': parsed_data['contractName'], 'contract_address': outputlist[0]}
            )
            try:
                participantAdd_txhash = connection.execute(
                    'SELECT participantAddress, transactionHash FROM contractparticipants'
                ).fetchall()
            finally:
                connection.close()
            ...
            if participantAdd_txhash_T and transaction_data['txid'] in participantAdd_txhash_T[1]:
                rejectComment = (
                    f"Contract deposit at transaction {transaction_data['txid']} rejected as it already exists "
                    f"in the Smart Contract db. This is unusual, please check your code"
                )
                logger.warning(rejectComment)
                rejected_contract_transaction_history(
                    transaction_data,
            ...
            # Check if contractAddress matches output address
            if 'contractAddress' in parsed_data:
                if parsed_data['contractAddress'] != outputlist[0]:
                    rejectComment = (
                        f"Contract deposit at transaction {transaction_data['txid']} rejected as contractAddress "
                        f"specified in flodata, {parsed_data['contractAddress']}, does not match "
                        f"transaction output address {outputlist[0]}"
                    )
                    logger.info(rejectComment)
                    rejected_contract_transaction_history(
                        transaction_data,
                    ...
                    pushData_SSEapi(
                        f"Error| Mismatch in contract address specified in flodata and transaction output address "
                        f"in {transaction_data['txid']}"
                    )
                    return 0

            # Pull out the contract structure
            contractStructure = extract_contractStructure(
                parsed_data['contractName'],
                outputlist[0]
            )

            # Transfer the token from depositor to contract
            logger.info(
                f"Initiating transfers for smartcontract deposit with transaction ID {transaction_data['txid']}"
            )
            returnval = transferToken(
                parsed_data['tokenIdentification'],
                parsed_data['depositAmount'],
                inputlist[0],
                outputlist[0],
                transaction_data,
                parsed_data,
            ...
            if returnval == 0:
                logger.info("Something went wrong in the token transfer method")
                pushData_SSEapi(
                    f"Error | Something went wrong while doing internal db transactions for {transaction_data['txid']}"
                )
                return 0

            # Save deposit in contract DB
            session = create_database_session_orm(
                'smart_contract',
                {'contract_name': parsed_data['contractName'], 'contract_address': outputlist[0]},
                ContractBase
            ...
            try:
                blockchainReference = neturl + 'tx/' + transaction_data['txid']
                session.add(ContractDeposits(
                    depositorAddress=inputadd,
                    depositAmount=parsed_data['depositAmount'],
                    depositBalance=parsed_data['depositAmount'],
                    expiryTime=parsed_data['depositConditions']['expiryTime'],
                    unix_expiryTime=expiry_time.timestamp,
                    status='active',
                    transactionHash=transaction_data['txid'],
                    blockNumber=transaction_data['blockheight'],
                    blockHash=transaction_data['blockhash']
                ...
                session.add(ContractTransactionHistory(
                    transactionType='smartContractDeposit',
                    transactionSubType=None,
                    sourceFloAddress=inputadd,
                    destFloAddress=outputlist[0],
                ...
                session.rollback()
                logger.error(f"Error while saving contract deposit: {e}")
                return 0
            finally:
                session.close()

            # Create refund time trigger in system DB
            systemdb_session = create_database_session_orm(
                'system_dbs',
                {'db_name': 'system'},
                SystemBase
            ...
            try:
                systemdb_session.add(TimeActions(
                    time=parsed_data['depositConditions']['expiryTime'],
                    activity='contract-deposit',
                    status='active',
                    contractName=parsed_data['contractName'],
                    contractAddress=outputlist[0],
                    contractType='continuous-event-swap',
                    tokens_db=json.dumps([parsed_data['tokenIdentification']]),
                    parsed_data=json.dumps(parsed_data),
                    transactionHash=transaction_data['txid'],
                    blockNumber=transaction_data['blockheight']
                ...
                systemdb_session.rollback()
                logger.error(f"Failed inserting into TimeActions: {e}")
                return 0
            finally:
                systemdb_session.close()

            # Add token mapping if new
            systemdb_connection = create_database_connection('system_dbs', {'db_name': 'system'})
            try:
                firstInteractionCheck = systemdb_connection.execute(
                    "SELECT * FROM tokenAddressMapping WHERE tokenAddress = :addr AND token = :token",
                    {'addr': outputlist[0], 'token': parsed_data['tokenIdentification']}
                ).fetchall()

                if not firstInteractionCheck:
                    systemdb_connection.execute(
                        "INSERT INTO tokenAddressMapping "
                        "(tokenAddress, token, transactionHash, blockNumber, blockHash) "
                        "VALUES (:address, :token, :txid, :blocknumber, :blockhash)",
                        {
            ...
            pushData_SSEapi(
                f"Deposit Smart Contract Transaction {transaction_data['txid']} for smart contract "
                f"{parsed_data['contractName']} at address {outputlist[0]}"
            )
            ...
            rejectComment = (
                f"Deposit Transaction {transaction_data['txid']} rejected because Smart Contract "
                f"{parsed_data['contractName']} at address {outputlist[0]} does not exist"
            )
            logger.info(rejectComment)
            rejected_contract_transaction_history(
                transaction_data,
        ...
        logger.exception(f"Exception occurred while processing smart contract deposit for tx {transaction_data['txid']}: {e}")
        return 0
...
def process_nft_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    """
    Incorporates an NFT into the system.
    """
    ...
    try:
        ...
        if 'nftHash' not in parsed_data:
            logger.warning(f"Transaction {transaction_data['txid']} rejected as nftHash is missing.")
            rejectComment = f"NFT incorporation rejected because nftHash is missing in flodata for transaction {transaction_data['txid']}"
            rejected_transaction_history(transaction_data, parsed_data, inputadd, outputlist[0], rejectComment)
            pushData_SSEapi(rejectComment)
            return 0
        ...
        try:
            ...
            session.rollback()
            logger.error(f"Error while incorporating NFT in token db: {e}")
            return 0
        finally:
            session.close()

        # Add token mapping in system db
        connection = create_database_connection('system_dbs', {'db_name': 'system'})
        try:
            connection.execute(
                """
                INSERT INTO tokenAddressMapping 
                (tokenAddress, token, transactionHash, blockNumber, blockHash)
                VALUES (:addr, :token, :txid, :blocknumber, :blockhash)
                """,
                {
                    'addr': inputadd,
                    'token': parsed_data['tokenIdentification'],
            ...
            connection.execute(
                ...
                INSERT INTO databaseTypeMapping
                (db_name, db_type, keyword, object_format, blockNumber)
                VALUES (:dbname, :dbtype, '', :object_format, :blocknumber)
                """,
                {
                    'dbname': parsed_data['tokenIdentification'],
                    'dbtype': 'nft',
                    'object_format': json.dumps(nft_data),
                    'blocknumber': transaction_data['blockheight']
                }
            )
        except Exception as e:
            logger.error(f"Error while inserting NFT into system db: {e}")
            return 0
        finally:
            connection.close()
        ...
        logger.exception(f"Unexpected error in NFT incorporation for tx {transaction_data['txid']}: {e}")
        return 0
...
def process_infinite_token_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    try:
        ...
        connection.execute(
            ...
            {
                'dbname': parsed_data['tokenIdentification'],
                'dbtype': 'infinite-token',
                'object_format': json.dumps(info_object),
                'blocknumber': transaction_data['blockheight']
            }
        )

        # Commit both only when all succeed
