...
RUN_FULL_UTXO_CLEANUP_ON_STARTUP = False
...
@address_indexer_app.after_request
...
@address_indexer_app.route("/<path:path>", methods=["OPTIONS"])
...
async def initialize_database():
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            # Check if utxos.block_height index exists
            await cursor.execute("""
                SELECT COUNT(1)
                FROM information_schema.STATISTICS
                WHERE TABLE_SCHEMA = %s AND TABLE_NAME = 'utxos' AND INDEX_NAME = 'idx_utxos_blockheight';
            """, (ADDRESS_INDEXER_DB_NAME,))
            index_exists = await cursor.fetchone()
            if index_exists[0] == 0:
                await cursor.execute("CREATE INDEX idx_utxos_blockheight ON utxos (block_height);")
...
async def process_address_externalsource(flo_address):
    ...
    while current_index < len(ADDRESS_INDEXER_URLS):
        ...
        try:
            ...
            stored_txids = {row[0] for row in rows}
...
async def store_transaction_data(transaction_data):
    ...
    """
    # Pure Python and network logic first
    vin_addresses = []
    spent_utxos = []
    for vin in transaction_data.get("vin", []):
        if "addresses" in vin:
            vin_addresses.extend(vin["addresses"])
    ...
    for index, vout in enumerate(transaction_data.get("vout", [])):
        if "scriptPubKey" in vout and "addresses" in vout["scriptPubKey"]:
            ...
            for address in addresses:
                new_utxos.append((transaction_data["txid"], index, address, vout["value"], int(value), None))
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            for utxo in new_utxos:
                utxo_with_blockheight = (*utxo[:-1], blockheight)
...
async def process_blocks(start_block, end_block, batch_size=200, max_concurrent_batches=5, retries=5):
    ...
    async def process_transaction(cursor, tx, blockhash, block_height, block_time):
            ...
            try:
                if "txid" in vin and "vout" in vin:
                    ...
                    if "result" in prev_tx_response:
                        ...
                        vin_addresses.extend(addresses)

                        vin_data.append((prev_txid, vout_index))
            except Exception as e:
        ...
        # DELETE spent UTXOs
        for prev_txid, vout_index in vin_data:
            await cursor.execute(
                """
                DELETE FROM utxos
                WHERE txid = %s AND vout = %s
        ...
        for utxo_row in vout_data:
            ...
            logger.info(f"Inserted new UTXO: address={utxo_row[2]}, txid={txid}, vout={utxo_row[1]}")
...
async def process_all_transactions_for_coinbase(flo_address, results, page=1, page_size=1000, from_scratch=False):
    ...
    filtered_transactions = [
        ...
        if from_scratch or tx["blockheight"] > last_computed_blockheight
    ]
    ...
    for transaction in filtered_transactions:
        try:
            ...
            if not raw_transaction.get("vin"):
                ...
                # Fetch block data from external API
                if blockhash not in processed_blocks:
                    ...
                    for index, vout in enumerate(coinbase_tx.get("vout", [])):
                        addresses = vout.get("scriptPubKey", {}).get("addresses", [])
                        value = Decimal(vout.get("value", 0))
                        total_output_value = sum(Decimal(v["value"]) for v in coinbase_tx.get("vout", []))
                        fee_share = total_transaction_fees * (value / total_output_value) if total_output_value else Decimal(0)
    ...
    async with conn.cursor() as cursor:
        ...
        await cursor.execute(store_balance_query, (
            flo_address,
            int(balance * Decimal(1e8)),
            int(total_received * Decimal(1e8)),
            int(total_sent * Decimal(1e8)),
...
async def calculate_balance_and_update(conn, flo_address, from_scratch=False):
    ...
    try:
        # ----- HOLD CONNECTION ONLY FOR DB WORK -----
        ...
        # Process transactions outside the DB connection
        ...
        # Write updated balances back into DB
        async with conn.cursor() as cursor:
            await cursor.execute(store_balance_query, (
                flo_address,
                int(balance_sat),
...
async def address_fetch(flo_address):
    ...
    try:
        ...
        # --- Acquire connection for DB work ---
        conn = await get_mysql_connection()
        try:
            # Fetch only raw DB results
            paginated_transactions, raw_response_data = await fetch_and_calculate_address_data(
                conn,
                flo_address,
                page,
                pagesize,
            ...
            # Release the connection immediately after DB operations
            await async_connection_pool.release(conn)

        # --- Do JSON transformation and formatting outside DB connection ---

        # If details is "basic", skip transaction serialization
        if details == "basic":
            return jsonify(raw_response_data)

        # Otherwise, build the tx list
        transactions = []
        for transaction in paginated_transactions:
            try:
                raw_transaction = json.loads(transaction['raw_transaction_json'])

                block_height_value = raw_transaction.get("blockheight") or transaction.get("blockheight")
                try:
                    block_height = int(block_height_value) if block_height_value is not None else None
                except (TypeError, ValueError):
                    block_height = None

                confirmations_from_best_height = (
                    BEST_HEIGHT - block_height + 1
                ) if (block_height is not None and BEST_HEIGHT) else 0
                if confirmations_from_best_height < 0:
                    confirmations_from_best_height = 0
                ...
                transactions.append(OrderedDict([
                    ...
                    ("blockhash", transaction["blockhash"]),
                    ("blockheight", raw_transaction.get("blockheight")),
                    ("confirmations", int(confirmations_from_best_height)),
                    ("time", transaction.get("time")),
                    ("valueIn", raw_transaction.get("valueIn", 0)),
                    ("valueOut", raw_transaction.get("valueOut", 0)),
                    ("fees", raw_transaction.get("fees", 0)),
        ...
        # Add the transactions list to the raw response data
        raw_response_data["txs"] = transactions
...
async def get_block_by_hash(blockhash):
    ...
    try:
        ...
        if isinstance(block_output, str):
            logger.error(f"Block processing returned string error for blockhash {blockhash}: {block_output}")
...
async def get_utxos_for_address(address):
    ...
    try:
        ...
        utxo_list = []
        for txid, vout, amount, satoshis, utxo_block_height, spent in utxos:
            # Compute confirmations from BEST_HEIGHT (not from DB aggregate)
            try:
                block_height = int(utxo_block_height) if utxo_block_height is not None else None
            except (TypeError, ValueError):
                block_height = None
            ...
            if confirmed_only and confirmations_from_best_height <= 0:
                continue
...
@address_indexer_app.route('/api/addr/<flo_address>/balance', methods=['GET'])
...
async def address_ui_direct(flo_address):
    ...
    if not re.match(r"^[Fe][a-zA-Z0-9]{33,34}$", flo_address):
            ...
            try:
                ...
                if topic == "hashblock":
                    ...
                    if block_height is not None:
                        ...
                        if isinstance(block_height, int) and block_height > BEST_HEIGHT:
...
async def check_mysqld_initialization():
    ...
    try:
        ...
        async with conn.cursor() as cursor:
            ...
            all_grants = " ".join([row[0] for row in grants]).upper()

            if "ALL PRIVILEGES" in all_grants:
                missing_privileges = []
            else:
                missing_privileges = [priv for priv in required_privileges if priv not in all_grants]
...
async def main():
    ...
    try:
        ...
        if RUN_FULL_UTXO_CLEANUP_ON_STARTUP:
            logger.info("Running full UTXO cleanup for duplicates.")
        ...
        hypercorn_config.keep_alive_timeout = 15
        hypercorn_config.max_app_queue_size = 100
        hypercorn_config.max_concurrency = 200
        hypercorn_config.backlog = 1024
