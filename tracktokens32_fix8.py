...
def process_token_transfer(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    try:
        ...
        try:
            ...
            rows = connection.execute("SELECT * FROM tokenAddressMapping WHERE tokenAddress = %s AND token = %s", (output_address, token_name)).fetchall()
            if not rows:
                connection.execute("INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) VALUES (%s, %s, %s, %s, %s)", (output_address, token_name, txid, transaction_data.get('blockheight'), transaction_data.get('blockhash')))
        except Exception as e:
...
def process_continuous_event_transfer(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd, connection, contract_session, contractStructure):
    try:
        ...
        if contract_subtype == 'tokenswap':
            ...
            add_contract_transaction_history(contract_name=parsed_data['contractName'], contract_address=outputlist[0], transactionType='participation', transactionSubType='swap', sourceFloAddress=inputlist[0], destFloAddress=outputlist[0], transferAmount=fulfilled_swapAmount, blockNumber=blockinfo['height'], blockHash=blockinfo['hash'], blocktime=blockinfo['time'], transactionHash=transaction_data['txid'], jsonData=json.dumps(transaction_data), parsedFloData=json.dumps(parsed_data))
...
def process_smart_contract_incorporation(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    # Normalize typo in contractType
    ...
    if not check_database_existence(
        ...
        systemdb_session = create_database_session_orm('system_dbs', {'db_name':'system'}, SystemBase)
        tokenAddressMapping_of_contractAddress = systemdb_session.query(TokenAddressMapping).filter(
...
def create_one_time_event_contract(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if parsed_data['contractAddress'] == inputadd:
        # Create smart_contract database session
        session = create_database_session_orm(
            'smart_contract',
            {
                'contract_name': parsed_data['contractName'],
        ...
        # Record ContractTransactionHistory
        ...
        # Create token association in token DB
        blockchainReference = urljoin(neturl, f"tx/{transaction_data['txid']}")
        session = create_database_session_orm('token', {'token_name': parsed_data['tokenIdentification']}, TokenBase)
        ...
        session = create_database_session_orm('system_dbs', {'db_name': "system"}, SystemBase)
        session.add(ActiveContracts(contractName=parsed_data['contractName'], contractAddress=parsed_data['contractAddress'], status='active', tokenIdentification=parsed_data['tokenIdentification'], contractType=parsed_data['contractType'], transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash'], incorporationDate=transaction_data['time']))
        session.commit()
        session.add(ContractAddressMapping(address=inputadd, addressType='incorporation', tokenAmount=None, contractName=parsed_data['contractName'], contractAddress=inputadd, transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash']))
        session.add(DatabaseTypeMapping(db_name=f"{parsed_data['contractName']}_{inputadd}", db_type='smartcontract', keyword='', object_format='', blockNumber=transaction_data['blockheight']))
        session.add(TimeActions(time=parsed_data['contractConditions']['expiryTime'], activity='contract-time-trigger', status='active', contractName=parsed_data['contractName'], contractAddress=inputadd, contractType='one-time-event-trigger', tokens_db=json.dumps([parsed_data['tokenIdentification']]), parsed_data=json.dumps(parsed_data), transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight']))
        session.commit()
        session.close()
...
def create_continuous_event_contract(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    if parsed_data['contractAddress'] == inputadd:
        # Create session for smart_contract database
        session = create_database_session_orm('smart_contract', {'contract_name': parsed_data['contractName'], 'contract_address': parsed_data['contractAddress']}, ContractBase)

        # Store contract header attributes
        session.add(ContractStructure(attribute='contractType', index=0, value=parsed_data['contractType'], blockNumber=blockinfo['height']))
        session.add(ContractStructure(attribute='contractName', index=0, value=parsed_data['contractName'], blockNumber=blockinfo['height']))
        session.add(ContractStructure(attribute='contractAddress', index=0, value=parsed_data['contractAddress'], blockNumber=blockinfo['height']))
        session.add(ContractStructure(attribute='flodata', index=0, value=parsed_data['flodata'], blockNumber=blockinfo['height']))
        ...
        if parsed_data['stateF'] not in [{}, False]:
        ...
        if 'subtype' in parsed_data['contractConditions']:
            if (
                parsed_data['contractConditions']['subtype'] == 'tokenswap'
                and check_database_existence('token', {'token_name': parsed_data['contractConditions']['selling_token'].split('#')[0]})
                and check_database_existence('token', {'token_name': parsed_data['contractConditions']['accepting_token'].split('#')[0]})
            ):
                ...
                # Save pricetype and related info
                ...
                elif parsed_data['contractConditions']['pricetype'] == 'dynamic':
                    session.add(ContractStructure(attribute='price', index=0, value=parsed_data['contractConditions']['price'], blockNumber=blockinfo['height']))
                ...
                # Record contract transaction history
                blockchainReference = urljoin(neturl, f"tx/{transaction_data['txid']}")
                ...
                for token_name in accepting_sending_tokenlist:
                    ...
                    token_session.add(TokenContractAssociation(tokenIdentification=token_name, contractName=parsed_data['contractName'], contractAddress=parsed_data['contractAddress'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash'], time=transaction_data['time'], transactionHash=transaction_data['txid'], blockchainReference=blockchainReference, jsonData=json.dumps(transaction_data), transactionType=parsed_data['type'], parsedFloData=json.dumps(parsed_data)))
                    token_session.commit()
                    token_session.close()

                # Register smart contract address in system DB
                system_session = create_database_session_orm('system_dbs', {'db_name': "system"}, SystemBase)
                system_session.add(ActiveContracts(contractName=parsed_data['contractName'], contractAddress=parsed_data['contractAddress'], status='active', tokenIdentification=str(accepting_sending_tokenlist), contractType=parsed_data['contractType'], transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash'], incorporationDate=transaction_data['time']))
                system_session.commit()
                system_session.add(ContractAddressMapping(address=inputadd, addressType='incorporation', tokenAmount=None, contractName=parsed_data['contractName'], contractAddress=inputadd, transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash']))
                system_session.add(DatabaseTypeMapping(db_name=f"{parsed_data['contractName']}_{inputadd}", db_type='smartcontract', keyword='', object_format='', blockNumber=transaction_data['blockheight']))
                system_session.commit()
                system_session.close()
                updateLatestTransaction(transaction_data, parsed_data, f"{parsed_data['contractName']}-{parsed_data['contractAddress']}")
...
def process_smart_contract_pays(parsed_data, transaction_data, blockinfo, inputlist, outputlist, inputadd):
    ...
    try:
        ...
        participantAdd_txhash = connection.execute("SELECT sourceFloAddress, transactionHash FROM contractTransactionHistory WHERE transactionType != 'incorporation'").fetchall()
    ...
    try:
        if check_database_existence('smart_contract', {'contract_name': f"{parsed_data['contractName']}", 'contract_address': f"{outputlist[0]}"}):
            ...
            if not hasattr(expiry_time, "timestamp"):
                logger.error(f"Expiry time object has no attribute 'timestamp'. Rejecting transaction {transaction_data['txid']}"); return 0
            if blockinfo['time'] > expiry_time.timestamp:
                ...
                logger.warning(rejectComment); rejected_contract_transaction_history(transaction_data, parsed_data, 'deposit', outputlist[0], inputadd, outputlist[0], rejectComment); return 0
            connection = create_database_connection('smart_contract', {'contract_name': parsed_data['contractName'], 'contract_address': outputlist[0]})
            try: participantAdd_txhash = connection.execute('SELECT participantAddress, transactionHash FROM contractparticipants').fetchall()
            ...
            if 'contractAddress' in parsed_data and parsed_data['contractAddress'] != outputlist[0]:
                ...
                logger.info(rejectComment); rejected_contract_transaction_history(transaction_data, parsed_data, 'deposit', outputlist[0], inputadd, outputlist[0], rejectComment); pushData_SSEapi(f"Error| Mismatch in contract address specified in flodata and transaction output address in {transaction_data['txid']}"); return 0
            contractStructure = extract_contractStructure(parsed_data['contractName'], outputlist[0])
            ...
            if returnval == 0:
                logger.info("Something went wrong in the token transfer method"); pushData_SSEapi(f"Error | Something went wrong while doing internal db transactions for {transaction_data['txid']}"); return 0
            session = create_database_session_orm('smart_contract', {'contract_name': parsed_data['contractName'], 'contract_address': outputlist[0]}, ContractBase)
            try:
                blockchainReference = urljoin(neturl, f"tx/{transaction_data['txid']}")
                session.add(ContractDeposits(depositorAddress=inputadd, depositAmount=parsed_data['depositAmount'], depositBalance=parsed_data['depositAmount'], expiryTime=parsed_data['depositConditions']['expiryTime'], unix_expiryTime=expiry_time.timestamp, status='active', transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash']))
                session.add(ContractTransactionHistory(transactionType='smartContractDeposit', transactionSubType=None, sourceFloAddress=inputadd, destFloAddress=outputlist[0], transferAmount=parsed_data['depositAmount'], blockNumber=transaction_data['blockheight'], blockHash=transaction_data['blockhash'], time=transaction_data['time'], transactionHash=transaction_data['txid'], blockchainReference=blockchainReference, jsonData=json.dumps(transaction_data), parsedFloData=json.dumps(parsed_data))); session.commit()
            except Exception as e:
                session.rollback(); logger.error(f"Error while saving contract deposit: {e}"); return 0
            finally:
                session.close()
            systemdb_session = create_database_session_orm('system_dbs', {'db_name': 'system'}, SystemBase)
            try:
                systemdb_session.add(TimeActions(time=parsed_data['depositConditions']['expiryTime'], activity='contract-deposit', status='active', contractName=parsed_data['contractName'], contractAddress=outputlist[0], contractType='continuous-event-swap', tokens_db=json.dumps([parsed_data['tokenIdentification']]), parsed_data=json.dumps(parsed_data), transactionHash=transaction_data['txid'], blockNumber=transaction_data['blockheight'])); systemdb_session.commit()
            except Exception as e:
                systemdb_session.rollback(); logger.error(f"Failed inserting into TimeActions: {e}"); return 0
            finally:
                systemdb_session.close()
            systemdb_connection = create_database_connection('system_dbs', {'db_name': 'system'})
            try:
                firstInteractionCheck = systemdb_connection.execute("SELECT * FROM tokenAddressMapping WHERE tokenAddress = :addr AND token = :token", {'addr': outputlist[0], 'token': parsed_data['tokenIdentification']}).fetchall()
                if not firstInteractionCheck:
                    systemdb_connection.execute("INSERT INTO tokenAddressMapping (tokenAddress, token, transactionHash, blockNumber, blockHash) VALUES (:address, :token, :txid, :blocknumber, :blockhash)", {'address': outputlist[0], 'token': parsed_data['tokenIdentification'], 'txid': transaction_data['txid'], 'blocknumber': transaction_data['blockheight'], 'blockhash': transaction_data['blockhash']})
            finally:
                systemdb_connection.close()
            updateLatestTransaction(transaction_data, parsed_data, f"{parsed_data['contractName']}_{outputlist[0]}")
            pushData_SSEapi(f"Deposit Smart Contract Transaction {transaction_data['txid']} for smart contract {parsed_data['contractName']} at address {outputlist[0]}")
            return 1
        else:
            ...
            logger.info(rejectComment); rejected_contract_transaction_history(transaction_data, parsed_data, 'smartContractDeposit', outputlist[0], inputadd, outputlist[0], rejectComment); return 0
    except Exception as e:
        logger.exception(f"Exception occurred while processing smart contract deposit for tx {transaction_data['txid']}: {e}"); return 0
