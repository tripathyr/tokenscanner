...
async def store_transaction_data(transaction_data):
    ...
    INSERT INTO `{ADDRESS_INDEXER_DB_NAME}`.utxos (
        txid, vout, address, amount, satoshis, block_height, spent, spent_at_block
    ) VALUES (%s, %s, %s, %s, %s, %s, FALSE, NULL)
    """
    utxo_update_query = f"""
    UPDATE `{ADDRESS_INDEXER_DB_NAME}`.utxos
    SET spent = TRUE, spent_at_block = %s
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            for index, vout in enumerate(transaction_data.get("vout", [])):
                if "scriptPubKey" in vout and "addresses" in vout["scriptPubKey"]:
                    ...
                    for address in addresses:
                        new_utxos.append((transaction_data["txid"], index, address, vout["value"], int(value), transaction_data.get("blockheight", 0)))
            ...
            for spent_txid, spent_vout in spent_utxos:
                await cursor.execute(utxo_update_query, (blockheight, spent_txid, spent_vout))
            ...
            try:
                if "txid" in vin and "vout" in vin:
                    ...
                    if "result" in prev_tx_response:
                        ...
                        # Mark the UTXO as spent instead of deleting it
                        await cursor.execute(
                            f"""
                            UPDATE `{ADDRESS_INDEXER_DB_NAME}`.utxos
                            SET spent = TRUE, spent_at_block = %s
                            WHERE txid = %s AND vout = %s
                            """,
                            (block_height, prev_txid, vout_index)
                        )
                        logger.info(f"Marked UTXO as spent: txid={prev_txid}, vout={vout_index}")
            except Exception as e:
