...
def extract_contract_conditions(text, contract_type, marker=None, blocktime=None):
    ...
    try:
        rulestext = extract_substring_between(
            text, 'contract-conditions', 'end-contract-conditions'
        )
...
def extract_tokenswap_contract_conditions(processed_text, contract_type, contract_token):
    ...
    # ✅ Extract clean block using extract_substring_between
    try:
        rulestext = extract_substring_between(
            processed_text,
            'contract-conditions',
...
def process_smart_contract_creation_ce_tokenswap(
    ...
    try:
        ...
        if price_type in ['determined', 'predetermined']:
            ...
            # Patch: restore original-case oracle address
            oracle_addr_original = find_original_case_regex(oracle_addr, clean_text)

            if not check_flo_address(oracle_addr_original):
                return outputreturn('noise')
...
def parse_floData(text, blockinfo, net):
    ...
    if categorization == 'tokensystem-C':
        ...
        return process_smart_contract_creation_C(
            first_classification,
            processed_text,
            clean_text,
            blockinfo,
        ...
        return process_smart_contract_participation_deposit_C(
            first_classification,
            processed_text,
            clean_text,
            blockinfo,
        ...
        return process_smart_contract_participation_ote_ce_C(
            first_classification,
            processed_text,
            clean_text,
            stateF_mapping,
        ...
        return process_userchoice_trigger(
            first_classification,
            processed_text,
            stateF_mapping
        )
        ...
        return process_smart_contract_creation_ce_tokenswap(
            first_classification,
            processed_text,
            clean_text,
            blockinfo,
