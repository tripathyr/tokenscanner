...
async def store_transaction_data(transaction_data):
    ...
    """
    utxo_insert_query = f"""
    INSERT INTO `{ADDRESS_INDEXER_DB_NAME}`.utxos (txid, vout, address, value, spent)
    VALUES (%s, %s, %s, %s, FALSE)
    """
    utxo_update_query = f"""
    UPDATE `{ADDRESS_INDEXER_DB_NAME}`.utxos
    SET spent = TRUE
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            for index, vout in enumerate(transaction_data.get("vout", [])):
                if "scriptPubKey" in vout and "addresses" in vout["scriptPubKey"]:
                    ...
                    for address in addresses:
                        new_utxos.append((transaction_data["txid"], index, address, value))
            ...
            for spent_txid, spent_vout in spent_utxos:
                await cursor.execute(utxo_update_query, (spent_txid, spent_vout))
                logger.info(f"Marked UTXO {spent_txid}:{spent_vout} as spent.")

            # Insert new UTXOs
            for utxo in new_utxos:
                await cursor.execute(utxo_insert_query, utxo)
                logger.info(f"Inserted new UTXO: {utxo}")
...
async def process_all_transactions_for_coinbase(conn, flo_address, results, page=1, page_size=1000):
    ...
    # Query to insert UTXOs
    insert_utxo_query = """
    INSERT INTO utxos (txid, vout, address, value, spent)
    VALUES (%s, %s, %s, %s, FALSE)
    ON DUPLICATE KEY UPDATE spent = FALSE;
    """
                        ...
                        async with conn.cursor() as cursor:
                            for address in addresses:
                                if address == flo_address:
                                    await cursor.execute(
                                        insert_utxo_query,
                                        (coinbase_tx["txid"], index, address, value * Decimal(1e8))
                                    )
    ...
    try:
        ...
        async with conn.cursor() as cursor:
            ...
            if not result or result[0] is None:
                return jsonify({"error": "Unable to fetch current block height"}), 500
...
async def rollback_all_blocks(conn, start_block, end_block):
    ...
    adjust the balances of associated indexed addresses, and update UTXOs.
    ...
    """
    # Queries for UTXO handling
    restore_spent_utxos_query = """
    UPDATE utxos
    SET spent = FALSE
    WHERE txid IN (
        SELECT vin.txid
    ...
    # Other existing queries (balance updates, block deletes, etc.) remain unchanged.
        ...
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            for transaction in transactions_to_rollback:
                try:
                    ...
                    # Restore spent UTXOs (mark as unspent)
                    for vin in raw_transaction.get("vin", []):
                        if "txid" in vin and "vout" in vin:
                            await cursor.execute(
                                """
                            ...
                            logger.info(f"Restored UTXO {vin['txid']}:{vin['vout']} as unspent.")

                    # Remove created UTXOs (mark as invalid)
                    for index, vout in enumerate(raw_transaction.get("vout", [])):
                        await cursor.execute(
                            """
                            DELETE FROM utxos
                            WHERE txid = %s AND vout = %s
                        ...
                        logger.info(f"Deleted created UTXO {txid}:{index}.")
            ...
            # Apply UTXO-related updates
            await cursor.execute(restore_spent_utxos_query, (start_block, end_block, start_block, end_block))
