...
async def get_optimal_pool_size():
    ...
    CPU_LOW_THRESHOLD = 40     # Below this, use the minimum pool size
    CPU_HIGH_THRESHOLD = 80    # Above this, use the maximum pool size
    ...
    if cpu_usage < CPU_LOW_THRESHOLD:
        return POOL_MIN_SIZE  # Return the minimum pool size
    elif cpu_usage > CPU_HIGH_THRESHOLD:
        return POOL_MAX_SIZE  # Return the maximum pool size
    else:
        # Scale proportionally between POOL_MIN_SIZE and POOL_MAX_SIZE
        scaled_size = int(POOL_MIN_SIZE + ((cpu_usage - CPU_LOW_THRESHOLD) / (CPU_HIGH_THRESHOLD - CPU_LOW_THRESHOLD)) * (POOL_MAX_SIZE - POOL_MIN_SIZE))
