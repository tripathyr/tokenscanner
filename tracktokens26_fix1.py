    ...
    try:
        ...
        while block_number > 0:
            ...
            try:
                ...
                api_url = f'{BLOCKBOOK_API_URL}api/blockheight/{block_number}'
                ...
                block_hash = response_json['hash']

            except (KeyError, requests.exceptions.RequestException) as e:
                logger.info(f"Primary API failed (blockheight). Attempting fallback to v2/block-index. Error: {e}")

                try:
                    fallback_url = f'{BLOCKBOOK_API_URL}api/v2/block-index/{block_number}'
                    logger.info(f"Fallback API Request URL: {fallback_url}")

                    response = requests.get(fallback_url, verify=API_VERIFY, timeout=RETRY_TIMEOUT_SHORT)
                    response.raise_for_status()
                    response_json = response.json()
                    block_hash = response_json['blockHash']

                except (KeyError, requests.exceptions.RequestException) as e2:
                    logger.error(f"Fallback API failed too for block number {block_number}. Error: {e2}")
                    raise
...
def get_blockhash_by_height(blockindex):
    """
    Attempts to fetch the block hash by height using primary and fallback Blockbook APIs.
    Returns block hash as a string or raises an Exception if all attempts fail.
    """
    while True:
        # Try primary API
        ...
        try:
            ...
            logger.info(f"Primary API blockheight/{blockindex} failed. Trying fallback v2/block-index...")

        # Try fallback API
        try:
            response = newMultiRequest(f"v2/block-index/{blockindex}")
            if 'blockHash' in response:
                return response['blockHash']
            else:
                logger.info(f"Fallback response missing 'blockHash' key. Retrying...")
        except Exception as e:
            logger.info(f"Fallback API v2/block-index/{blockindex} also failed with error: {e}. Retrying...")
...
def check_reorg():
    ...
    while block_number > 0:
        # get the block hash from local DB
        block_hash = list(connection.execute(
            f"SELECT blockHash FROM latestBlocks WHERE blockNumber = {block_number}"
        ).fetchone())[0]

        try:
            # Try primary Blockbook endpoint
            response = requests.get(
                f'{blockbook_api_url}api/blockheight/{block_number}',
                verify=API_VERIFY,
                timeout=RETRY_TIMEOUT_SHORT
            )
            response.raise_for_status()
            blockbook_json = response.json()
            remote_block_hash = blockbook_json.get('hash')

            # Fallback if 'hash' is not in response
            if not remote_block_hash:
                raise KeyError("Missing 'hash' in /api/blockheight response")

        except (requests.RequestException, KeyError):
            logger.warning(f"Primary API failed for block {block_number}, trying fallback")

            try:
                fallback_response = requests.get(
                    f'{blockbook_api_url}api/v2/block-index/{block_number}',
                    verify=API_VERIFY,
                    timeout=RETRY_TIMEOUT_SHORT
                )
                fallback_response.raise_for_status()
                fallback_json = fallback_response.json()
                remote_block_hash = fallback_json.get('blockHash')

                if not remote_block_hash:
                    raise KeyError("Missing 'blockHash' in fallback response")

            except Exception as e:
                logger.error(f"Fallback API also failed for block {block_number}: {e}")
                sys.exit(0)  # TODO: Replace with retry logic or softer failure

        if remote_block_hash == block_hash:
            # Match found, no reorg for this block
            break
        else:
            # Possible reorg, backtrack
            logger.info(f"Block hash mismatch at height {block_number}, checking earlier block")
...
def scanBlockchain():
    ...
    while current_index == -1:
        ...
        try:
            current_index = response.get('bestHeight') or response['blockbook']['bestHeight']
        except:
            logger.info('Latest block count response not in expected format. Logging raw response and retrying...')
...
def reconnectWebsocket(socket_variable):
    ...
    while not socket_variable.connected:
        ...
        try:
            ...
            logger.info(f"Reconnect failed for {newurl}")
...
def initialize_neturl(blockbook_neturl_list):
    """
    Initialize `neturl` by selecting a random server and testing its availability and validity.
    Perform failover if the selected server fails or the response is invalid.

    :param blockbook_neturl_list: List of available BLOCKBOOK_NETURL servers
    :return: A valid `neturl`
    """
    tried_servers = []
    ...
    while len(tried_servers) < len(blockbook_neturl_list):
        ...
        try:
            # Remove trailing slash and test the `/api/latest-block` endpoint
            test_url = f"{neturl.rstrip('/')}/api/latest-block"
            ...
            # Accept original OR blockbook-compatible response structure
            if (
                (
                ...
                print(f"Selected BLOCKBOOK_NETURL: {neturl}")
                return neturl


            else:
                raise Exception(f"Invalid response data: {response_data}")

        except requests.exceptions.Timeout:
            print(f"Timeout with BLOCKBOOK_NETURL: {neturl}")
        except requests.exceptions.RequestException as e:
            print(f"Error with BLOCKBOOK_NETURL: {neturl}, {e}")
        except Exception as e:
            print(f"Error with {neturl}: {type(e).__name__}: {e}")
...
def newMultiRequest(apicall):
    ...
    while True:  # Infinite loop
        try:
            ...
            response = requests.get(f"{current_server}/api/{apicall}", verify=API_VERIFY, timeout=RETRY_TIMEOUT_SHORT)
            ...
            if response.status_code == 200:
                try:
                    ...
                    # Normalize transaction data for compatibility
                    if 'txs' in response_data:  # If the response contains transactions
                        for tx in response_data['txs']:
                            normalize_transaction_data(tx)  # Apply transformation
...
# Load server list from BLOCKBOOK_URL_LIST
serverlist = [url.strip().rstrip('/') for url in config['DEFAULT']['BLOCKBOOK_URL_LIST'].split(',')]
...
# Set websocket URI
websocket_uri = f"{serverlist[0].replace('https', 'wss')}/websocket"

# Initialize working Blockbook URL
try:
    neturl = initialize_neturl(serverlist)
except Exception as e:
...
# SSE endpoint
tokenapi_sse_url = config['DEFAULT']['TOKENAPI_SSE_URL'].rstrip('/')
...
def fetch_current_block_height():
    ...
    while current_index == -1:  # Keep trying until a valid block height is retrieved
        try:
            response = newMultiRequest('latest-block')  # Make the API call
            current_index = response.get('bestHeight') or response['blockbook']['bestHeight']
