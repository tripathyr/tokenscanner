    ...
    try:
        async with aiohttp.ClientSession(auth=auth) as session:
            async with session.post(RPC_ADDRESS, json=payload) as response:
                raw = await response.read()
                text = raw.decode("utf-8", errors="replace")
                logger.info(f"RPC raw tx response for {txid}: {text[:200]}... (truncated)")
                return json.loads(text)
    except Exception as e:
        logger.error(f"Error fetching tx {txid}: {e}", exc_info=True)
        return {}
...
async def b_compute_vin_values(vin):
    for vin_entry in vin:
        try:
            ...
            if not txid or vout_index < 0:
                continue

            logger.info(f"VIN: Fetching tx={txid}, vout={vout_index}")
            tx_data = await b_fetch_transaction(txid)
            logger.info(f"VIN: tx_data received for {txid} (len={len(str(tx_data))})")
            ...
            if vout_index < len(vout_list):
                ...
                logger.warning(f"VIN: vout index {vout_index} out of range for txid {txid}")
                vin_entry.update({"value": 0, "valueSat": 0, "addresses": [], "isAddress": False})

            # 🚫 Strip heavy raw fields
            vin_entry.pop("scriptSig", None)
            vin_entry.pop("txinwitness", None)
            vin_entry.pop("witness", None)
            vin_entry.pop("sequence", None)

        except Exception as e:
            logger.error(f"VIN: Failed to process txid={txid}, vout={vout_index}: {e}", exc_info=True)
            vin_entry.update({"value": 0, "valueSat": 0, "addresses": [], "isAddress": False})
    return vin
...
async def b_process_transactions(transactions, blockhash, blockheight):
    ...
    for tx in transactions:
        ...
        # Strip heavy fields from vin before returning
        for vin in tx["vin"]:
            vin.pop("scriptSig", None)
            vin.pop("txinwitness", None)
            vin.pop("sequence", None)
            vin.pop("scriptWitness", None)
...
async def b_process_block(blockhash):
    try:
        ...
        for tx in txs:
            try:
                logger.info(f"Processing TX {tx['txid']}")
                ...
                # Compute transaction-level fields (valueIn, valueOut, fees)
                await b_compute_transaction_values(tx, block_result["hash"], blockheight)

                processed_txs.append(tx)

            except Exception as e:
                logger.warning(f"Skipping tx {tx.get('txid')} due to error: {e}", exc_info=True)
                skipped_txs.append(tx.get("txid"))

        output = OrderedDict([
            ("hash", block_result["hash"]),
            ("height", blockheight),
            ("txCount", len(processed_txs)),
            ("skippedTxs", skipped_txs),
            ("txs", processed_txs),
        ])
        ...
        logger.error(f"Error in b_process_block for {blockhash}: {e}", exc_info=True)
...
async def find_and_process_missing_blocks(batch_size=200, max_concurrent_batches=5):
    try:
