...
async def initialize_database():
    ...
    create_utxos_table_query = """
        CREATE TABLE IF NOT EXISTS utxos (
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            UNIQUE KEY idx_utxo (txid, vout), -- Ensure uniqueness for each UTXO
            INDEX idx_address (address),      -- Index for fast lookup by address
            INDEX idx_height (height)         -- Index for fast lookup by block height
        );
        """
...
async def process_blocks(start_block, end_block, batch_size=2000, max_concurrent_batches=5, retries=5):
    ...
    async def process_transaction(cursor, tx, blockhash, block_height, block_time):
        ...
        # Process `vin` to fetch input addresses and remove spent UTXOs
            ...
            try:
                if "txid" in vin and "vout" in vin:
                    ...
                    if "result" in prev_tx_response:
                        ...
                        logger.info(f"Removed spent UTXO: txid={prev_txid}, vout={vout_index}")
            except Exception as e:
                ...
                if "scriptPubKey" in vout and "addresses" in vout["scriptPubKey"]:
                    ...
                    await cursor.execute(
                        ...
                        INSERT INTO `{ADDRESS_INDEXER_DB_NAME}`.utxos (
                            address, txid, vout, amount, satoshis, height, confirmations
                        ...
                        (
                            ...
                            block_height,
                            0  # Confirmations will be updated in batch processing
                        )
                    )
