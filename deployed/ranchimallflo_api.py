...
def update_transaction_confirmations(transactionJson):
    url = f"{apiUrl}api/tx/{transactionJson['txid']}"
...
def fetch_dynamic_swap_price(contractStructure, blockinfo):
    ...
    response = requests.get(f'{apiUrl}api/address/{oracle_address}?details=txs')
    if response.status_code == 200:
...
async def categoriseString(urlstring):
    ...
    response = requests.get('{}api/tx/{}'.format(apiUrl, urlstring))
    if response.status_code == 200:
        return jsonify(type='transaction')
    else:
        response = requests.get('{}api/block/{}'.format(apiUrl, urlstring))
        if response.status_code == 200:
